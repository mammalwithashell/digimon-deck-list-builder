...
| `POST /admin/models/{id}/confirm` | admin | HEAD the key → populate `file_size_bytes`; stream-hash → populate `file_sha256`; download-once to a temp file (actually: use `iter_object_chunks` to stream into a `NamedTemporaryFile(suffix=".onnx", delete=False)`; run `onnxruntime.InferenceSession` on the file for tensor/action shape extraction; `os.unlink` in `finally`). Set `state='uploaded'`. Idempotent: re-running on an already-confirmed row re-hashes and updates columns. On any error, set `state='failed'`, delete the Spaces object, return 422 with the exception message. |
