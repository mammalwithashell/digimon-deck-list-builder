...
description: Use when implementing a Digimon archetype, deck, card group, or card list as Rust engine YAML DSL cards rather than only assessing readiness or planning gap groups.
---
...
**REQUIRED SUB-SKILLS:** Use `superpowers:test-driven-development`, `superpowers:subagent-driven-development`, and `superpowers:verification-before-completion`. Use `assess-archetype-rust` for card-pool and gap-resolution evidence.
...
3. Run a readiness pass before writing code.
   - Reuse `assess-archetype-rust` logic.
