  ...
  (a) =>
    agent(
      `${COMMON}\nTASK: Phases 4-5 of the skill for "${a.scout.canonicalName}" (slug "${a.slug}").\n` +
        ...
        `FAITHFULNESS RULES (the reviewer rejects violations): (a) Use REAL implemented DSL cards for EVERY role — named combo pieces (enablers, payoffs, keyword sources) AND fillers, neutral targets, opponents, and stack bases — loaded by real card ID via dsl_card/dsl_builder. Synthetic make_test_card is a LAST RESORT, allowed only when NO real implemented DSL card can fill a role (then add a one-line comment naming the role + why); prefer effectless/vanilla real DSL Digimon for filler/target roles so their effects don't perturb the asserted outcome. (b) Exercise each card's ACTUAL ability — play the Option through its real action, trigger the keyword via the real trigger path — do NOT call a low-level engine helper (e.g. \`delete_permanent_with_effects\`) that bypasses the named card's own effect, or the combo is not really tested. (c) Do NOT weaken an assertion to make it pass; if the faithful outcome can't be produced, mark the test \`#[ignore]\` with the reason and route the gap to the trackers.\n` +
  ...
  (prev, a) => {
    ...
    return agent(
      `${COMMON}\nTASK: REVISE the interaction tests + model for "${a.scout.canonicalName}" (slug "${a.slug}") to resolve the reviewer's blocker/major findings. Do NOT re-implement any card or weaken assertions.\n` +
        ...
        `\n\nApply the FAITHFULNESS RULES: write the \`-model.md\` system model if it is missing (do not cite a per-card doc); replace synthetic make_test_card cards with real implemented DSL cards for ALL roles (not just named pieces — also fillers/targets/opponents/stack bases), keeping a synthetic only as a documented last resort when no real card fits; exercise the card's ACTUAL ability rather than a bypassing engine helper. If a faithful outcome genuinely can't be produced, \`#[ignore]\` the test with the reason and file the gap. Do NOT edit \`tests/archetypes/main.rs\`.\n` +
