# BT17-028 — AncientGarurumon

## Alt paths (parsed from xros_req)

_(none)_

## Source record

```json
{
  "attribute_eng": [
    "Data"
  ],
  "card_colors": [
    1,
    2
  ],
  "card_effect_class_name": "BT17_028",
  "card_id": "BT17-028",
  "card_index": 28,
  "card_kind": 0,
  "card_name_eng": "AncientGarurumon",
  "card_name_jpn": "",
  "digixros_costs": [
    {
      "different_card_numbers": false,
      "different_names": false,
      "elements": [
        {
          "color": null,
          "count": 1,
          "is_digimon_only": false,
          "level_max": null,
          "name_contains": "Lobomon",
          "trait_alternatives": [],
          "trait_match": ""
        },
        {
          "color": null,
          "count": 1,
          "is_digimon_only": false,
          "level_max": null,
          "name_contains": "KendoGarurumon",
          "trait_alternatives": [],
          "trait_match": ""
        }
      ],
      "has_text": "",
      "max_materials": 2,
      "reduce_cost_per_card": 3,
      "source_zones": [
        "hand",
        "field"
      ]
    }
  ],
  "dp": 12000,
  "effect_description_eng": "[On Play] [When Digivolving] Return 1 of your opponent's Digimon with the lowest level to the hand.\r\n[Your Turn] [Once Per Turn] When an effect adds cards to your or your opponent's hand, your opponent adds the top card of their security stack to the hand.\r\n[On Deletion] You may return 1 Tamer card and 1 [Hybrid] trait Digimon card from your trash to the hand. Then, you may play 1 Tamer card from your hand without paying the cost.",
  "evo_costs": [
    {
      "card_color": 1,
      "level": 5,
      "memory_cost": 4
    }
  ],
  "form_eng": [],
  "index": 1812,
  "inherited_effect_description_eng": "",
  "level": 6,
  "norm_id": 0.0906,
  "play_cost": 12,
  "rarity": 3,
  "security_effect_description_eng": "",
  "type_eng": [
    "Ancient Animal",
    "Ten Warriors"
  ],
  "xros_req": "DigiXros Requirements [DigiXros -3] [Lobomon] x [KendoGarurumon] \r\nWhen this would be played, you may place specified cards from your hand/battle area under it. Each one reduces the play cost."
}
```

## Unparsed xros_req

```text
DigiXros Requirements [DigiXros -3] [Lobomon] x [KendoGarurumon]
When this would be played, you may place specified cards from your hand/battle area under it. Each one reduces the play cost.
```
