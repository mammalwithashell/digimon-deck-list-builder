...
class P_102(CardScript):
    """P-102 SkullGreymon | Lv.5"""
        ...
        def process2(ctx: Dict[str, Any]):
            ...
            def play_filter(c):
                if not getattr(c, 'is_digimon', False):
                    return False
                if not ('Purple' in [col.name for col in getattr(c, 'card_colors', [])] or 'Red' in [col.name for col in getattr(c, 'card_colors', [])]):
                    return False
                return True
