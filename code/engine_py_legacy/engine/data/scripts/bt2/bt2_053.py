...
class BT2_053(CardScript):
    """BT2-053 Keramon | Lv.3 Rookie | Black | Unidentified

    Inherited Effect: [Your Turn] When you play another Digimon with the
        same name as this Digimon, <Draw 1>.
    """

    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        effects = []

        # --- Effect 0: Inherited [Your Turn] Draw 1 when same-name played ---
        effect0 = ICardEffect()
        effect0.set_timing(EffectTiming.OnEnterFieldAnyone)
        effect0.set_effect_name("BT2-053 Inherited: Draw 1 on same-name play")
        effect0.set_effect_description(
            "[Your Turn] When you play another Digimon with the same name "
            "as this Digimon, <Draw 1>."
        )
