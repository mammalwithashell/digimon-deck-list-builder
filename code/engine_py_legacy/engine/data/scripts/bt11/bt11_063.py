...
class BT11_063(CardScript):
    """BT11-063 Geremon | Lv.4"""

    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        effects = []
        card.also_treated_as_names = ['Numemon']

        # Timing: EffectTiming.None
        # Also Treated As
        effect0 = ICardEffect()
        effect0.set_effect_name("BT11-063 Also treated as [Numemon]")
        ...
        def process1(ctx: Dict[str, Any]):
            ...
            if not (player and game):
                return
            def hand_filter(c):
                if not (any('Numemon' in _n or 'Sukamon' in _n or 'Nanimon' in _n or 'Etemon' in _n for _n in getattr(c, 'card_names', []))):
                    return False
                return True
