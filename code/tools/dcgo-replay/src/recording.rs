...
//! `encoder_failure`, and `game_end`. Unknown row types are tolerated (logged
//! but not fatal) so old-harness-new-recorder pairings don't break catastrophically.

use serde::{Deserialize, Serialize};

/// Schema version this build of the harness understands. Bumped in lockstep
/// with `digimon_engine::action::space::SCHEMA_VERSION` when the action-space
/// layout or wire format changes.
...
/// The harness maps these strings to `digimon_engine::opaque_deck::RevealKind`
...
pub struct RecordingV1 {
    pub start: GameStart,
    pub rows: Vec<Row>, // includes Action, EncoderFailure, Unknown rows in order
    pub end: GameEnd,
}
