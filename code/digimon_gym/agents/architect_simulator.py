...
Runs simulated games between decks using the Rust engine
(``digimon_engine.RustHeadlessGame``) and returns win rates. Serves as the
black-box evaluator for the deck-building architect agent.

No DB imports. No auth. Pure engine + inference.

Migrated off the sunset Python engine (excise-legacy-engine-from-hosted-api):
the win-rate simulation runs on ``RustHeadlessGame`` and the greedy opponent
uses the engine's native ``greedy_action`` (the same heuristic used across
training and desktop), replacing the former hand-rolled Python-engine
heuristic.
"""
...
# NOTE: the former 225-line `_headless_greedy_action` Python-engine heuristic
# was removed (excise-legacy-engine-from-hosted-api). The greedy opponent now
# uses the Rust engine's native `RustHeadlessGame.greedy_action()` — see
# `_HeadlessGreedyPolicy` below.


class _HeadlessGreedyPolicy:
    """Greedy policy backed by the Rust engine's native ``greedy_action``.

    Holds a reference to the active ``RustHeadlessGame`` runner (bound via
    ``bind_runner`` before each game loop) and delegates the decision to
    ``runner.greedy_action()`` — the same greedy heuristic used across
    training and desktop.
    """
    ...
    def predict(self, obs: np.ndarray, mask: np.ndarray) -> int:
        ...
        return int(self._runner.greedy_action())
...
def _run_matchup(
    ...
    for i in range(n_games):
        ...
        try:
            # Even-parity seed keeps the first player constant (seed % 2),
            # so the d1/d2 side-swap above is what removes first-player bias.
            runner = RustHeadlessGame(d1, d2, seed=2 * i)
