...
fn main() {
    ...
    let model = args.next().expect("usage: evaluator_bench <model.onnx> <obs_size>");
    ...
    for &batch in &[1usize, 8, 32, 128, 512] {
        ...
        for _ in 0..3 {
            evaluator.evaluate_batch(&obs_refs, &mask_refs).expect("warmup");
        }
        let iters = (2000 / batch).max(5);
        let t = Instant::now();
        for _ in 0..iters {
            let r = evaluator.evaluate_batch(&obs_refs, &mask_refs).expect("eval");
