...
fn push_to_hand(r: &mut DebugRunner, player: digimon_engine::enums::PlayerId, card_id: &str) -> CardHandle {
    let data_idx = r
        .game
        .card_data
        .iter()
...
fn ordered_permutation_collects_picks_in_order() {
    ...
    {
        let sel = r.game.pending_selection.as_ref().expect("step 1 must park a selection");
        assert_eq!(
            sel.kind,
            SelectionKind::OrderedPermutation { remaining: 3 },
            "step 1 kind must be OrderedPermutation {{ remaining: 3 }}"
        );
        assert_eq!(sel.valid_action_ids.len(), 3, "step 1 must offer 3 action IDs");
    ...
    {
        let sel = r.game.pending_selection.as_ref().expect("step 2 must park a selection");
        assert_eq!(
            sel.kind,
            SelectionKind::OrderedPermutation { remaining: 2 },
            "step 2 kind must be OrderedPermutation {{ remaining: 2 }}"
        );
        assert_eq!(sel.valid_action_ids.len(), 2, "step 2 must offer 2 action IDs");
        assert_eq!(r.game.current_phase, GamePhase::SelectPermutation);
    ...
    {
        let sel = r.game.pending_selection.as_ref().expect("step 3 must park a selection");
        assert_eq!(
            sel.kind,
            SelectionKind::OrderedPermutation { remaining: 1 },
            "step 3 kind must be OrderedPermutation {{ remaining: 1 }}"
        );
        assert_eq!(sel.valid_action_ids.len(), 1, "step 3 must offer exactly 1 action ID");
        assert_eq!(r.game.current_phase, GamePhase::SelectPermutation);
    ...
    let result = delivered.lock().unwrap().take().expect("final callback must have fired");
    assert_eq!(
        result,
...
fn singleton_surfaces_as_one_choice_selection() {
    ...
    let result = delivered.lock().unwrap().take().expect("callback must have fired");
