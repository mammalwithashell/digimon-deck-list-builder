...
//!
//! # Known gaps that block behavioral coverage
//!
//! - **G-DSL-DISTINCT-TAMER-COLORS-FORMULA** (qa/dsl-vocab-gaps.md, AD1-014 +
//!   BT21-102 entries): no DSL formula primitive that counts distinct colors
//!   across own Tamers in battle area, plus no companion `repeat_n` /
//!   `per_n` step driver to install N selections in sequence. Together these
//!   make the [On Play][When Digivolving] tamer-color immunity clause
//!   unimplementable in pure YAML today. Per the no-approximations policy +
//!   skill rule §11, the clause is OMITTED from the YAML rather than
//!   implemented as a fixed-1 fallback that would silently mis-represent
...
/// Negative gap-tracker: the [On Play][When Digivolving] tamer-color immunity
/// clause IS the real intent of the printed text but cannot be authored
/// faithfully today — see header notes for G-DSL-DISTINCT-TAMER-COLORS-FORMULA
/// + companion repeat_n step driver. When both gaps close, restore this test
/// to assert the OP+WD immunity clause is present.
#[test]
#[ignore = "pending: G-DSL-DISTINCT-TAMER-COLORS-FORMULA — distinct-color count over own Tamers + companion `per_n` / `repeat_n` step driver missing; immunity clause OMITTED from YAML to avoid silently approximating tamer-color synergy"]
...
/// With no opponent permanents, the [WD] delete must install no selection
/// (the outer `condition: any_permanent { of: opponent kind: digimon }` gate).
#[test]
...
/// opponent's lowest DP Digimon" — selection plumbing is asserted; the
/// lowest-DP filter is over-permissive until G-PRED-DP-LTE closes
/// (covered by the ignored test below).
#[test]
...
#[test]
#[ignore = "pending: G-DSL-DISTINCT-TAMER-COLORS-FORMULA + companion `per_n` step driver — immunity clause OMITTED from YAML"]
...
/// G-DSL-DISTINCT-TAMER-COLORS-FORMULA: with 2 Tamer colors, the player gets
/// to pick exactly 1 own Digimon for immunity (2 / 2 = 1).
#[test]
#[ignore = "pending: G-DSL-DISTINCT-TAMER-COLORS-FORMULA + companion `per_n` step driver — immunity clause OMITTED from YAML"]
fn st20_11_on_play_with_two_tamer_colors_grants_one_immunity_selection() {
    ...
    // Pick the ally; it should receive CannotBeAffected (effect immunity)
...
/// G-DSL-DISTINCT-TAMER-COLORS-FORMULA: with 4 Tamer colors, the player gets
/// to pick exactly 2 distinct own Digimon for immunity (4 / 2 = 2). The
/// companion `per_n` step driver is required for sequential picks with
/// `not_in_binding` filtering.
#[test]
#[ignore = "pending: G-DSL-DISTINCT-TAMER-COLORS-FORMULA + companion `per_n` step driver — immunity clause OMITTED from YAML"]
fn st20_11_on_play_with_four_tamer_colors_grants_two_immunity_selections() {
    ...
    let (sp1, aid1) = {
        let v = runner
            .pending_selection_view()
            .expect("first immunity pick (4 colors / 2 = 2)");
        (v.selecting_player, v.valid_action_ids[0])
    };
    runner.execute_action(sp1, aid1).unwrap();

    // Pick 2 — must NOT include the previously-picked permanent.
    let (sp2, aid2) = {
        let v = runner
            .pending_selection_view()
            .expect("second immunity pick");
        (v.selecting_player, v.valid_action_ids[0])
    };
    runner.execute_action(sp2, aid2).unwrap();
    let _ = runner.auto_resolve();

    assert!(
        runner
            .modifiers()
            .has(ally_a, ModifierType::CannotBeAffected)
            && runner
                .modifiers()
                .has(ally_b, ModifierType::CannotBeAffected),
        "both selected own Digimon must receive effect immunity"
    );
}

/// G-DSL-DISTINCT-TAMER-COLORS-FORMULA: [When Digivolving] mirrors [On Play].
/// The same shared formula gates immunity-pick count.
#[test]
#[ignore = "pending: G-DSL-DISTINCT-TAMER-COLORS-FORMULA + companion `per_n` step driver — immunity clause OMITTED from YAML"]
fn st20_11_when_digivolving_offers_immunity_selection_in_addition_to_delete() {
    ...
    // Both the [WD][WA] delete clause AND the [OP][WD] immunity clause should
    // install selections (in some order). At least one selection must be
    // present immediately.
    assert!(
        runner.pending_selection().is_some(),
        "at least one selection (immunity or delete) must install at WhenDigivolving"
    );
}
