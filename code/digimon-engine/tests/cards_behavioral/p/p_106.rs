//! P-106 Agility Training — Option, Cost 2, Green.
//!
//! # Card text (official Bandai DB / data/card_bundles/P-106.md)
//!
//! **[Main]** Reveal the top 2 cards of your deck. Add 1 green card among
//! them to your hand. Return the rest to the bottom of the deck. Then, place
//! this card in the battle area.
//!
//! **[Main] ＜Delay＞** (By trashing this card after the placing turn,
//! activate the effect.)
//! ・1 of your Digimon may digivolve into a green Digimon card in the hand
//! with the digivolution cost reduced by 2.
//!
//! **Inherited:** Security Effect [Security] Place this card in the battle
//! area.
//!
//! # DCGO C# reference
//! DCGO/Assets/Scripts/CardEffect/P/Green/P_106.cs
//!
//! P_106.cs is structurally identical to P_103.cs / P_105.cs (same
//! `SimplifiedRevealDeckTopCardsAndSelect` / `PlaceDelayOptionCards` /
//! `DeletePeremanentAndProcessAccordingToResult` / `SelectPermanentEffect` /
//! `DigivolveIntoHandOrTrashCard` / `PlaceSelfDelayOptionSecurityEffect`
//! chokepoints), with `targetColor = CardColor.Green`. Critically, the
//! [Main] reveal-select call (`SimplifiedRevealDeckTopCardsAndSelect`) does
//! NOT pass `canNoSelect` (defaults to `false`) — so when >=1 green card is
//! present among the top 2, adding it to hand is MANDATORY, not "you may".
//! This matches P-105 (`optional: false` on `select_reveal`), not P-103
//! (whose `optional: true` is a discrepancy from its own DCGO source — see
//! P_103.cs line 36-50, no `canNoSelect` passed there either). The Delay
//! body's `SelectPermanentEffect` DOES pass `canNoSelect: true` (line ~120),
//! so `select_own_permanent` stays optional.
//!
//! # Clause map
//!
//! - Clause 0 ([Main], `main_from_hand`, mandatory): reveal top 2 →
//!   `select_reveal` (NOT optional — mandatory-if-candidate-exists, green
//!   filter) → `add_to_hand_from_reveal` → `place_remainder_on_deck(bottom)`
...
//!   (optional Digimon) → `select_hand` (green Digimon, NOT optional per DCGO
//!   `CanSelectCardCondition` gating `CanSelectPermanentCondition` — a
//!   qualifying green hand card is a precondition for the permanent even
//!   appearing selectable) → `effect_initiated_digivolve` with
//!   `cost: { reduce: 2 }`.
...
//! # Gap history
//!
//! No gaps. Reuses the already-resolved `place_self_as_delay_option` verb
//! (G-PLACE-SELF-AS-OPTION-PERMANENT — resolved 2026-05-02, see P-103) and
//! the standard `select_reveal` / `select_own_permanent` / `select_hand` /
//! `effect_initiated_digivolve` verbs exercised by P-103/P-105.
//!
//! # Patterns this test covers
//! - A2  Two-pass reveal (reveal 2, select-1 green (mandatory-if-candidate),
//!       place rest at bottom)
//! - E2  Empty-candidate skip in reveal pipeline (`select_reveal` with no
//!       green card present in the top 2 — `select_reveal` installs nothing
//!       regardless of `optional`, since `valid_action_ids` is empty)
//! - E1  Delay body: `select_own_permanent` (optional) → `select_hand`
//!       (green) → `effect_initiated_digivolve` (cost reduce 2)
...
const YAML: &str = include_str!("../../../cards/p/P-106.yaml");

// ─── helpers ──────────────────────────────────────────────────────────────────

/// A green Digimon Lv.4 card to serve as the digivolve target in Delay body tests.
...
/// A green Lv.3 Digimon carrier — the on-field Digimon the Delay body digivolves.
fn make_green_carrier(id: &str) -> CardData {
    let mut c = make_test_card(id, id);
    c.card_kind = CardKind::Digimon;
    c.level = Some(3);
    c.dp = Some(3000);
...
/// A green Lv.4 Digimon with a printed `Lv.3 Green, cost 2` evo cost. With the
...
/// (P-106's Delay uses `ignore_requirements: false`).
fn make_green_evo_lv4(id: &str) -> CardData {
    let mut c = make_test_card(id, id);
    c.card_kind = CardKind::Digimon;
    c.level = Some(4);
    c.dp = Some(5000);
...
/// A green Option card — should appear as a valid reveal-select result.
fn make_green_option(id: &str) -> CardData {
    let mut c = make_test_card(id, id);
...
/// A non-green card — should NOT be selectable in the reveal step.
...
#[test]
fn p_106_yaml_parses_and_compiles() {
    let _runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("P-106 YAML must parse and compile without errors");
}

/// P-106 is an Option card with cost 2.
#[test]
fn p_106_is_option_cost_2() {
    ...
    let compiled = runner
        .compiled_card("P-106")
        .expect("P-106 compiled card present");
...
#[test]
fn p_106_clause0_is_main_from_hand_not_optional_face_up() {
    let runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .memory(0)
...
/// Clause 0's process ends with `place_self_as_delay_option` — the printed
/// "Then, place this card in the battle area" tail.
#[test]
fn p_106_clause0_process_places_self_as_delay_option() {
    let runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .memory(0)
...
#[test]
fn p_106_clause1_is_delay_declarative() {
    let runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .memory(0)
...
#[test]
fn p_106_clause2_is_on_security_inherited_scope() {
    let runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .memory(0)
...
#[test]
fn p_106_clause2_process_places_self_as_delay_option() {
    let runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .memory(0)
...
/// P-106 has exactly 2 triggered clauses (main_from_hand + on_security).
#[test]
fn p_106_has_exactly_two_triggered_clauses() {
    let runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .memory(0)
...
/// When P-106's [Main] effect fires from hand, the reveal pipeline runs.
///
...
#[test]
fn p_106_main_from_hand_fires_reveal_then_select() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_green_digimon("GRN1"))
        .add_card(make_filler("FILL"))
        .hand(0, &["P-106"])
        .hand(1, &["FILL"])
        // The deck builder pushes in array order; reveal_top_deck pops from the
        // END of the Vec — so the array's last entry is the deck top. GRN1 last
        // → it shows up in the reveal.
        .deck(0, &["FILL", "FILL", "FILL", "FILL", "GRN1"])
        .deck(1, &["FILL"])
        .memory(10)
        .start();
    ...
    assert!(
        fired,
        "activate_hand_main must return true for P-106 at hand index 0"
    );
    // After reveal, a selection should be pending (select_reveal — mandatory
    // when a green candidate is present) or the effect completed synchronously
    // if no candidate exists. Either way the effect must not panic.
}

/// When the deck has a green card in the top 2, the reveal pipeline is
/// MANDATORY to select it (DCGO: `SimplifiedRevealDeckTopCardsAndSelect` with
/// no `canNoSelect` → defaults to `false`) and the hand grows accordingly. The
/// remaining card is placed at the deck bottom.
///
/// Positive condition test: green card present → select_reveal has a
/// candidate and PASS is NOT a legal option (mandatory pick).
#[test]
fn p_106_main_reveals_green_card_adds_to_hand_mandatory() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_green_digimon("GRN1"))
        ...
        // last two entries are the top 2: GRN1 (top), BLUE1 (second).
        .deck(0, &["FILL", "FILL", "FILL", "BLUE1", "GRN1"])
        .deck(1, &["FILL"])
        .memory(10)
        .start();
    ...
    // The select_reveal prompt must be mandatory (no PASS) when a green
    // candidate exists among the top 2.
    if let Some(sel) = &runner.game.pending_selection {
        assert!(
            !sel.is_optional,
            "select_reveal must be mandatory when a green card is present in \
             the top 2 (DCGO passes no canNoSelect → defaults to false)"
        );
    }
    ...
    // Hand should have grown by 1 (added the selected green card).
    // P-106 itself leaves the hand via place_self_as_delay_option, so the net
    // change is: +1 (GRN1) -1 (P-106) = 0. GRN1 must be present in hand.
    assert!(
        runner.game.players[0]
            .hand
            .iter()
            .any(|cs| cs.card_id(&runner.game.card_data) == "GRN1"),
        "GRN1 (the selected green card) must be in hand after the reveal-select"
    );
    // Deck must have shrunk net by at least 1 (GRN1 moved to hand; BLUE1
    // returned to bottom; P-106 went to battle area).
    assert!(
        deck_after < deck_before,
        "Deck must shrink (the selected green card left the deck); \
         deck_before={deck_before}, deck_after={deck_after}"
    );
}

/// When the deck has NO green cards in the top 2, the select_reveal installs no
...
///
/// Negative condition test: no green card in top 2. `select_reveal` returns
/// false when `valid_action_ids` is empty → the runner advances to the sibling
/// steps, `add_to_hand_from_reveal` no-ops on the absent `picked` binding, and
/// `place_remainder_on_deck` still returns the 2 revealed blues to the bottom.
#[test]
fn p_106_main_no_green_card_in_top2_no_add_to_hand() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_blue_card("BLUE1"))
        ...
        // two entries are the top 2: BLUE1 (top), BLUE2 (second). No green card.
    ...
    // Neither blue card may be added to hand — only green cards qualify.
    assert!(
        !runner.game.players[0].hand.iter().any(|cs| {
            let id = cs.card_id(&runner.game.card_data);
            id == "BLUE1" || id == "BLUE2"
        }),
        "No blue card may be added to hand when no green card appears in the top 2"
    );
    // Both revealed blues must be returned to the deck — the deck loses only
    // P-106-unrelated cards. The 2 revealed cards re-enter the deck at the
...
/// not a panic.
#[test]
fn p_106_main_empty_deck_no_panic() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_filler("FILL"))
...
/// "Then, place this card in the battle area" — the [Main] clause seats P-106
...
#[test]
fn p_106_main_places_self_in_battle_area() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_green_digimon("GRN1"))
        ...
        .hand(0, &["P-106"])
        .hand(1, &["FILL"])
        .deck(0, &["GRN1", "FILL", "FILL", "FILL", "FILL"])
        .deck(1, &["FILL"])
        .memory(10)
        .start();
    ...
    assert!(
        runner.game.activate_hand_main(0, 0),
        "activate_hand_main must fire P-106 [Main]"
    );
    drain_selections(&mut runner);

    // P-106 must no longer be in hand — it was placed in the battle area.
    assert!(
        !runner.game.players[0]
            .hand
            .iter()
            .any(|cs| cs.card_id(&runner.game.card_data) == "P-106"),
        "P-106 must leave the hand when placed in the battle area"
    );

    let placed = runner.game.players[0]
        .battle_area
        .iter()
        .find(|p| p.top_card().card_id(&runner.game.card_data) == "P-106")
        .expect("P-106 must become a battle-area permanent after the [Main] clause");
    assert!(
        matches!(placed.option_state, OptionState::Delayed { owner: 0, .. }),
        "P-106 must be seated as an OptionState::Delayed permanent; got {:?}",
        placed.option_state
    );
}
...
#[test]
fn p_106_main_placement_dispatches_on_option_placed() {
    let witness = Arc::new(Mutex::new(0u32));

    let mut observer = make_filler("P106-OBSERVER");
    ...
    runner.place_on_field(1, "P106-OBSERVER", Some(0));
...
#[test]
fn p_106_delay_clause_is_declarative_delay() {
    let runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .memory(0)
...
/// When P-106 is placed in the battle area (as a Delay permanent) and the Delay
/// body fires, the player can select a Digimon to digivolve into a green
/// Digimon in hand with cost -2.
///
/// Positive condition: at least 1 Digimon on field + green Digimon in hand →
/// selection installs.
#[test]
fn p_106_delay_body_installs_digimon_selection_when_conditions_met() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_green_evo_lv4("GRN_EVO"))
        .add_card(make_green_carrier("CARRIER"))
        .add_card(make_filler("FILL"))
        .hand(0, &["GRN_EVO", "FILL"])
        .hand(1, &["FILL"])
    ...
    // Place P-106 as a Delay permanent in the battle area (simulating it was
    // placed there by the Main clause). place_on_field bypasses play costs.
    let delay_perm = runner.place_on_field(0, "P-106", Some(0));
...
#[test]
fn p_106_delay_body_optional_target_when_no_digimon_on_field() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_green_evo_lv4("GRN_EVO"))
    ...
    // Place P-106 in battle area but NO other Digimon on field.
    let delay_perm = runner.place_on_field(0, "P-106", Some(0));
    ...
    // GRN_EVO must still be in hand — there was no Digimon to digivolve.
    assert!(
        runner.game.players[0]
            .hand
            .iter()
            .any(|cs| cs.card_id(&runner.game.card_data) == "GRN_EVO"),
        "GRN_EVO must remain in hand when there is no Digimon to digivolve"
    );
}

/// Delay body: the player selects a Lv.3 green carrier Digimon and a Lv.4
/// green Digimon in hand; `effect_initiated_digivolve` runs with cost
/// reduction 2.
///
/// The carrier is `make_green_carrier` (Lv.3 Green); the hand card is
/// `make_green_evo_lv4` (Lv.4 Green, printed `Lv.3 Green, cost 2`). With
/// `cost: { reduce: 2 }` the effective cost is 0, so the digivolve succeeds:
/// GRN_EVO becomes the top card of the carrier's stack.
///
/// DCGO: payCost: true, reduceCostTuple: (2, null) → cost: { reduce: 2 }.
#[test]
fn p_106_delay_body_digivolves_with_cost_reduction_2() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_green_evo_lv4("GRN_EVO"))
    ...
    // After the chained selections + digivolve, GRN_EVO must be the top card
    // of the (former) CARRIER stack, and must have left the hand.
    let carrier = &runner.game.players[0].battle_area[carrier_perm.index as usize];
    assert_eq!(
        carrier.top_card().card_id(&runner.game.card_data),
        "GRN_EVO",
        "GRN_EVO must be the top card of the carrier stack after the Delay digivolve"
    );
    assert!(
        !runner.game.players[0]
            .hand
            .iter()
            .any(|cs| cs.card_id(&runner.game.card_data) == "GRN_EVO"),
        "GRN_EVO must leave the hand once it digivolves onto the carrier"
    );
}

/// Playing P-106 from hand parks it as a `MainPhaseActivated` delayed Option.
...
/// `<Delay>` body (1 of your Digimon may digivolve into a green Digimon in
/// hand for cost −2).
///
...
#[test]
fn p_106_delay_activation_digivolves_via_main_phase_action() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_green_evo_lv4("GRN_EVO"))
        .add_card(make_green_carrier("CARRIER"))
        .add_card(make_filler("FILL"))
        // Reveal top 2: 2 filler (no green card to add — keeps the reveal
        // simple and deterministic in this integration test).
        .hand(0, &["P-106", "GRN_EVO"])
        .hand(1, &["FILL"])
    ...
    // The on-field green Lv.3 Digimon the Delay body will digivolve. As the
    // sole eligible target it is also a green card that satisfies P-106's
    // green-Option play-color requirement.
    let carrier_perm = runner.place_on_field(0, "CARRIER", Some(0));
    runner.game.enter_main_phase();

    // Play P-106 as an Option from hand. Reveal + delay placement installs a
    // pending selection → OptionPlayResult::Pending.
    assert_eq!(
        runner.game.play_option_from_hand(0, 0),
        OptionPlayResult::Pending,
        "P-106 install + reveal selection must park as Pending"
    );
    runner
        .auto_resolve()
        .expect("resolve P-106 reveal selection and delay placement");

    // P-106 parks as a MainPhaseActivated delayed Option permanent.
    let delay_idx = runner
        ...
        .expect("playing P-106 must park it as a MainPhaseActivated delayed option");
    ...
    assert_eq!(
        ...
        0.0,
        "P-106 <Delay> must not be activatable on the placing turn"
    );
    ...
    assert_eq!(
        mask[bit], 1.0,
        "P-106 <Delay> activation must be a legal action after the placing turn"
    );
    assert_eq!(
    ...
    // Take the activation: trash P-106 as cost, then run the Delay body.
    runner.game.decode_action(bit as u16, 0);
    // The Delay body installs select_own_permanent → select_hand chained
    // selections (digivolve a Digimon into a green Digimon in hand).
    runner
        .auto_resolve()
        .expect("resolve Delay body selections");

    // P-106 must be trashed as the <Delay> activation cost — no delayed Option
    // permanent remains.
    assert!(
        !runner
            .game
        ...
        "P-106 must be trashed as the <Delay> activation cost"
    );

    // The Delay body ran: GRN_EVO digivolved onto the CARRIER stack (cost −2
    // makes the printed Lv.3 Green cost-2 evo free) and left the hand.
    let carrier = &runner.game.players[0].battle_area[carrier_perm.index as usize];
    assert_eq!(
        carrier.top_card().card_id(&runner.game.card_data),
        "GRN_EVO",
...
#[test]
fn p_106_security_clause_is_inherited_scope() {
    let runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .memory(0)
    ...
    assert!(
        ...
        "P-106 must have an on_security clause"
    );
    assert_eq!(
...
#[test]
fn p_106_security_clause_fires_without_panic() {
    let mut runner = DebugRunner::builder()
        .from_dsl_yaml(YAML)
        .expect("parses")
        .add_card(make_filler("FILL"))
    ...
    // Place P-106 in the battle area (as if it was placed by the Main clause).
    let field_handle = runner.place_on_field(0, "P-106", Some(0));
...
/// combat / security-check path: P-106 sits in the defender's security stack,
/// an attacker checks it, and the inherited [Security] clause places P-106 in
...
#[test]
fn p_106_security_places_self_in_battle_area() {
    let mut attacker = make_filler("P106-ATK");
    ...
    let attacker_handle = runner.place_on_field(0, "P106-ATK", Some(0));
    assert_eq!(
        runner.security_count(1),
        1,
        "precondition: P-106 in the defender's security stack"
    );

    let _ = runner.attack_player(attacker_handle, 1, false);
    runner.auto_resolve().expect("security selections resolve");

    // P-106's inherited [Security] clause must have run: P-106 leaves the
    // security stack and is seated as a Delay-Option permanent on the
    // defender's field.
    assert_eq!(
        runner.security_count(1),
        0,
        "P-106 must leave the defender's security stack after the security check"
    );
    ...
    let placed = runner.game.players[1]
        ...
        .iter()
        .find(|p| p.top_card().card_id(&runner.game.card_data) == "P-106")
        .expect("P-106 must be placed in the defender's battle area after [Security]");
    assert!(
        matches!(placed.option_state, OptionState::Delayed { owner: 1, .. }),
        "P-106 must be seated as an OptionState::Delayed permanent owned by the \
         defender; got {:?}",
        placed.option_state
    );

    // P-106 must not have fallen through to the security-dispose trash.
    assert!(
        !runner.game.players[1]
            .trash
            .iter()
            .any(|cs| cs.card_id(&runner.game.card_data) == "P-106"),
        "P-106 must not fall through to the trash — the [Security] clause placed it"
    );
}
