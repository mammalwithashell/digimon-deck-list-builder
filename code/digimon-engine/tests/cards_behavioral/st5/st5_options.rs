...
fn stack_sources(runner: &DebugRunner, player: usize, field_index: usize) -> usize {
    runner.game.players[player].battle_area[field_index].card_sources.len()
}
...
fn st5_15_main_de_digivolves_up_to_two_opponent_digimon() {
    ...
    runner.execute_action(0, first).expect("select first Laser Eye target");
    assert_eq!(
        runner.pending_kind(),
    ...
    runner.execute_action(0, second).expect("select second Laser Eye target");
...
fn st5_15_security_activates_laser_eye_main_effect() {
    ...
    runner
        .game
        .enqueue_triggered(EffectTiming::SecuritySkill, TriggerSource::Permanent(option));
    ...
    runner.execute_action(0, pick).expect("select security Laser Eye target");
...
fn st5_16_main_deletes_one_opponent_digimon_with_play_cost_seven_or_less() {
    ...
    assert_eq!(
        runner.pending_selection_view().unwrap().valid_action_ids.len(),
        1,
        "play_cost_lte: 7 excludes the cost-8 Digimon"
    );
    let pick = runner.pending_selection_view().unwrap().valid_action_ids[0];
    runner.execute_action(0, pick).expect("delete cost-7 Digimon");
...
fn st5_16_security_activates_dark_side_attack_main_effect() {
    ...
    assert_eq!(runner.pending_kind(), Some(SelectionKind::OppField));
    let pick = runner.pending_selection_view().unwrap().valid_action_ids[0];
    runner.execute_action(0, pick).expect("delete security target");
