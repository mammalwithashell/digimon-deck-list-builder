//! BT25-046 Gekkomon — Digimon, Lv.3, Green, DP 2000, Cost 3.
...
//!   [Glowing Dawn] trait and 1 green card with the [BEATBREAK] trait among
//!   them to the hand. Return the rest to the bottom of the deck.
//!
//! # Printed inherited text
//!   <Piercing> (When this Digimon attacks and deletes an opponent's Digimon in
//!   battle, it checks security before the attack ends.)
//!
//! # Alt-digivolve (cards.json xros_req)
//!   [Digivolve] Lv.2 w/[Glowing Dawn] trait: Cost 0
//!
//! # DCGO C# reference
//!   DCGO/Assets/Scripts/CardEffect/BT25/Green/BT25_046.cs
...
//!     HasCardColor(Green), maxCount 1 each, mode AddHand,
//!     remainingCardsPlace DeckBottom, mutualConditions: true).
//!   - OnDetermineDoSecurityCheck: PierceSelfEffect(isInheritedEffect: true).
//!
...
//!   - Group G: inherited static keyword grant (<Piercing>)
...
const CARD_ID: &str = "BT25-046";

fn runner() -> DebugRunner {
    DebugRunner::builder()
        .dsl_card(CARD_ID)
        .expect("BT25-046 YAML parses and compiles")
        .build()
}
...
#[test]
fn bt25_046_is_green_glowing_dawn_digimon() {
    let runner = runner();
    let card = runner.compiled_card(CARD_ID).expect("compiled");
    assert_eq!(card.name, "Gekkomon");
...
#[test]
fn bt25_046_has_on_play_reveal_two_pick_clause() {
    let runner = runner();
    let card = runner.compiled_card(CARD_ID).expect("compiled");
    ...
    let on_play = card
        ...
        .expect("BT25-046 must have an [On Play] clause");
...
#[test]
fn bt25_046_has_inherited_piercing() {
    let runner = runner();
    let card = runner.compiled_card(CARD_ID).expect("compiled");
    let piercing = card.effects.iter().any(|c| match c {
    ...
    assert!(piercing, "BT25-046 must grant inherited <Piercing>");
}

#[test]
fn bt25_046_registers_glowing_dawn_alt_digivolve() {
    let runner = runner();
    let card = runner.compiled_card(CARD_ID).expect("compiled");
    ...
    assert_eq!(
        digivolve_paths, 1,
        "BT25-046 must register the Glowing-Dawn-trait Lv.2 alt digivolve path"
    );
}
...
#[test]
fn bt25_046_on_play_adds_glowing_dawn_and_green_beatbreak_rest_to_bottom() {
    let mut gd = make_test_card("GD-046A", "GD 046A");
    gd.traits.push("Glowing Dawn".to_string());
    let mut bb = make_test_card("GBB-046", "Green BB 046");
    bb.traits.push("BEATBREAK".to_string());
    bb.colors = vec![CardColor::Green];
    let filler = make_test_card("FILL-046", "Filler 046");
    let holder = make_test_card("BT25_046_HOLDER", "Holder");

    let mut runner = DebugRunner::builder()
        .dsl_card(CARD_ID)
        .expect("BT25-046 compiles")
        .add_card(gd)
        .add_card(bb)
        .add_card(filler)
        .add_card(holder)
    ...
    let holder_perm = runner.place_on_field(0, "BT25_046_HOLDER", None);
    let src = runner.top_card(holder_perm);

    let deck_before = runner.game.players[0].deck.len();
    // Top-3 (last pushed = top): FILL-046, GBB-046, GD-046A.
    stack_deck_top(&mut runner, &["FILL-046", "GBB-046", "GD-046A"]);
    ...
    let hand_ids: Vec<&str> = runner.game.players[0]
        ...
        .collect();
    assert!(hand_ids.contains(&"GD-046A"), "Glowing Dawn pick to hand: {hand_ids:?}");
    assert!(
        hand_ids.contains(&"GBB-046"),
        "green BEATBREAK pick to hand: {hand_ids:?}"
    );
    assert!(
        !hand_ids.contains(&"FILL-046"),
...
/// Negative (color filter): a YELLOW [BEATBREAK] card does NOT satisfy the
/// "green [BEATBREAK]" bucket — only the Glowing Dawn card is added.
#[test]
fn bt25_046_beatbreak_bucket_rejects_non_green_color() {
    let mut gd = make_test_card("GD-046B", "GD 046B");
    gd.traits.push("Glowing Dawn".to_string());
    let mut yellow_bb = make_test_card("YBB-046", "Yellow BB 046");
    yellow_bb.traits.push("BEATBREAK".to_string());
    yellow_bb.colors = vec![CardColor::Yellow];
    let filler = make_test_card("FILL-046B", "Filler 046B");
    let holder = make_test_card("BT25_046_HOLDER2", "Holder2");
    ...
    let holder_perm = runner.place_on_field(0, "BT25_046_HOLDER2", None);
    let src = runner.top_card(holder_perm);
    stack_deck_top(&mut runner, &["FILL-046B", "YBB-046", "GD-046B"]);
    ...
    let hand_ids: Vec<&str> = runner.game.players[0]
        ...
        .map(|c| c.card_id(&runner.game.card_data))
        .collect();
    assert!(hand_ids.contains(&"GD-046B"), "Glowing Dawn pick to hand: {hand_ids:?}");
    assert!(
        !hand_ids.contains(&"YBB-046"),
        "a YELLOW [BEATBREAK] must NOT satisfy the green BEATBREAK bucket: {hand_ids:?}"
    );
}
...
#[test]
fn bt25_046_on_play_fizzles_when_no_candidates() {
    let fa = make_test_card("F046-A", "f a");
    let fb = make_test_card("F046-B", "f b");
    let fc = make_test_card("F046-C", "f c");
    let holder = make_test_card("BT25_046_HOLDER3", "Holder3");
    ...
    let holder_perm = runner.place_on_field(0, "BT25_046_HOLDER3", None);
    let src = runner.top_card(holder_perm);

    let hand_before = runner.game.players[0].hand.len();
    let deck_before = runner.game.players[0].deck.len();
    stack_deck_top(&mut runner, &["F046-C", "F046-B", "F046-A"]);
