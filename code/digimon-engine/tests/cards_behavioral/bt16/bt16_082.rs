...
//! - G-ON-MOVE (hybrid gap): entire clause blocked; OnMove timing missing from
//!   both the Rust engine (no EffectTiming::OnMove, no dispatch hook in
//!   game_actions::move_from_breeding) AND the DSL (no on_move_from_breeding
//!   when-token in compiled.rs CompiledTiming). See:
//!   - qa/archetype-qa/engine-gaps.md [G-ON-MOVE]
//!   - qa/dsl-vocab-gaps.md [EX11-008 entry, same gap]
...
//!
//! # Status: BLOCKED (hybrid)
//!
//! The YAML uses a `kind: raw_rust` no-op escape hatch (bt16_082_on_move_noop)
//! as a placeholder for the clause body. All behavioral tests are #[ignore]'d
//! pending G-ON-MOVE resolution. Structural tests (compiled card shape) pass.
...
// ---------------------------------------------------------------------------
// Section 2 — Behavioral tests (all ignored: G-ON-MOVE)
// ---------------------------------------------------------------------------
//
// The entire behavioral path requires:
//   1. EffectTiming::OnMove added to code/digimon-engine/src/enums.rs
//   2. game_actions::move_from_breeding fires enqueue_triggered(OnMove, ...)
//      for every permanent in the player's battle area after the move.
//   3. DSL timing_map adds on_move_from_brewing token → EffectTiming::OnMove.
//   4. CompiledTiming::OnMoveFromBreeding added to digimon-dsl/src/compiled.rs.
//
// Until G-ON-MOVE is resolved (both engine and DSL halves), all behavioral
...
#[test]
#[ignore = "pending: G-ON-MOVE from qa/archetype-qa/engine-gaps.md — OnMove dispatch not wired in engine or DSL"]
...
#[test]
#[ignore = "pending: G-ON-MOVE from qa/archetype-qa/engine-gaps.md"]
fn bt16_082_select_reveal_filters_to_digimon_or_tamer() {
    ...
    unimplemented!("pending G-ON-MOVE resolution");
}
...
#[test]
#[ignore = "pending: G-ON-MOVE from qa/archetype-qa/engine-gaps.md and G-OPT-TRIGGERED"]
fn bt16_082_opt_blocks_second_trigger_same_turn() {
    unimplemented!("pending G-ON-MOVE and G-OPT-TRIGGERED resolution");
}
