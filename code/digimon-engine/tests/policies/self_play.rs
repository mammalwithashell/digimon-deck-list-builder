//! Headless self-play tripwire: run `HeadlessRunner` end-to-end across
//! many seeds with `greedy_action` on both seats and assert each game
//! declares a winner within a bounded step budget. Cheap regression check
//! that catches action-mask drift, policy off-by-ones, or engine hangs
...
fn greedy_self_play_terminates_across_seeds() {
    ...
    for seed in 0..NUM_GAMES {
        ...
        let winner = runner.run_until_conclusion(
            MAX_STEPS_PER_GAME,
            Some(|g: &digimon_engine::Game, m: &[f32]| greedy_action(g, m)),
        );
