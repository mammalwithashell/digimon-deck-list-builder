...
impl Game {
    ...
    fn play_option_core(
        ...
        // The `OptionMain` body runs for every mode EXCEPT Link: plugging
        // a Plug-In in via Link Requirements resolves straight to an
        // attach, with no `[Main]` / `[Security]` effect. (For a dual-mode
        // card this is what keeps the Standard `[Main]` body from firing on
        // a Link play.) `enqueue_option_main_from_pending` itself skips
        // link-metadata effects, so a Standard play never enqueues a
        // spurious no-op `OptionMain` entry either.
        if !mode.is_link() {
    ...
    fn enqueue_option_main_from_pending(
        ...
        for (slot, effect) in effects.iter().enumerate() {
            ...
            // A `link_requirement` clause lowers to an `OptionMain`-timing
            // effect carrying only link metadata (cost + host filter) — it
            // has no `[Main]` body to run. Skipping it keeps a dual-mode
            // Plug-In Option's Standard play from enqueuing a no-op
            // `OptionMain` entry (which would force a spurious
            // `TriggerOrder` prompt alongside the real `[Main]` body).
            if effect.link_cost.is_some() {
                continue;
            }
            self.effect_queue.push_back(QueuedEffect {
                source_card: card_handle,
