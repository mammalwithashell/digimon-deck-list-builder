...
**Open 2026-06-07 (BT25 link-subsystem gaps — scoped + planned):** Four net-new link substrate subsystems surfaced by the BT25 gap→card table, each confirmed by source investigation and scoped in `.claude/plans/rust-engine-gaps-bt25-link.md` (per-gap design, DCGO refs, file surfaces, TDD targets). (1 of 5 — the `ChangeLinkMax` aura — is RESOLVED below.) Remaining: **Gap 5** predicated WhenWouldLink cost-reduce (BT25-004/045 — needs the link host exposed in the `WhenWouldLink` replacement window, which `collect_candidates` reaches but does not currently carry, + a cost-reduce path); **Gap 2** link-N-cards DSL step over the shipped `link_chosen_card_into_host` primitive (BT25-075/060/089); **Gap 3** link-card-trash `WhenWouldLeaveBattleArea` replacement cost + Option self-as-link-source persistence (BT25-066/073/101); **Gap 4** App Fuse mechanic (BT25-036/060/089 — recommend `/assess-archetype-rust` first). Ordering rec: 2 → 5 → 3a → 3b → 4.
