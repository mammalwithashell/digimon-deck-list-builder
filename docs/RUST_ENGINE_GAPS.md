...
- **Severity:** 🔴 BLOCKING
- **Discovered in:** Dark Masters (2026-04-18)
- **Card(s):** EX10-010 BlackWarGreymon, BT16-026 Vikemon, EX8-026 MetalSeadramon, LM-043 Darkdramon, EX10-074 Beelzemon, BT16-046 GranKuwagamon, BT21-051 Puppetmon, BT19-064 Justimon: Blitz Arm
- **Effect text:** "[Hand] [Counter] <Blast Digivolve> (Your Digimon may digivolve into this card without paying the cost.)"
- **What's missing:** RUST_ENGINE_API.md §9 lists "Block / Counter / Alliance interrupt phases" as not-yet-wired; combat is atomic. The `[Hand][Counter]` activation timing is a defender-side reactive window opened during the opponent's attack. `<Blast Digivolve>` is also unimplemented as a flow: it digivolves a friendly battle-area permanent into the Counter-window card from hand without paying digivolve cost AND (per parity §4.6d) without honoring digivolution requirements. No `EffectTiming::CounterWindow` fire site, no `ctx.prompt_blast_digivolve(...)` primitive. Distinct from the existing "Zone-manipulation: effect-initiated digivolve" gap, which is framed around active-player initiated digivolves on resolved permanents.
- **Suggested API shape:** Add `EffectTiming::CounterWindow` fired from `combat::begin_attack` to the defender. Builder: `Effect::counter(card)` constructor. `ctx.prompt_blast_digivolve(this_card_handle, base_filter, ignore_reqs: bool)` installs an own-permanent selection on the defender side; on accept, removes the card from hand and pushes onto the chosen base's `card_sources`, fires `WhenDigivolving` with cost = 0, then yields back to the attack state machine. Native `<Blast Digivolve>` printed-keyword parsing emits the constructor at registry-build time. Action-mask emission: `BLAST_DIGIVOLVE_FROM_HAND_<i>` slots gated to hand cards with `Keyword::BlastDigivolve` and to base permanents meeting digivolution requirement.
...
### `OnAllyAttack` / `OnOpponentAttack` observer timings not fired
- **Severity:** 🔴 BLOCKING
- **Discovered in:** Dark Masters (2026-04-18)
- **Card(s):** BT15-008 Muchomon
- **Effect text:** "[Your Turn] [Once Per Turn] When one of your red Digimon attacks a player, <Draw 1>."
- **What's missing:** `EffectTiming::OnAllyAttack` and `EffectTiming::OnOpponentAttack` are declared in `enums.rs` but no dispatch site exists in `combat.rs` or `game_actions.rs`. Neither `begin_attack` nor `attack_player` / `attack_digimon` call `enqueue_triggered(OnAllyAttack, ...)`. A fan-out to all battle-area permanents observing an ally's attack declaration is required. The context passed must carry (a) the attacker `PermanentHandle`, (b) the attack target variant (`AttackTarget::Player` vs `AttackTarget::Digimon`) so scripts can gate on "attacks a player" vs "attacks a Digimon". Distinct from `OnAttack` (which fires on the attacking permanent's own effects).
...
- **Workaround:** "None — BLOCKED." Reading `ctx.game.pending_attack` directly violates the curated-API contract.
