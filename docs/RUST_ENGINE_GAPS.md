...
| ~~[`play_from_security_at(index)` / bound selected-security play](#play_from_security_atindex-bt13-012-geogreymon-bt14-033-patamon)~~ — RESOLVED 2026-05-23 | ✅ | — | — |
...
### `play_from_security_at(index)` / bound selected-security play (BT13-012 GeoGreymon, BT14-033 Patamon) — RESOLVED 2026-05-23
- **Status:** Closed by `unblock-mastemon-rust-dsl`. `EffectContext::play_from_security_card(player, handle)` resolves a bound security-card selection through the same indexed removal / free-play pipeline as `play_from_security_index`, preserving remaining security order, firing security-loss observers, and recording play success for card-local tails. DSL `play_security_card: { of, card }` now covers selected-security play; selected-security digivolve was already available through `effect_initiated_digivolve` from a bound security card and is proven by `BT14-033`.
- **Coverage:** `cargo test --manifest-path code/digimon-engine/Cargo.toml --test dsl -- security_stack_steps`; `cargo test --manifest-path code/digimon-engine/Cargo.toml --test cards_behavioral -- bt14_033`; `cargo test --manifest-path code/digimon-engine/Cargo.toml --test cards_behavioral -- ex6_029`.
- **Related:** "Search-own-security-stack primitive" and "Effect-initiated digivolve from security stack" remain closed entries in `qa/resolved-gaps.md`.
...
- **Updated 2026-05-23 (`unblock-mastemon-rust-dsl`):** BT14-033 no longer blocks on the "if digivolved by this effect" card-local tail. The DSL records successful effect-initiated digivolutions in the per-effect result log (`effect_digivolved_any_digimon`), allowing the hand-to-security tail to run only after the selected security digivolve succeeds. The remaining partial scope is still the stricter cross-effect identity / cleanup-token family described above.
