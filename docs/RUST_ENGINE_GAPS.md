...
- **Card(s):** **BT25-089 Kazuki & Itsuki** (Tamer): `[End of Your Turn] [Once Per Turn] 1 of your Digimon may app fuse into a Digimon card in the hand.` This is the Appmon "App Fuse" mechanic shared across the BT25 Appmon package (and prior Appmon sets), so the primitive unblocks more than one card.
...
### `OnDiscardHand` / "when your hand is trashed from" trigger timing + "played by an effect" condition (BT25-080 Witchmon)
...
- **Discovered in:** BT25 "titan" slice (2026-06-06).
- **Card(s):** **BT25-080 Witchmon** — inherited `[All Turns][Once Per Turn] When your hand is trashed from, if this Digimon has the [Titan] trait, delete 1 of your opponent's level 4 or lower Digimon`. DCGO `BT25_080.cs` consumes `EffectTiming.OnDiscardHand` via `CardEffectCommons.CanTriggerOnTrashHand(...)`. The hash string (`BT25_029_AT_ESS`) indicates **BT25-029** shares the same trigger — unblocks >=2 cards.
...
### `cost_reduction` `pay_cost` drops the reduction when the cost is interactive (parks on a selection)  [G-COST-REDUCTION-INTERACTIVE-PAY-COST]
- **Severity:** 🔴 BLOCKER for the BT25 BEATBREAK / Glowing Dawn "trash a face-down card under a Tamer → reduce a [Glowing Dawn] card's cost" mechanic.
- **Discovered in:** BT25 "beatbreak" slice (2026-06-06) implementing BT25-088 Kyo Sawashiro (clause 3), BT25-090 Tomoro Tenma (clause 3), BT25-049 Armalizamon (clause 2).
- **Card(s):** BT25-088 / BT25-090 (`[Your Turn][OPT] When a [Glowing Dawn] card would be played / Option would be used, by trashing the bottom face-down card under any of your Tamers, reduce the cost by 1`), BT25-049 (same, reduce by 3). Generalizes to any `kind: cost_reduction` whose `pay_cost` is an **interactive** (selection-installing) step.
- **What's missing (engine):** A `kind: cost_reduction` clause with `pay_cost: [trash_bottom_face_down_source_under_tamer: { of: you }]` does not credit its `amount` reduction. The DSL lowers `pay_cost` to a closure that returns `matches!(run_steps_with_runtime(...), RunOutcome::Synchronous)` (`dsl_cards/lower_cost_reduction.rs:193`). The `trash_bottom_face_down_source_under_tamer` step ALWAYS installs a `select_own_permanent` (a 1-option selection even for a single eligible Tamer — the no-approximations contract) and returns `RunOutcome::Parked`. So the `pay_cost_fn` returns `false`, `apply_cost_reduction_candidate` (`game_actions.rs:5504`) returns `None` (line 5522-5524), and the reduction `amount` is **discarded** — yet the parked Tamer-selection still resolves and trashes the face-down card. Net: the player pays the cost (loses the face-down stash) but gets NO cost reduction. Confirmed by a behavioral test: GD cost 4, one eligible Tamer with one face-down stash → accept reduction + resolve Tamer pick → face-down trashed (`trash` grows by 1) but memory spent is 4, not 3.
- **Root cause:** the play-from-hand cost-reduction accept-path (`continue_play_from_hand_cost_reduction_chain` accept callback, `game_actions.rs:631`) DOES wrap a continuation when `pending_selection.is_some()` after `apply_cost_reduction_candidate` — but it threads the UN-incremented `reduction` because `apply_cost_reduction_candidate` already returned `None` (the pay_cost parked, so its `Some(amount)` was never produced). The amount needs to be credited when the **parked pay_cost resolves successfully**, not synchronously inside `apply_cost_reduction_candidate`.
- **Suggested API shape:** Make `apply_cost_reduction_candidate` (and the accept-callback that calls it) defer the `amount` credit for an interactive pay_cost: when `pay_cost_fn` installs a selection, capture the candidate's `amount` and add it to `accumulated_reduction` only inside the parked-selection's success continuation (wire it through `wrap_pending_play_cost_continuation`, gating the credit on the pay_cost having actually been paid — e.g. a `paid: bool` the trash step sets on its callback). Synchronous pay_costs (suspend-self, BT5-092) keep their current immediate credit. The OptionUse cost path (`play_option_core` → `scan_before_pay_cost_reduction_with_target`) needs the same fix for BT25-090's "would use Option" variant.
- **Workaround:** None faithful. Auto-resolving the single-eligible-Tamer pick (to keep the pay_cost synchronous) would violate no-approximations. BT25-088 / BT25-090 / BT25-049 ship YAML with their other clauses working and the cost-reduction clause OMITTED (no silent under/over-charge); each is PARTIAL pending this gap.
- **Related:** `G-TRASH-N-BOTTOM-FACE-DOWN-UNDER-TAMER` (`qa/dsl-vocab-gaps.md`, the multi-count sibling — BT25-035 trash-2); `G-COST-REDUCTION-DIGIVOLVE-INTO` (the resolved synchronous-suspend pay_cost precedent, BT5-092). NOTE: `trash_bottom_face_down_source_under_tamer` works correctly as a **process activation cost** (with a captured tail) — only the `cost_reduction` `pay_cost` slot drops the amount.
