...
### `activation_cost` on a `main_on_field` clause is never paid on the `[Main]` action path, and the mask does not gate on it  [G-ENGINE-MAIN-ON-FIELD-ACTIVATION-COST-UNPAID]  — OPEN 2026-09-17
- **Severity:** 🔴 faithfulness bug (existing primitive) — a printed "By suspending this Tamer" / "By returning this Tamer to the bottom of the deck" cost is skipped whenever the ability is activated through the action space.
- **Discovered in:** three-musketeers-1 gap-closure stage for BT25-092 (2026-09-17), while choosing the `[Main]` suspend-cost idiom. Probed empirically: `Game::activate_field_main(0, kazuki)` on BT25-089 → `fired=true, suspended_after_fire=false, pending=true`; suspending the Tamer first → `fired=true` again.
- **Card(s):** every `when: main_on_field` clause whose first step is `activation_cost: {…}` — currently BT25-089 Kazuki & Itsuki (`suspend_self`) and EX11-071 (`return_self_to_deck_bottom`). Their per-card tests pass only because they drive the clause through `enqueue_triggered(MainOnField)` + `drain_effect_queue`, where `effect_queue::run_queued_effect_inner` DOES consult `activation_cost_fn`.
- **Root cause:** `Game::activate_field_main` (`src/game_actions/play.rs`) — the FIELD_EFFECT `+2` sub-slot decoder target — checks `condition` and `outer_optional_guard`, then calls `effect.process` directly; it never invokes `effect.activation_cost_fn` (`src/effect.rs`, documented as "consumed during triggered-ability resolution"). `main_effect_select::field_main_match` (the mask) likewise has no activation-cost / suspension gate, so a suspended Tamer's `[Main]` bit stays legal.
- **Workaround (established idiom, used by BT11-061 / P-224 / ST4-14 / BT25-092):** author the cost as `condition: { source_is_unsuspended: true }` + `suspend: { target: source }` as the first body step instead of `activation_cost`. The two affected YAMLs should be migrated to this idiom, OR the engine fixed as below.
- **Fix sketch:** (1) in `activate_field_main`, after the condition gate and before `process`, run `activation_cost_fn` and return `false` without recording activation when it fails; (2) add a read-only "can pay activation cost" probe (suspend_self → `!is_suspended && !CannotSuspend`; return_self → always) surfaced through `Effect` so `field_main_match` can suppress the bit — or lower `activation_cost` on `MainOnField` clauses into the `condition` + first-step pair at lowering time (`lower_triggered.rs`), which needs no engine change and reuses the proven idiom. Add a regression test that activates through `activate_field_main` (not the queue) and asserts the cost was paid and a suspended carrier is refused.
