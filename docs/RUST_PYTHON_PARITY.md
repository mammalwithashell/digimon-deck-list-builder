...
| `engine.data.deck_loader.{validate_deck, RESTRICTED_LIST, CardRestriction}` | `db/routers/decks.py` (legacy EDH / Titan paths only) | ✅ Resolved by `add-deck-format-registry`: banlists + EDEN anomaly protocol + format descriptors now live in `data/deck_formats.json` (single source of truth, baked into Rust, read by the hosted API), parsed by the `format` registry. `decks.py` / `deck_tools.py` defer to `validate_deck_for_game_mode` for `standard`/`no_restriction`/`pauper`/`eden`/`eden_singleton` — no parallel Python banlist remains. **Singleton validation now lives in Rust** (generic `min(restriction_limit, singleton ? 1 : default)`), so EDH's Python singleton check could migrate next. Only EDH/Titan (player-count/size rules the binding doesn't expose) keep the Python `validate_deck` path. New engine surface: `list_formats`, `card_legality`. |
