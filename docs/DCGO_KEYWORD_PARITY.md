...
| MaterialSave(count) | Optional deletion/removal-timed rescue that moves up-to-N recipe-eligible stack sources under an own Tamer | ✅ | Corrected by the Xros Heart DigiXros substrate on 2026-05-24; no longer emits a `[Main]` action. Uses deletion snapshots, own-Tamer destination selection, recipe-filtered source selection from trash/snapshot movement, and count-capped placement under the chosen Tamer. See `keyword_effects.rs`, `tests/material_save_keyword.rs`, and `tests/keyword_phase_d/material_save.rs`. |
