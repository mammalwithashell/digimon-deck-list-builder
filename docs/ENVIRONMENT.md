...
`code/frontend/.env.local` or passed at build time.
...
|-----|----------|---------|---------|---------|
| `SPACES_ENDPOINT` | yes† | — | `code/server/storage/spaces.py` | Spaces origin, e.g. `https://nyc3.digitaloceanspaces.com`. Consumed by boto3 and the manifest URL fallback. |
| `SPACES_BUCKET` | yes | — | `code/server/storage/spaces.py` | Bucket name, e.g. `digimon-tcg-models`. |
| `SPACES_REGION` | yes | — | `code/server/storage/spaces.py` | Region slug, e.g. `nyc3`. Required for signature-v4. |
| `SPACES_KEY` | yes | — | `code/server/storage/spaces.py` | Spaces access key (admin-level IAM **not** required — per-object `ACL="public-read"` suffices). |
| `SPACES_SECRET` | yes | — | `code/server/storage/spaces.py` | Spaces secret key. |
| `SPACES_CDN_URL` | optional | — (falls back to origin) | `code/server/storage/spaces.py:public_url()` | CDN base URL, e.g. `https://digimon-tcg-models.nyc3.cdn.digitaloceanspaces.com`. When set, manifest `url` fields become `{SPACES_CDN_URL}/{key}` (no bucket path). When unset, manifest falls back to path-style `{SPACES_ENDPOINT}/{bucket}/{key}`. Switching between modes requires no desktop client change — SHA256+size verification matches either way. Trailing slash and whitespace-only values are normalized/ignored. |
...
| Var | Required | Default | Used by | Purpose |
|-----|----------|---------|---------|---------|
| `DATABASE_URL` | optional | `sqlite+aiosqlite:///./data/app.db` | `code/server/db/database.py` | SQLAlchemy async URL. Production should point at PostgreSQL, e.g. `postgresql+asyncpg://user:pw@host/db`. |

---

## AI pipeline (hosted API `/admin/*` routes)

All variables below are consumed by `code/server/ai/`. The pipeline is
...
| `OPENAI_API_KEY` | required if `AI_PROVIDER=openai` | — | OpenAI credentials. Also used by `code/server/ai/retrieval.py` for Pinecone-adjacent embedding calls. |
...
|-----|---------|---------|---------|
| `AI_TASK_MAX_COST_USD` | `5.0` | `code/server/db/routers/admin_ai.py`, `code/server/ai/worker.py` | Hard cap per task. Over-budget runs abort before issuing the LLM call. |
| `AI_AUTOFIX_CONTEXT_CHARS_PER_FILE` | `8000` | `code/server/ai/autofix_apply.py` | Per-file context window cap. |
| `AI_AUTOFIX_CONTEXT_CHARS_TOTAL` | `16000` | `code/server/ai/autofix_apply.py` | Total context window cap across all files in a task. |
| `AI_BATCH_FAILURE_MIN_SAMPLE` | `10` | `code/server/ai/batch_orchestrator.py` | Minimum sample size before a batch run can be auto-aborted on failure rate. |
| `AI_APPLY_MAIN_ALLOWED` | `0` | `code/server/ai/batch_orchestrator.py`, `code/server/ai/set_run_orchestrator.py` | Set to `1` to permit the AI pipeline to write directly to `main` (worktrees only otherwise). |
| `AI_WORKER_POLL_SECONDS` | `2.0` | `code/server/ai/worker.py` | Worker poll interval. |
| `AI_WORKER_STALE_SECONDS` | `1800` | `code/server/ai/worker.py` | Stale-task reclaim threshold (seconds). |
| `AI_WORKER_MAX_CONCURRENT` | `1` | `code/server/ai/worker.py` | Max parallel AI tasks. |
| `AI_WORKER_DISABLED` | `0` | `code/server/api.py` | Set to `1` to skip starting the AI worker on app startup. |
| `AI_WORKTREE_DIR` | `$TMPDIR/ai_worktrees` | `code/server/ai/git_adapter.py` | Where the pipeline clones isolated worktrees. |
...
| `MATCHMAKING_RANKED_ENABLED` | `0` | Gate for the ranked public queue (`code/server/routers/matchmaking.py`). Leave unset/`0` during alpha — the jank/casual/sweat queues still run. Set to `1` to expose ranked. |
...
| Var | Default | Purpose |
|-----|---------|---------|
| `DIGIMON_BACKEND` | `py` | Selects the game engine driving `DigimonEnv`. Set to `rust` to route through the PyO3 `digimon-engine` binding (requires `maturin develop` in `code/digimon-engine-py/`). Anything else falls back to the Python engine. |
...
| Var | Default | Used by | Purpose |
|-----|---------|---------|---------|
| `ONNX_MODELS_DIR` | `models` | `code/engine_py_legacy/engine/model_utils.py` | Local directory where exported ONNX models are written/read. |
| `ARCHITECT_MODELS_DIR` | `architect_runs` | `code/server/routers/deck_optimizer.py` | Output dir for the Architect/Q-DeckRec agent runs. |
| `DIGIMON_DATA_DIR` | `data` (at repo root) | `code/data_paths.py` | Override the whole data directory. Individual file env vars below still take precedence. |
...
| `DIGIMON_DECK_LIBRARY` | `$DIGIMON_DATA_DIR/deck_library.json` | `code/data_paths.py` | Path to the scraped deck library. |
| `DIGIMON_ARCHETYPE_ALIASES` | `$DIGIMON_DATA_DIR/archetype_aliases.json` | `code/server/digilab_client.py`, `code/digimon_gym/agents/gauntlet.py` | Override path for the archetype-alias lookup table. |
...
| `DIGIMON_TESTED_CARDS` | `$DIGIMON_DATA_DIR/tested_cards.json` | `code/engine_py_legacy/engine/data/tested_cards.py` | Tested-cards allowlist snapshot consumed by the deck builder. |
...
`code/frontend/.env.local` for dev; pass at build time for production bundles.

| Var | Default | Used by | Purpose |
|-----|---------|---------|---------|
| `VITE_BUILD_TARGET` | *(empty, treated as web)* | `code/frontend/src/App.tsx` and all desktop gating paths | Set to `desktop` to tree-shake admin/training UI for the Tauri build. Any other value (or unset) produces the hosted web bundle. |
| `VITE_API_URL` | `/api` | `code/frontend/src/api/client.ts`, `useWebSocketGame.ts`, `useMatchmaking.ts` | Base URL for REST + WebSocket calls. Leave as default for same-origin deployments; point at the hosted API host for split frontend/backend. |
| `VITE_MODELS_MANIFEST_URL` | *(empty)* | `code/frontend/src/pages/ModelsPage.tsx` | Override for the manifest host. Empty → relative path (`/models/manifest.json`). Useful for pointing the desktop build at a specific hosted API. |
...
| Var | Where | Purpose |
|-----|-------|---------|
| `DIGIMON_ENGINE_COMMIT` | `code/src-tauri/src/models.rs` (`option_env!`) | Engine git SHA baked into the binary, used by the model compatibility gate to display/record which engine build the manifest was validated against. Typically injected by CI at build time. |
| `CARGO_PKG_VERSION` | `code/src-tauri/src/models.rs` (`env!`) | Version string in the HTTP `User-Agent` when fetching the manifest. Set by Cargo automatically. |
