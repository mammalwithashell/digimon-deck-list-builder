...
| `use_option_from_revealed(card: CardHandle, CostDelta) -> OptionPlayResult` | USE an Option from the transient **reveal pool** (`Game::revealed_cards`), identified by stable handle. Same lifecycle; the used card leaves the reveal pool. Driver EX7-048. `G-DSL-USE-OPTION-FROM-SOURCES` (Gap 2). |
| `use_option_from_source(host: PermanentHandle, card: CardHandle, CostDelta) -> OptionPlayResult` | USE an Option sitting as a **digivolution source** under `host` (never the top card — guarded). The Option is pulled out of the stack and disposed per its subtype. Driver BT25-085. `G-DSL-USE-OPTION-FROM-SOURCES` (Gap 2). |
