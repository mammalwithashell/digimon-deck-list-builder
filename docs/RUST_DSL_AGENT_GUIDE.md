...
<!-- vocab-structural-sha: 86e964604e1cd5c49488939a4647a4e7a265f345f91a7d00da4ad5aa3abee66d -->
...
Complete, enum-derived index of every authoring primitive: **163 step verbs**, **184 predicates**, **59 timings**, **12 declarative kinds**. `uses` = card YAMLs referencing the key; `tag` flags `unused` (0 uses) or `rare` (1–2) vocabulary; `fixture` is a real card to open.
...
| `if` | `IfStep` | 236 |  | `_examples/BT17-015.yaml` |  |
| `optional` | `OptionalStep` | 515 |  | `_examples/BT11-042.yaml` |  |
| `per_selected` | `PerSelectedStep` | 36 |  | `ad1/AD1-014.yaml` |  |
| `place_self_as_delay_option` | `EmptyArgs` | 44 |  | `bt13/BT13-110.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `effect_initiated_digivolve` | `EffectDigivolveArgs` | 84 |  | `_examples/BT17-015.yaml` |  |
...
| `recover` | `DrawArgs` | 20 |  | `_examples/BT11-042.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `gain_memory` | `FormulaSpec` | 118 |  | `_examples/BT11-042.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `add_dp_modifier` | `AddDpModifierArgs` | 96 |  | `ad1/AD1-016.yaml` |  |
...
| `place_remainder_on_deck` | `PlaceRemainderArgs` | 78 |  | `_examples/EX11-027.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `play_from_hand` | `PlayFromHandArgs` | 18 |  | `ad1/AD1-019.yaml` |  |
| `play_from_hand_free` | `PlayFromHandFreeArgs` | 72 |  | `bt12/BT12-038.yaml` |  |
| `play_from_materials` | `PlayFromMaterialsArgs` | 24 |  | `_examples/BT20-083.yaml` |  |
...
| `play_or_use_from_hand` | `PlayOrUseFromHandArgs` | 7 |  | `bt25/BT25-041.yaml` | Unified "play OR use 1 card from hand" — inspects the bound hand card's kind and routes Digimon/Tamer → play and Option → use (a DUAL card surfaces a "Play as Digimon / Use as Option" face choice). The card is bound by an upstream `select_hand` step. `G-PLAY-OR-USE-FROM-HAND`. |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `add_to_hand_from_reveal` | `HandleMoveArgs` | 82 |  | `_examples/EX11-027.yaml` |  |
...
| `reveal_top_deck` | `RevealArgs` | 97 |  | `_examples/BT13-007.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `add_bottom_security_to_hand` | `PlayerArg` | 7 |  | `bt24/BT24-090.yaml` |  |
...
| `place_on_security` | `PlaceOnSecurityArgs` | 31 |  | `_examples/BT18-102.yaml` |  |
...
| `select_effect_choice` | `SelectEffectChoiceArgs` | 64 |  | `_examples/BT17-015.yaml` |  |
| `select_hand` | `SelectZoneArgs` | 236 |  | `_examples/BT17-015.yaml` |  |
| `select_material` | `SelectMaterialArgs` | 20 |  | `bt21/BT21-060.yaml` |  |
...
| `select_opponent_permanent` | `SelectFieldArgs` | 281 |  | `_examples/BT12-112.yaml` |  |
...
| `any_field_permanent` | `ExistentialPredicate?` | 27 |  | `bt21/BT21-093.yaml` |  |
| `any_permanent` | `ExistentialPredicate?` | 197 |  | `_examples/BT11-042.yaml` |  |
| `color_matches_any_field_digimon` | `PlayerRefSelector?` | 1 | rare | `p/P-206.yaml` |  |
| `count_gte` | `CountAggregate?` | 104 |  | `_examples/EX11-027.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `all_of` | `[PredicateSpec]` | 584 |  | `_examples/BT11-042.yaml` |  |
| `any_of` | `[PredicateSpec]` | 340 |  | `_examples/BT11-042.yaml` |  |
| `none_of` | `[PredicateSpec]` | 43 |  | `bt13/BT13-019.yaml` |  |
| `not` | `PredicateSpec?` | 10 |  | `ad1/AD1-017.yaml` |  |
| `other` | `bool?` | 27 |  | `ad1/AD1-012.yaml` |  |
| `owner` | `PlayerRef?` | 55 |  | `_examples/BT11-042.yaml` |  |
| `zone` | `[Zone]` | 370 |  | `_examples/BT11-042.yaml` |  |
...
| `your_turn` | `bool?` | 219 |  | `_examples/BT11-042.yaml` |  |
...
| `event_target_owner` | `PlayerRef?` | 114 |  | `_examples/BT11-042.yaml` |  |
...
| `event_target_trait_has` | `str?` | 25 |  | `ad1/AD1-019.yaml` |  |
...
| `color_is` | `ColorSpec?` | 430 |  | `_examples/BT11-042.yaml` |  |
...
| `level_lte` | `DpConstraint?` | 71 |  | `ad1/AD1-014.yaml` |  |
...
| `name_contains` | `str?` | 173 |  | `_examples/AD1-025.yaml` |  |
| `name_in` | `[str]?` | 12 |  | `ad1/AD1-005.yaml` |  |
| `name_is` | `str?` | 114 |  | `_examples/BT12-112.yaml` |  |
...
| `of_permanent` | `str?` | 26 |  | `_examples/BT18-102.yaml` |  |
...
| `trait_has` | `str?` | 443 |  | `_examples/BT11-042.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `replacement_cause` | `ReplacementCauseSpec?` | 35 |  | `bt13/BT13-075.yaml` |  |
| `replacement_source_is_opponent` | `bool?` | 0 | unused | — |  |
| `replacement_subject_is_mine` | `bool?` | 39 |  | `bt13/BT13-075.yaml` |  |
...
| `self_same_level_source_pairs_gte` | `i32?` | 1 | rare | `ex12/EX12-032.yaml` | True when the effect carrier has at least N pairs of same-level digivolution source cards below its top card. Each pair consumes two source cards from the same level bucket (`count / 2` per level), matching the existing `same_level_pairs_in_sources` formula selector. Driver: EX12-032 WereGarurumon — "[When Attacking] If this Digimon's stack has 2 or more same-level cards...". |
