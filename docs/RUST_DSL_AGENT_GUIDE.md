...
<!-- vocab-structural-sha: 77d6cdd3d99ed3c757029ab5d6fd00e8b681fc4f7bcc5ee48b3c55c0b77c34fe -->
...
Complete, enum-derived index of every authoring primitive: **152 step verbs**, **146 predicates**, **56 timings**, **12 declarative kinds**. `uses` = card YAMLs referencing the key; `tag` flags `unused` (0 uses) or `rare` (1–2) vocabulary; `fixture` is a real card to open.
...
| `may_attack_now` | `MayAttackNowArgs` | 24 |  | `ad1/AD1-004.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `activation_cost` | `ActivationCostArgs` | 28 |  | `ad1/AD1-019.yaml` |  |
| `for_each` | `ForEachStep` | 31 |  | `_examples/BT13-007.yaml` |  |
| `if` | `IfStep` | 147 |  | `_examples/BT15-003.yaml` |  |
| `optional` | `OptionalStep` | 357 |  | `_examples/BT11-042.yaml` |  |
| `per_selected` | `PerSelectedStep` | 24 |  | `ad1/AD1-014.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `effect_initiated_digivolve` | `EffectDigivolveArgs` | 60 |  | `_examples/BT17-015.yaml` |  |
...
| `delete_permanent` | `TargetArg` | 104 |  | `_examples/BT17-015.yaml` |  |
| `hatch` | `PlayerArg` | 5 |  | `bt16/BT16-082.yaml` |  |
| `return_to_deck` | `ReturnPermanentArgs` | 25 |  | `_examples/BT12-112.yaml` |  |
| `return_to_hand` | `TargetArg` | 14 |  | `_examples/BT13-060.yaml` |  |
| `suspend` | `TargetArg` | 65 |  | `_examples/BT13-060.yaml` |  |
| `trash_breeding_permanent` | `TrashBreedingPermanentArgs` | 1 | rare | `bt13/BT13-112.yaml` |  |
| `unsuspend` | `TargetArg` | 39 |  | `ad1/AD1-006.yaml` |  |
...
| `add_to_hand_from_trash` | `HandleMoveArgs` | 14 |  | `_examples/BT7-107.yaml` |  |
| `draw` | `DrawArgs` | 84 |  | `_examples/TST_DNA_TRIGGER.yaml` |  |
...
| `recover` | `DrawArgs` | 12 |  | `_examples/BT11-042.yaml` |  |
| `return_all_trash_to_deck_bottom` | `PlayerArg` | 1 | rare | `bt17/BT17-077.yaml` |  |
| `return_trash_list_to_deck_bottom` | `ReturnTrashListToDeckBottomArgs` | 4 |  | `bt24/BT24-017.yaml` |  |
| `shuffle_deck` | `PlayerArg` | 0 | unused | — |  |
| `trash_from_hand_by_index` | `IndexedMoveArgs` | 34 |  | `ad1/AD1-002.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `gain_memory` | `i32` | 86 |  | `_examples/BT11-042.yaml` |  |
| `gain_memory_fn` | `FormulaStepArgs` | 4 |  | `bt11/BT11-033.yaml` | Phase 2 Track F (G-DSL-GAIN-MEMORY-FN) — formula-valued gain. Mirrors the literal `gain_memory: N` shape but accepts a `FormulaSpec` evaluated at resolution time. Use for printed text like "[When Digivolving] Gain 1 memory for every 4 cards in your hand." (EX1-021 MetalGarurumon). |
| `lose_memory` | `i32` | 9 |  | `bt1/BT1-090.yaml` |  |
| `lose_memory_fn` | `FormulaStepArgs` | 0 | unused | — | Symmetric of `GainMemoryFn` — kept for completeness so author-facing API doesn't surprise (literal `lose_memory: N` has a `lose_memory_fn` sibling). No known card uses it as of 2026-05-17 but adding both halves at once keeps the eval-arm coverage matrix uniform. |
...
| `grant_triggered_effect` | `GrantTriggeredEffectArgs` | 7 |  | `bt21/BT21-073.yaml` | Track H §3 — install a granted triggered effect on each permanent matching `target`. The granted body fires on the carrier's matching `timing` (DCGO `AddSkillClass.cs` analog). EX1-068 Ice Wall! is the canonical fixture. |

#### Other — 6 (1 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `add_this_option_to_hand` | `EmptyArgs` | 17 |  | `_examples/BT7-107.yaml` |  |
...
| `play_from_hand_free` | `PlayFromHandFreeArgs` | 57 |  | `bt13/BT13-020.yaml` |  |
...
| `play_token` | `PlayTokenArgs` | 11 |  | `bt20/BT20-017.yaml` |  |
| `play_union_bound_free` | `PlayUnionBoundFreeArgs` | 18 |  | `ad1/AD1-002.yaml` | PUPPETS-G014 — play a `select_union_zone`-bound card for free from its true origin zone (hand, trash, or material), recovered from the binding. |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `cancel_replacement` | `EmptyArgs` | 17 |  | `bt17/BT17-095.yaml` |  |
...
| `place_self_option_at_security` | `SelfSecurityPlacementArgs` | 5 |  | `bt24/BT24-090.yaml` |  |
| `play_from_security` | `PlayFromSecurityArgs` | 82 |  | `_examples/BT9-092.yaml` |  |
...
| `select_security` | `SelectZoneArgs` | 3 |  | `bt13/BT13-012.yaml` |  |
| `shuffle_security` | `PlayerArg` | 3 |  | `_examples/BT11-042.yaml` |  |
| `trash_bottom_security` | `PlayerArg` | 3 |  | `bt15/BT15-003.yaml` |  |
...
| `trash_top_security` | `TrashTopSecurityArgs` | 37 |  | `_examples/BT15-003.yaml` |  |
| `trash_top_security_and_cancel_replacement` | `PlayerArg` | 1 | rare | `bt24/BT24-101.yaml` |  |

#### Selection — 17 (1 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `as_selecting_player` | `AsSelectingPlayerArgs` | 5 |  | `bt21/BT21-024.yaml` |  |
| `digi_burst` | `DigiBurstArgs` | 3 |  | `bt4/BT4-072.yaml` |  |
| `select_any_permanent` | `SelectFieldArgs` | 14 |  | `bt23/BT23-102.yaml` |  |
| `select_count_capped_multi` | `SelectCountCappedArgs` | 18 |  | `_examples/BT18-019.yaml` |  |
| `select_dna_pair` | `SelectDnaPairArgs` | 5 |  | `ad1/AD1-009.yaml` |  |
| `select_effect_choice` | `SelectEffectChoiceArgs` | 48 |  | `_examples/BT15-003.yaml` |  |
| `select_hand` | `SelectZoneArgs` | 166 |  | `_examples/BT17-015.yaml` |  |
...
| `select_opponent_permanent` | `SelectFieldArgs` | 197 |  | `_examples/BT12-112.yaml` |  |
...
| `select_trash` | `SelectZoneArgs` | 62 |  | `_examples/BT18-019.yaml` |  |
| `select_union_zone` | `SelectUnionArgs` | 29 |  | `ad1/AD1-002.yaml` |  |

#### Stack / Source — 13 (0 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `place_as_bottom_source` | `PlaceAsBottomSourceArgs` | 31 |  | `_examples/BT13-007.yaml` |  |
...
| `select_own_sources` | `SelectOwnSourcesArgs` | 16 |  | `bt12/BT12-031.yaml` |  |
...
| `trash_top_stacked_sources` | `TrashTopStackedSourcesArgs` | 3 |  | `bt21/BT21-030.yaml` |  |
...
| `any_permanent` | `ExistentialPredicate?` | 150 |  | `_examples/BT11-042.yaml` |  |
| `color_matches_any_field_digimon` | `PlayerRefSelector?` | 1 | rare | `p/P-206.yaml` |  |
| `count_gte` | `CountAggregate?` | 57 |  | `bt13/BT13-088.yaml` |  |
| `count_lte` | `CountAggregate?` | 15 |  | `bt21/BT21-009.yaml` |  |
| `has_alt_path` | `str?` | 4 |  | `bt10/BT10-087.yaml` |  |
| `level_matches_aggregate` | `LevelAggregatePredicate?` | 6 |  | `ad1/AD1-012.yaml` |  |
| `materials_count_gte` | `DpConstraint?` | 12 |  | `bt13/BT13-112.yaml` |  |
| `materials_count_lte` | `DpConstraint?` | 15 |  | `ad1/AD1-025.yaml` |  |
| `materials_count_matches_aggregate` | `MaterialCountAggregatePredicate?` | 1 | rare | `bt24/BT24-030.yaml` |  |
| `no_permanent` | `ExistentialPredicate?` | 15 |  | `bt12/BT12-016.yaml` |  |

#### Binding — 11 (1 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `binding_absent` | `str?` | 3 |  | `ad1/AD1-024.yaml` |  |
...
| `binding_present` | `str?` | 35 |  | `ad1/AD1-002.yaml` |  |
...
| `equals` | `[]?` | 48 |  | `_examples/BT15-003.yaml` |  |
| `is_source` | `bool?` | 1 | rare | `ad1/AD1-024.yaml` | `is_source: true` — the subject permanent must BE the effect's source permanent (the mirror of `other: true`). Use it to filter a select down to "this Digimon" — e.g. DCGO's standalone "Will you unsuspend this card?" prompt becomes an optional `select_own_permanent` with `is_source: true`, exposing the Yes/No to the RL action space. |
| `not_equals` | `[]?` | 0 | unused | — |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `all_of` | `[PredicateSpec]` | 420 |  | `_examples/BT11-042.yaml` |  |
| `any_of` | `[PredicateSpec]` | 255 |  | `_examples/BT11-042.yaml` |  |
| `none_of` | `[PredicateSpec]` | 23 |  | `bt17/BT17-095.yaml` |  |
| `not` | `PredicateSpec?` | 6 |  | `ad1/AD1-024.yaml` |  |
| `other` | `bool?` | 21 |  | `ad1/AD1-012.yaml` |  |
| `owner` | `PlayerRef?` | 48 |  | `_examples/BT11-042.yaml` |  |
| `zone` | `[Zone]` | 268 |  | `_examples/BT11-042.yaml` |  |

#### Context / global — 18 (2 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `all_turns` | `bool?` | 97 |  | `ad1/AD1-001.yaml` |  |
| `can_hatch` | `PlayerRef?` | 2 | rare | `bt16/BT16-082.yaml` |  |
| `dna_origin` | `bool?` | 13 |  | `_examples/BT18-019.yaml` |  |
...
| `opponents_turn` | `bool?` | 30 |  | `_examples/BT11-042.yaml` |  |
...
| `security_count_gte` | `DpConstraint?` | 11 |  | `_examples/BT15-003.yaml` |  |
| `security_count_lte` | `DpConstraint?` | 11 |  | `_examples/BT20-083.yaml` |  |
| `your_turn` | `bool?` | 154 |  | `_examples/BT11-042.yaml` |  |

#### Effect-history — 10 (3 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `effect_added_any_card_to_hand` | `bool?` | 0 | unused | — |  |
| `effect_deleted_any_opponent_digimon` | `bool?` | 6 |  | `bt12/BT12-016.yaml` |  |
...
| `effect_text_contains` | `str?` | 10 |  | `bt12/BT12-057.yaml` | Case-insensitive substring scan against the candidate card's printed text — `effect_text`, `inherited_text`, and `security_text` concatenated. Distinct from `name_contains`, which only scans `card_name`. Used by BT22-017's bucket 1 ("1 card with [Omnimon] in its text"). DCGO `source.HasText(s)`. G-DSL-PREDICATE-TEXT-CONTAINS. |
...
| `event_card_name_contains` | `str?` | 12 |  | `ad1/AD1-001.yaml` |  |
| `event_card_trait_has` | `str?` | 5 |  | `_examples/BT13-007.yaml` |  |
...
| `event_is_effect_initiated` | `bool?` | 9 |  | `ad1/AD1-024.yaml` |  |
| `event_permanent_is_source` | `bool?` | 18 |  | `bt15/BT15-101.yaml` |  |
| `event_target_color_any_of` | `[ColorSpec]?` | 4 |  | `bt13/BT13-012.yaml` | Match when the *event target* permanent's printed color set intersects this list — i.e. the digivolving / played / deleted / suspended permanent on the triggered-effect read context has at least one of the listed colors. Sibling of `event_target_kind` / `event_target_trait_has`, using the same `event_target_card` resolver. Used by BT13-012's inherited clause ("when one of your red or yellow Tamers becomes suspended"). G-EVENT-TARGET-COLOR. |
...
| `event_target_is_source` | `bool?` | 3 |  | `bt20/BT20-017.yaml` |  |
| `event_target_kind` | `CardKind?` | 60 |  | `ad1/AD1-014.yaml` |  |
| `event_target_level_eq` | `i32?` | 1 | rare | `bt8/BT8-094.yaml` | Match the event-target permanent's printed level. Works for live event targets (played/digivolved/moved/suspended permanents) and deleted-object snapshots. G-EVENT-TARGET-LEVEL-LTE. |
| `event_target_level_gte` | `DpConstraint?` | 0 | unused | — |  |
...
| `event_target_owner` | `PlayerRef?` | 93 |  | `_examples/BT11-042.yaml` |  |
| `event_target_same_level_as_previous` | `bool?` | 0 | unused | — |  |
| `event_target_trait_has` | `str?` | 19 |  | `ad1/AD1-019.yaml` |  |
| `event_target_was_self` | `bool?` | 0 | unused | — |  |

#### Identity / state — 45 (7 unused)
...
| `card_number_is` | `str?` | 47 |  | `bt17/BT17-093.yaml` |  |
| `color_is` | `ColorSpec?` | 324 |  | `_examples/BT11-042.yaml` |  |
...
| `dp_gte` | `DpConstraint?` | 8 |  | `bt15/BT15-101.yaml` |  |
| `dp_lte` | `DpConstraint?` | 58 |  | `_examples/BT17-015.yaml` |  |
...
| `is_source_permanent` | `bool?` | 1 | rare | `bt24/BT24-062.yaml` | True when the subject permanent IS the effect's source permanent (the carrier/host). Lets `kind: flood_gate` / `kind: aura` target `self` — install a modifier on the carrier itself instead of scanning the whole board with an aux self-identity predicate. Pair with `scope: both` so the carrier resolves to the active top (face_up) AND the host of the digivolution stack (inherited). (BT24-062 attack-target lock.) |
| `is_suspended` | `bool?` | 19 |  | `ad1/AD1-012.yaml` |  |
...
| `level_lte` | `DpConstraint?` | 38 |  | `ad1/AD1-014.yaml` |  |
| `name_contains` | `str?` | 125 |  | `_examples/AD1-025.yaml` |  |
| `name_in` | `[str]?` | 10 |  | `ad1/AD1-005.yaml` |  |
| `name_is` | `str?` | 74 |  | `_examples/BT12-112.yaml` |  |
...
| `play_cost_gte` | `DpConstraint?` | 3 |  | `bt22/BT22-089.yaml` |  |
| `play_cost_lte` | `DpConstraint?` | 32 |  | `ad1/AD1-018.yaml` |  |
...
| `trait_has` | `str?` | 292 |  | `_examples/BT11-042.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `replacement_cause` | `ReplacementCauseSpec?` | 20 |  | `bt17/BT17-095.yaml` |  |
| `replacement_source_is_opponent` | `bool?` | 0 | unused | — |  |
| `replacement_subject_is_mine` | `bool?` | 23 |  | `bt17/BT17-095.yaml` |  |
...
| `self_digivolution_sources_trait_has` | `str?` | 4 |  | `bt13/BT13-110.yaml` | Like `self_digivolution_sources_contain_name`, but matches any digivolution source card carrying the named trait. Used by Royal Knights breeding-source effects to gate carriers that actually contain playable [Royal Knight] sources. |
...
| `source_is_unsuspended` | `bool?` | 19 |  | `_examples/BT20-083.yaml` |  |
| `source_name_contains` | `str?` | 12 |  | `_examples/BT17-015.yaml` |  |
...
| `end_of_opponents_turn` | 92 |  |
| `end_of_your_next_turn` | 9 |  |
| `end_of_your_turn` | 27 |  |
| `main` | 27 |  |
| `main_from_hand` | 88 |  |
| `main_from_trash` | 0 |  |
| `main_on_field` | 14 |  |
| `on_add_to_hand` | 1 | `[All Turns]`-style observer: an EFFECT added one or more cards to a player's hand (return-to-hand, security/trash/deck/reveal-to-hand, …). Gate the gaining player with `event_add_to_hand_player:` and effect-vs-draw with `event_is_effect_initiated:` inside `active_when:`. See G-ON-ADD-TO-HAND-OBSERVER. |
| `on_added_to_security` | 1 |  |
| `on_ally_attack` | 4 |  |
| `on_ally_played` | 11 |  |
| `on_any_deletion` | 21 |  |
| `on_any_digimon_played` | 7 |  |
...
| `on_digivolve` | 34 |  |
| `on_digixros` | 0 |  |
| `on_discard_security` | 2 |  |
| `on_dna_digivolve` | 5 |  |
| `on_enter_field_anyone` | 24 |  |
| `on_hatch` | 1 |  |
| `on_leave_field` | 1 |  |
| `on_lose_security` | 4 |  |
| `on_move` | 11 |  |
| `on_opponent_attack` | 15 |  |
| `on_opponent_security_removed` | 17 |  |
| `on_option_placed` | 2 |  |
| `on_option_trashed` | 1 |  |
| `on_own_security_removed` | 10 |  |
| `on_place_security` | 0 |  |
| `on_play` | 225 |  |
| `on_security` | 168 |  |
| `on_security_check` | 0 |  |
