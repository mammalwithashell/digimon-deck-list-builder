...
<!-- vocab-structural-sha: 7f66c5ae097da61cde7eaa250aba381b81aab541f035ed6132265857ccc6a5f4 -->
...
Complete, enum-derived index of every authoring primitive: **160 step verbs**, **183 predicates**, **59 timings**, **12 declarative kinds**. `uses` = card YAMLs referencing the key; `tag` flags `unused` (0 uses) or `rare` (1–2) vocabulary; `fixture` is a real card to open.
...
| `if` | `IfStep` | 215 |  | `_examples/BT15-003.yaml` |  |
| `optional` | `OptionalStep` | 479 |  | `_examples/BT11-042.yaml` |  |
...
| `trash_link_card_of_own_digimon` | `TrashLinkCardOfOwnDigimonArgs` | 1 | rare | `bt25/BT25-073.yaml` | G-DSL-LINK-TRASH-AS-COST (BT25-073 Dragomon) â€” the ACTIVATION-cost sibling of the replacement-only `cost: { trash_own_link_card: true }`. Picks one of `of`'s Digimon that carries â‰¥1 link card (first selection), then one of ITS link cards (second selection), trashes it (firing `OnLinkedCardTrashed`), and runs the step's tail ONLY if the trash happened (no-approximations cost gate). When no own Digimon has a link card the cost is unpayable â†’ the clause's remaining steps are skipped (`TailAlreadyRan`). DCGO `BT25_073.cs`: `SelectPermanentEffect` (`!HasNoLinkCards`) â†’ `SelectCardEffect Root.LinkedCards` â†’ `TrashLinkCardsAndProcessAccordingToResult` â†’ `successProcess`. |

#### Memory — 3 (0 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `gain_memory` | `FormulaSpec` | 116 |  | `_examples/BT11-042.yaml` |  |
...
| `place_remainder_on_deck` | `PlaceRemainderArgs` | 75 |  | `_examples/BT9-092.yaml` |  |
...
| `play_or_use_from_hand` | `PlayOrUseFromHandArgs` | 5 |  | `bt25/BT25-041.yaml` | Unified "play OR use 1 card from hand" â€” inspects the bound hand card's kind and routes Digimon/Tamer â†’ play and Option â†’ use (a DUAL card surfaces a "Play as Digimon / Use as Option" face choice). The card is bound by an upstream `select_hand` step. `G-PLAY-OR-USE-FROM-HAND`. |
| `play_token` | `PlayTokenArgs` | 12 |  | `bt20/BT20-017.yaml` |  |
| `play_union_bound_free` | `PlayUnionBoundFreeArgs` | 30 |  | `ad1/AD1-002.yaml` | PUPPETS-G014 â€” play a `select_union_zone`-bound card for free from its true origin zone (hand, trash, or material), recovered from the binding. |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `add_to_hand_from_reveal` | `HandleMoveArgs` | 79 |  | `_examples/BT9-092.yaml` |  |
...
| `reveal_top_deck` | `RevealArgs` | 94 |  | `_examples/BT13-007.yaml` |  |
| `select_reveal` | `SelectZoneArgs` | 38 |  | `_examples/BT9-092.yaml` |  |
...
| `play_from_security` | `PlayFromSecurityArgs` | 100 |  | `_examples/BT9-092.yaml` |  |
...
| `trash_top_security` | `TrashTopSecurityArgs` | 49 |  | `_examples/BT15-003.yaml` |  |
...
| `select_effect_choice` | `SelectEffectChoiceArgs` | 62 |  | `_examples/BT15-003.yaml` |  |
...
| `select_trash` | `SelectZoneArgs` | 87 |  | `_examples/BT18-019.yaml` |  |
...
| `count_gte` | `CountAggregate?` | 94 |  | `_examples/EX11-027.yaml` |  |
...
| `equals` | `[]?` | 62 |  | `_examples/BT15-003.yaml` |  |
| `is_source` | `bool?` | 2 | rare | `ad1/AD1-024.yaml` | `is_source: true` â€” the subject permanent must BE the effect's source permanent (the mirror of `other: true`). Use it to filter a select down to "this Digimon" â€” e.g. DCGO's standalone "Will you unsuspend this card?" prompt becomes an optional `select_own_permanent` with `is_source: true`, exposing the Yes/No to the RL action space. |
| `not_equals` | `[]?` | 0 | unused | — |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `all_of` | `[PredicateSpec]` | 550 |  | `_examples/BT11-042.yaml` |  |
...
| `zone` | `[Zone]` | 354 |  | `_examples/BT11-042.yaml` |  |
...
| `event_discard_player` | `PlayerRef?` | 1 | rare | `st16/ST16-14.yaml` | For `on_discard_hand` observers: the player whose HAND was trashed (`TriggerContext.discard_hand_player`) must match this player-ref ("your hand is trashed from" â‡’ `you`). G-ENGINE-ON-DISCARD-HAND. |
...
| `event_target_color_any_of` | `[ColorSpec]?` | 10 |  | `bt11/BT11-089.yaml` | Match when the *event target* permanent's printed color set intersects this list â€” i.e. the digivolving / played / deleted / suspended permanent on the triggered-effect read context has at least one of the listed colors. Sibling of `event_target_kind` / `event_target_trait_has`, using the same `event_target_card` resolver. Used by BT13-012's inherited clause ("when one of your red or yellow Tamers becomes suspended"). G-EVENT-TARGET-COLOR. |
...
| `kind` | `CardKind?` | 775 |  | `_examples/AD1-025.yaml` |  |
...
| `level_eq` | `i32?` | 482 |  | `_examples/AD1-025.yaml` |  |
...
| `level_lte` | `DpConstraint?` | 57 |  | `ad1/AD1-014.yaml` |  |
...
| `on_security` | 213 |  |
| `on_security_check` | 0 |  |
| `on_source_returned_to_deck_bottom` | 2 | `[All Turns]`-style observer: a digivolution source was RETURNED to the bottom of a player's deck (not trashed) â€” "when any [Vemmon] return to the bottom of the deck from this Digimon's digivolution cards" (BT21-058, BT18-065). Gate host-scope with `active_when: { event_host_permanent_is_source: true }` and the returned card's name with `event_card_name_contains:`. Distinct from `on_digivolution_card_trashed`. G-ENGINE-DIGIVOLUTION-CARD-RETURNED-TO-DECK-BOTTOM. |
| `on_suspend` | 34 |  |
| `on_unsuspend` | 2 |  |
