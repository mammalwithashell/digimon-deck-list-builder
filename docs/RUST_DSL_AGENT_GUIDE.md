...
<!-- vocab-structural-sha: 993bf4c3eb4b5fe2d23325284ebc6157518fef564357d3a9f0dccef5b0635644 -->
...
Complete, enum-derived index of every authoring primitive: **164 step verbs**, **185 predicates**, **59 timings**, **12 declarative kinds**. `uses` = card YAMLs referencing the key; `tag` flags `unused` (0 uses) or `rare` (1–2) vocabulary; `fixture` is a real card to open.
...
| `force_attack` | `ForceAttackArgs` | 7 |  | `bt20/BT20-102.yaml` |  |
...
| `refund_opt` | `EmptyArgs` | 2 | rare | `ad1/AD1-024.yaml` | Refund this clause's once-per-turn use (DCGO `ActivateClass.RemoveUse()` — "if nothing executed, the per-turn use is not consumed"). Place it under a final `if:` whose condition detects the nothing-executed case (typically `binding_absent` over every pick the body could make). Only meaningful inside a `once_per_turn`/`max_per_turn` triggered clause; a no-op elsewhere. G-OPT-REFUND-ON-DECLINE. |

#### Control flow — 8 (0 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `activation_cost` | `ActivationCostArgs` | 48 |  | `ad1/AD1-019.yaml` |  |
| `for_each` | `ForEachStep` | 35 |  | `_examples/BT13-007.yaml` |  |
| `if` | `IfStep` | 242 |  | `_examples/BT17-015.yaml` |  |
| `optional` | `OptionalStep` | 537 |  | `_examples/BT11-042.yaml` |  |
| `per_selected` | `PerSelectedStep` | 37 |  | `_examples/BT18-019.yaml` |  |
| `place_self_as_delay_option` | `EmptyArgs` | 45 |  | `bt13/BT13-110.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `effect_initiated_digivolve` | `EffectDigivolveArgs` | 85 |  | `_examples/BT17-015.yaml` |  |
...
| `de_digivolve` | `DeDigivolveArgs` | 43 |  | `ad1/AD1-009.yaml` |  |
| `delete_bound_permanents` | `DeleteBoundPermanentsArgs` | 4 |  | `bt17/BT17-018.yaml` |  |
| `delete_permanent` | `TargetArg` | 154 |  | `_examples/BT17-015.yaml` |  |
...
| `unsuspend` | `TargetArg` | 48 |  | `ad1/AD1-006.yaml` |  |
...
| `add_to_hand_from_trash` | `HandleMoveArgs` | 22 |  | `_examples/BT7-107.yaml` |  |
| `draw` | `DrawArgs` | 119 |  | `_examples/TST_DNA_TRIGGER.yaml` |  |
...
| `return_trash_list_to_deck_bottom` | `ReturnTrashListToDeckBottomArgs` | 10 |  | `bt13/BT13-083.yaml` |  |
| `shuffle_deck` | `PlayerArg` | 0 | unused | — |  |
| `trash_from_hand_by_index` | `IndexedMoveArgs` | 55 |  | `ad1/AD1-002.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `gain_memory` | `FormulaSpec` | 119 |  | `_examples/BT11-042.yaml` |  |
| `lose_memory` | `FormulaSpec` | 11 |  | `bt1/BT1-090.yaml` |  |
| `set_memory` | `FormulaSpec` | 24 |  | `bt1/BT1-085.yaml` |  |

#### Modifier / Keyword — 8 (0 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `add_dp_modifier` | `AddDpModifierArgs` | 97 |  | `ad1/AD1-016.yaml` |  |
| `add_modifier` | `AddModifierArgs` | 78 |  | `_examples/BT13-060.yaml` |  |
...
| `grant_keyword` | `GrantKeywordArgs` | 78 |  | `_examples/BT11-042.yaml` |  |
...
| `play_from_trash_free` | `PlayFromHandArgs` | 37 |  | `_examples/BT18-019.yaml` |  |
...
| `play_union_bound_free` | `PlayUnionBoundFreeArgs` | 31 |  | `ad1/AD1-002.yaml` | PUPPETS-G014 — play a `select_union_zone`-bound card for free from its true origin zone (hand, trash, or material), recovered from the binding. |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `add_to_hand_from_reveal` | `HandleMoveArgs` | 84 |  | `_examples/EX11-027.yaml` |  |
...
| `reveal_top_deck` | `RevealArgs` | 100 |  | `_examples/BT13-007.yaml` |  |
| `select_reveal` | `SelectZoneArgs` | 39 |  | `_examples/EX11-027.yaml` |  |
| `select_reveal_buckets` | `SelectRevealBucketsArgs` | 53 |  | `bt11/BT11-061.yaml` |  |
...
| `play_from_security` | `PlayFromSecurityArgs` | 100 |  | `ad1/AD1-019.yaml` |  |
...
| `select_count_capped_multi` | `SelectCountCappedArgs` | 29 |  | `_examples/BT18-019.yaml` |  |
...
| `select_hand` | `SelectZoneArgs` | 240 |  | `_examples/BT17-015.yaml` |  |
...
| `select_opponent_permanent` | `SelectFieldArgs` | 285 |  | `_examples/BT12-112.yaml` |  |
...
| `select_own_permanent` | `SelectFieldArgs` | 189 |  | `_examples/BT12-112.yaml` |  |
| `select_trash` | `SelectZoneArgs` | 94 |  | `_examples/BT18-019.yaml` |  |
| `select_union_zone` | `SelectUnionArgs` | 52 |  | `ad1/AD1-002.yaml` |  |

#### Stack / Source — 17 (1 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `place_as_bottom_source` | `PlaceAsBottomSourceArgs` | 60 |  | `_examples/BT13-007.yaml` |  |
...
| `any_permanent` | `ExistentialPredicate?` | 202 |  | `_examples/BT11-042.yaml` |  |
| `color_matches_any_field_digimon` | `PlayerRefSelector?` | 1 | rare | `p/P-206.yaml` |  |
| `count_gte` | `CountAggregate?` | 112 |  | `_examples/EX11-027.yaml` |  |
...
| `level_matches_aggregate` | `LevelAggregatePredicate?` | 11 |  | `ad1/AD1-012.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `binding_absent` | `str?` | 5 |  | `ad1/AD1-024.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `all_of` | `[PredicateSpec]` | 599 |  | `_examples/BT11-042.yaml` |  |
| `any_of` | `[PredicateSpec]` | 350 |  | `_examples/BT11-042.yaml` |  |
| `none_of` | `[PredicateSpec]` | 44 |  | `bt13/BT13-019.yaml` |  |
| `not` | `PredicateSpec?` | 12 |  | `ad1/AD1-017.yaml` |  |
| `other` | `bool?` | 27 |  | `ad1/AD1-012.yaml` |  |
| `owner` | `PlayerRef?` | 56 |  | `_examples/BT11-042.yaml` |  |
| `zone` | `[Zone]` | 381 |  | `_examples/BT11-042.yaml` |  |

#### Context / global — 22 (4 unused)

| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `all_turns` | `bool?` | 131 |  | `ad1/AD1-001.yaml` |  |
...
| `memory_lte` | `DpConstraint?` | 35 |  | `bt1/BT1-085.yaml` |  |
...
| `your_turn` | `bool?` | 227 |  | `_examples/BT11-042.yaml` |  |
...
| `event_target_owner` | `PlayerRef?` | 116 |  | `_examples/BT11-042.yaml` |  |
...
| `color_is` | `ColorSpec?` | 453 |  | `_examples/BT11-042.yaml` |  |
...
| `level_lte` | `DpConstraint?` | 79 |  | `ad1/AD1-014.yaml` |  |
...
| `name_is` | `str?` | 118 |  | `_examples/BT12-112.yaml` |  |
...
| `play_cost_lte` | `DpConstraint?` | 44 |  | `ad1/AD1-018.yaml` |  |
...
| `trait_has` | `str?` | 457 |  | `_examples/BT11-042.yaml` |  |
...
| `source_count` | `SourceCountPredicate?` | 2 | rare | `ex7/EX7-010.yaml` | Permanent-subject predicate. True when the candidate carries at least `at_least` digivolution SOURCE cards (the cards beneath its top card) matching the nested `filter`. Unlike `materials_count_gte` (which counts ALL sources by raw stack length), this counts only sources satisfying an arbitrary card predicate — the DCGO `DigivolutionCards.Count(predicate) >= N` idiom. Drives P-094 Destromon's inherited gate: "1 of your [Galacticmon]'s digivolution cards" must carry ≥2 [Vemmon] before the return-2-Vemmon cost is offered. The nested `filter` is evaluated against each source card (source subject), so it accepts `name_is` / `name_contains` / `trait_has` / `kind` / etc. G-DSL-SOURCE-COUNT-FILTERED. |
...
| `source_is_unsuspended` | `bool?` | 32 |  | `_examples/BT20-083.yaml` |  |
...
| `end_of_attack` | 15 |  |
| `end_of_battle` | 0 |  |
| `end_of_opponents_next_turn` | 8 |  |
| `end_of_opponents_turn` | 125 |  |
...
| `start_of_your_main_phase` | 68 |  |
| `start_of_your_turn` | 35 |  |
