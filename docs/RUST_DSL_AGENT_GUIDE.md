...
<!-- vocab-structural-sha: 01d8357792419f06d948507e3b3dff647c6798632c9bd4f0ba68fc207e3bfbdd -->
...
Complete, enum-derived index of every authoring primitive: **165 step verbs**, **189 predicates**, **61 timings**, **12 declarative kinds**. `uses` = card YAMLs referencing the key; `tag` flags `unused` (0 uses) or `rare` (1–2) vocabulary; `fixture` is a real card to open.
...
| `optional` | `OptionalStep` | 551 |  | `_examples/BT11-042.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `effect_initiated_digivolve` | `EffectDigivolveArgs` | 86 |  | `_examples/BT17-015.yaml` |  |
...
| `suspend` | `TargetArg` | 74 |  | `_examples/BT13-060.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `gain_memory` | `FormulaSpec` | 121 |  | `_examples/BT11-042.yaml` |  |
...
| `place_remainder_on_deck` | `PlaceRemainderArgs` | 81 |  | `_examples/EX11-027.yaml` |  |
...
| `trash_union_bound` | `UnionBoundArgs` | 7 |  | `bt21/BT21-054.yaml` | Trash a `select_union_zone`-bound card from its true origin zone. Used for costs that can be paid by trashing a card from hand or from one of your Digimon's digivolution cards. |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `add_to_hand_from_reveal` | `HandleMoveArgs` | 86 |  | `_examples/EX11-027.yaml` |  |
...
| `reveal_top_deck` | `RevealArgs` | 102 |  | `_examples/BT13-007.yaml` |  |
| `select_reveal` | `SelectZoneArgs` | 39 |  | `_examples/EX11-027.yaml` |  |
| `select_reveal_buckets` | `SelectRevealBucketsArgs` | 55 |  | `bt11/BT11-061.yaml` |  |
...
| `select_hand` | `SelectZoneArgs` | 244 |  | `_examples/BT17-015.yaml` |  |
...
| `select_own_permanent` | `SelectFieldArgs` | 195 |  | `_examples/BT12-112.yaml` |  |
| `select_trash` | `SelectZoneArgs` | 96 |  | `_examples/BT18-019.yaml` |  |
| `select_union_zone` | `SelectUnionArgs` | 59 |  | `ad1/AD1-002.yaml` |  |
...
| `any_permanent` | `ExistentialPredicate?` | 208 |  | `_examples/BT11-042.yaml` |  |
| `color_matches_any_field_digimon` | `PlayerRefSelector?` | 1 | rare | `p/P-206.yaml` |  |
| `count_gte` | `CountAggregate?` | 118 |  | `_examples/EX11-027.yaml` |  |
...
| key | arg | uses | tag | fixture | description |
|-----|-----|------|-----|---------|-------------|
| `all_of` | `[PredicateSpec]` | 612 |  | `_examples/BT11-042.yaml` |  |
| `any_of` | `[PredicateSpec]` | 358 |  | `_examples/BT11-042.yaml` |  |
...
| `zone` | `[Zone]` | 393 |  | `_examples/BT11-042.yaml` |  |
...
| `your_turn` | `bool?` | 228 |  | `_examples/BT11-042.yaml` |  |
...
| `color_is` | `ColorSpec?` | 466 |  | `_examples/BT11-042.yaml` |  |
...
| `name_contains` | `str?` | 174 |  | `_examples/AD1-025.yaml` |  |
...
| `trait_has` | `str?` | 469 |  | `_examples/BT11-042.yaml` |  |
...
| `source_count` | `SourceCountPredicate?` | 5 |  | `bt21/BT21-054.yaml` | Permanent-subject predicate. True when the candidate carries at least `at_least` digivolution SOURCE cards (the cards beneath its top card) matching the nested `filter`. Unlike `materials_count_gte` (which counts ALL sources by raw stack length), this counts only sources satisfying an arbitrary card predicate — the DCGO `DigivolutionCards.Count(predicate) >= N` idiom. Drives P-094 Destromon's inherited gate: "1 of your [Galacticmon]'s digivolution cards" must carry ≥2 [Vemmon] before the return-2-Vemmon cost is offered. The nested `filter` is evaluated against each source card (source subject), so it accepts `name_is` / `name_contains` / `trait_has` / `kind` / etc. G-DSL-SOURCE-COUNT-FILTERED. |
...
| `source_is_unsuspended` | `bool?` | 33 |  | `_examples/BT20-083.yaml` |  |
