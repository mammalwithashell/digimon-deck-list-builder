...
class BT12_022(CardScript):
    """BT12-022 ExVeemon | Lv.4 Digimon"""
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        effect_dna.set_effect_name("BT12-022 DNA digivolve memory +1")
        effect_dna.set_effect_description(
            "[Your Turn] When this Digimon would DNA digivolve into a green "
            "Digimon card, gain 1 memory."
        )
        effect_dna.set_hash_string("Memory+1_BT12_022")
        ...
        def condition_dna(context: Dict[str, Any]) -> bool:
            ...
            if not perm:
                return False
            # This card must be in the digi-stack of the digivolved permanent
            digivolved = context.get('digivolved_permanent')
            if not digivolved:
                return False
            if card not in digivolved.card_sources:
                return False
            # The resulting Digimon must be green
        ...
        def process_dna(ctx: Dict[str, Any]):
            ...
            if player and game:
                player.add_memory(1)
                game.logger.log(
                    f"[BT12-022] {player.player_name} gained 1 memory "
                    f"(DNA digivolve into green)."
                )
        ...
        effect0 = ICardEffect()
        effect0.set_effect_name("BT12-022 Jamming (inherited)")
