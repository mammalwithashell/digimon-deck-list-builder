...
## Mastemon closeout (2026-05-23) — best-deck substrate closed

`unblock-mastemon-rust-dsl` closes the reusable security-stack and result-gating shapes needed by the resolved Mastemon best deck:

- `place_permanent_on_owners_security` routes selected own or opponent permanents into the selected permanent's owner's security stack and preserves replacement / observer handling.
- `if_trash_top_security_cost` and `if_place_permanent_on_owners_security_cost` model security-stack costs as payable gates; declined, unpayable, prevented, and successful paths keep player choices visible through pending selections.
- `play_security_card` and existing bound `effect_initiated_digivolve` cover selected-security play/digivolve, preserve remaining stack order, record success, and support card-local `if you did` tails.
- `play_this_from_trash_free`, `recover_for_deleted`, successful deletion result logging, and iteration result-log merging cover the support-core recursion / deletion / recovery shapes surfaced by `BT9-082`, `BT7-107`, and `EX6-022`.
- `queued_effect_wants_outer_optional_prompt` now evaluates event-context predicates under the queued trigger context, so optional triggered clauses with mandatory first steps are prompted instead of silently suppressed.

Verification drivers: DSL `zone_movement_verbs`, `security_stack_steps`, and the Mastemon best-deck card filters under `cards_behavioral` (`ex6_029`, `p_187`, `bt23_102`, `bt14_033`, `st10_04`, `ex6_020`, `bt23_031`, `bt23_067`, `bt11_042`, `bt11_083`, `bt11_094`, `ex6_074`, `bt15_037`, `ex6_022`, `bt9_082`, `bt7_107`, plus pre-existing `bt15_003`, `bt22_089`, `ex8_030`, `p_206`).
...
- Updated 2026-05-23 (`unblock-mastemon-rust-dsl`): Mastemon best-deck blockers are closed without action/tensor contract changes. New or verified DSL vocabulary includes `place_permanent_on_owners_security`, `if_trash_top_security_cost`, `if_place_permanent_on_owners_security_cost`, `play_security_card`, `play_this_from_trash_free`, and `recover_for_deleted`. The engine now records successful permanent deletion for result-gated tails, merges per-effect result-log deltas out of `for_each` / `per_selected` iteration bodies, and evaluates outer optional prompt conditions with the queued trigger context installed. Closed card drivers include `EX6-029`, `P-187`, `BT23-102`, `BT14-033`, `ST10-04`, `EX6-020`, `BT23-031`, `BT23-067`, `BT11-042`, `BT11-083`, `BT11-094`, `EX6-074`, `BT15-037`, `EX6-022`, `BT9-082`, and `BT7-107`; selected-security play/digivolve tails and searched-security shuffle behavior are covered by focused DSL and card tests.
