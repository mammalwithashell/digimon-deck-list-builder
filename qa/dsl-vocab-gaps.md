...
- Engine substrate (already present): `EffectContext::select_count_capped_multi` with `CountCappedZone::Material(PermanentHandle)` + `DistinctByMode::Name`. `select_materials` lowers straight onto it — REUSING the existing count-capped action mask (no `ACTION_SPACE_SIZE` change). No new `SelectionKind` variant or `play_from_own_digivolution_cards` helper was needed.
- DSL syntax (landed):
  ```yaml
  - select_materials:
