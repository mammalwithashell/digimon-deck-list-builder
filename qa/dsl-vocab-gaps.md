...
- Updated 2026-04-29: the OnDigivolve half now has runtime event-card context for normal `Game::digivolve_from_hand`; `event_card_trait_has` reads the new top card. Covered by `cargo test --manifest-path code/digimon-engine/Cargo.toml --test dsl -- on_digivolve_event_card_trait_predicate_matches_new_top_card`. Source-trash host/source context remains open.
