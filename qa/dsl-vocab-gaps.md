...
- **Status:** RESOLVED → **IMPLEMENTED** (2026-09-17, three-musketeers-1 / gap-closure stage). Two new predicate leaves close DCGO's `DigivolveIntoHandOrTrashCard` gates for a digivolve whose TARGET is a bound permanent rather than the effect's source: card-subject **`can_digivolve_onto: <binding>`** (DCGO `cardSource.CanPlayCardTargetFrame(selectedPermanent.PermanentFrame, …)` — ≥1 non-App-Fusion route via `Game::all_digivolve_routes_for_card`, the same enumerator the commit path uses) and permanent-subject **`has_digivolve_candidate: { of, zone: [hand|trash], filter }`** (DCGO `CanDigivolveDigimon(permanent)`). Re-audit of the two sub-gaps: (1) the RESULT union was already lowerable — a `select_union_zone{hand,trash}` binding resolves to its true `CardSourceRef::Hand/Trash` and `effect_initiated_digivolve { source: evo }` digivolves into a trash-resident card (BT16-040 precedent; §8-1-3-1..3-3 procedure, §8-1-2-7 restore-on-failure); (2) the COST union was already lowerable — `select_union_zone { zones: [hand, material] }` with `material_of` omitted scans every own Digimon's digivolution cards (BT25-085 convention) and `trash_union_bound` trashes from the true origin. The genuinely missing pieces were the two gates above; without them the target pick would offer Digimon with no legal result and the result pick would offer cards the commit rejects (a mask/no-op mismatch). YAML: `cards/bt25/BT25-092.yaml` clause 2; tests: `tests/cards_behavioral/bt25/bt25_092.rs` (12: cost from hand / cost from sources / both origins in ONE prompt / result from hand cost −1 / result from trash / no-candidate target ends clause / decline target / decline result / suspended-Tamer + no-Option activation refusals) + `tests/dsl/digivolve_target_predicates.rs` (6: compile, undeclared-binding and unsupported-zone validator rejections, both leaves' semantics). `[Main]` suspend cost authored with the `condition: { source_is_unsuspended }` + `suspend: { target: source }` idiom (BT11-061) — NOT `activation_cost`, which at the time was not paid on the `[Main]` action path (G-ENGINE-MAIN-ON-FIELD-ACTIVATION-COST-UNPAID — RESOLVED 2026-09-20, `qa/resolved-gaps.md`; this authoring stays as-is and tested). Verdict PARTIAL → IMPLEMENTED in `validated_cards_dsl.json`.
...
> **BT25-085 BeelStarmon — RESOLVED 2026-09-13** (three-musketeers-1 / slice ex7-tops). Both facets closed: (1) use-from-hand-OR-this-Digimon's-digivolution-cards = ONE `select_union_zone { zones: [hand, material], material_of: source }` + `use_option_bound`, whose `Material` origin now routes `OptionSource::Source { host, card }` (was a silent no-op); (2) trash-Option-from-digivolution-OR-link-cards-as-cost = the shipped `trash_option_from_own_stacks` (both origins verified), activation-gated by `source_count` + the NEW `link_card_count` predicate leaf. YAML: `code/digimon-engine/cards/bt25/BT25-085.yaml`; tests: `tests/cards_behavioral/bt25/bt25_085.rs` (18).
>
> **G-DSL-DRAW-FORMULA-COUNT — RESOLVED 2026-09-13.** `draw: { of, count: <formula> }` (literal → `CompiledStep::Draw`, formula → new `CompiledStep::DrawFn`, evaluated at resolution, clamped at 0). Driver EX7-013 "draw until you have 6 in your hand" (`max: [ { subtract: [6, hand-count] }, 0 ]`, with DCGO's `if deck ≥ 1` guard authored as an `if`). `recover` keeps a literal-only count.
>
> **G-DSL-FORMULA-DISTINCT-NAMES-COUNT — RESOLVED 2026-09-13.** PerSelector `distinct_names_count: { of, zone, filter }` (`CompiledPerSelector::DistinctNamesCountScoped`): field permanents contribute synth-identity `card_names` (same normalisation as `distinct_named_count_gte`), other zones the printed names. Driver EX7-066 "for each of your [Three Musketeers] Digimon with different names, add 3000 to this DP-deletion effect's maximum" (`dp_lte: { base: 9000, per: {...}, delta: 3000 }`).
>
> **G-DSL-LINK-CARD-COUNT-FILTERED — RESOLVED 2026-09-13.** Permanent-subject predicate `link_card_count: { filter, at_least }` — the `Permanent.linked_cards` sibling of `source_count` (false on breeding permanents for `at_least ≥ 1`). Driver BT25-085's DCGO `DigivolutionOrLinkCards.Any(IsOption)` activation gate. Note (documented, not changed): `source_count` / `link_card_count` evaluate their nested filter with the card-leaf evaluator — no `any_of`/`all_of` inside; put the OR at the top level as two existential legs.

> **G-DSL-ON-USE-OPTION-TIMING — RESOLVED 2026-09-15** (three-musketeers-1 / slice tamers-options). `when: on_use_option` lowers to `EffectTiming::OnUseOption`, the board-wide "when a player uses an Option card" observer dispatched to every battle-area permanent of BOTH players; scope the actor with `active_when: { your_turn | all_turns }` (an `option_filter:` for "[TS] trait Option cards" is expressible as a `condition` once the used card is exposed on the trigger context — not yet needed). Driver BT3-096 Mimi Tachikawa (`cards/bt3/BT3-096.yaml`, `tests/cards_behavioral/bt3/bt3_096.rs`, 6). **Engine ordering fix shipped alongside:** `Game::use_option_from` used to enqueue `OnUseOption` BEFORE the Option's own `OptionMain` in one drain (bundling an observer with the body into a spurious `TriggerOrder`); it now ARMS `Game::on_use_option_armed` when the use cost is paid and `fire_on_use_option_observers` fires the observers AFTER the body has fully resolved — from the synchronous path or from `advance_pending_option` (selection-resume, including a body that claimed `pending_option`) — parking a still-present Option in the new `OptionResolutionPhase::OnUseOptionDrain`. Evidence: DCGO `UseOptionClass.UseOption` → `StackSkillInfos(OnUseOption)` (deferred stack) then `OptionSkill` inline; official BT3-096 Q&A "It can be activated after activating the used Option card's [Main] effect." BT25-091 Monica Simmons clause 3 (`[Your Turn] When you use [TS] trait Option cards …`) — **AUTHORED 2026-09-18** (`event_target_owner: you` + `event_card_trait_has: TS` over `TriggerSource::OptionUsed`, G-ENGINE-ON-USE-OPTION-EVENT-CARD, engine 35958972b); BT25-091 is IMPLEMENTED.
>
> **G-DSL-DELETE-ALL-PERMANENTS — RESOLVED 2026-09-15** (three-musketeers-1 / slice tamers-options). Step `delete_all_permanents: { over: <predicate> }` — scan BOTH battle areas ONCE, then `Game::delete_permanents_batch` (rule 25) so the whole set leaves the field simultaneously (DCGO `DestroyPermanentsClass(list).Destroy()`); effect-immune permanents skipped. Distinct from `for_each` + `delete_permanent` (sequential, observers see a sequence). Driver BT6-105 Gewalt Schwärmer "[Main] Delete all Digimon with play costs of 7 or less" (`none_of: [kind: token]` = DCGO `HasPlayCost`).

> **G-DSL-DELETE-PERMANENTS-BATCH — RESOLVED 2026-09-18** (three-musketeers-1 / exam triage EX7-071#effect#1). Step `delete_permanents: { targets: [<binding>, ...] }` — resolve several SEPARATELY-bound single-permanent bindings (unset = self-skipped arm, ignored; duplicates collapse; effect-immune skipped), then ONE `Game::delete_permanents_batch` (rule 25). For the printed "Delete 1 X, 1 Y, and 1 Z" shape: each pick is its own selection but the deletion is a single event (DCGO accumulates `selectedPermanents` then one `DestroyPermanentsClass(list).Destroy()` — `EX7_071.cs:170-252`; general_rule 15-4-3-2). Found by the DCGO exam: the previous per-arm `{select, delete_permanent}` authoring of EX7-071 Hurricane Screw Shot had already trashed the Lv.3 pick at the Lv.4 pick row. Tests: `cards_behavioral/ex7/ex7_071.rs` (`*_triple_delete_is_simultaneous_after_third_pick`). Other "1 of A and 1 of B" multi-arm deletes authored per-arm (e.g. EX4-073-style) should be re-audited against this step.
>
> **G-DSL-EFFECT-TRASHED-HAND-CARD — RESOLVED 2026-09-15** (three-musketeers-1 / slice tamers-options). Result predicates over the per-effect hand-trash log: `effect_trashed_any_hand_card` (bare bool) and `trashed_hand_card_matching: { <card-shape predicate> }` (evaluated as a `Card` subject against each card THIS effect trashed from a hand via `trash_from_hand_by_index` / hand-origin `trash_union_bound` — zone-agnostic, reads the card now in the trash). Sibling of `returned_card_matching`. Driver P-212 Asuna Shiroki "If this effect trashed a card with the [Three Musketeers] or [TS] trait, delete 1 of your opponent's level 3 Digimon."
>
...
>
> **Follow-up fix 2026-07-04 — second pick was silently mandatory.** `install_link_card_trash_second_selection` wired `on_decline` under `optional` but never flipped `pending.is_optional = true`; `resolve_generic_selection`'s `is_pass && !sel.is_optional` gate therefore rejected PASS on the which-link-card pick (DCGO BT25_073.cs:99 `canNoSelect: () => true` says it must be declinable). One-line installer fix mirroring the trash_option_from_own_stacks sibling; resume-path PASS arm was already correct. Regression tests: `optional_decline_on_second_pick_skips_trash_and_tail` (tests/dsl/trash_link_card_of_own_digimon.rs) + un-ignored `bt25_073_decline_second_pick_skips_trash_and_tail`.
...
> **RESOLVED 2026-09-19 (fb5517067)** — new per-selector `per: source_rules_color_count` (the effect carrier's synthesized rules colors, which include the AddColor treatment). BT8-084 authors the leg; BT8-084#effect#1 oracle re-diff CLEAN, CONFIRMED.
...
> **RESOLVED 2026-09-19 (fb5517067)** — collapsed into `self_color_count_gte` once the treatment landed; the permanent-subject leaf now reads only the synthesized colors (the delegated printed-card pass no longer re-rejects). BT8-084 authors `active_when: { your_turn: true, self_color_count_gte: 4 }` + `dp_modifier: 4000`.
...
> **RESOLVED 2026-09-19 (fb5517067, ENGINE FIX)** — `Permanent::synth_identity` reads `ModifierType::AddColor` additively after `ChangeBaseCardColor` (payload `None` = non-flipped digivolution-card colors; `Colors { value }` = listed colors). DSL surface: a self-aura `modifier: AddColor`. See docs/RUST_ENGINE_GAPS.md.
