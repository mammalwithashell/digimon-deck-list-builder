...
### §DigiXros material-consumption fires no leave trigger (G-DIGIXROS-DEPARTURE-LEAVE-TRIGGER — RESOLVED 2026-06-03) + redirect-extraction / unpayable-return-to-hand (G-DIGIXROS-REDIRECT-EXTRACTION + G-DIGIXROS-UNPAYABLE-RETURN-TO-HAND) — PARTIAL

- **RESOLVED 2026-06-03 — G-DIGIXROS-DEPARTURE-LEAVE-TRIGGER (judge-quiz Q25 PASS).** A DigiXros battle-area material consumption now fires the `WhenWouldLeaveBattleArea` replacement window. Implementation: added `ReplacementCause::DigiXros` (`replacement.rs`; threaded through the exhaustive `trigger_context.rs` `EventCause::from` map → `OwnEffect`, and `dsl_cards/lower_partition.rs` → `"digixros"`). `commit_play_from_hand_after_reductions` (`game_actions.rs`) now routes any pending DigiXros transaction through `run_digixros_leave_windows_then_commit`, which calls `try_replace(WhenWouldLeaveBattleArea, Permanent(material), ReplacementCause::DigiXros, None)` for each battle-area material BEFORE the host is committed. `DigiXros` is NOT `Battle`, so BT17-095's `active_when: { none_of: [replacement_cause: battle] }` passes and its `[All Turns]` `<Delay>` accept installs on a DigiXros departure. Pinned by `tests/judge_quiz/e_partition_digixros.rs::q25_all_turns_fires_on_digixros_departure_not_battle` (un-ignored, green). Regression-clean: `judge_quiz` (23 pass), `cards_behavioral` (3891 pass), `combat` (213 pass), `option_flow` (93 pass).
- **STILL OPEN — G-DIGIXROS-REDIRECT-EXTRACTION (blocks Q26/Q27).** When BT17-095's leave observer parks its optional `<Delay>` accept, the engine commits the DigiXros host eagerly (tucking the departing material UNDER the host) so the material stops being a standalone top card — what the Q25 *and* Q26 preconditions both require. But Q26/Q27 additionally need BT17-095's DNA-evo (driven later) to **extract** the departing material OUT of the in-flight transaction and INTO an Omnimon, then recompute cost → return-to-hand. The two requirements are in direct tension at the parked point: Q25 (no `auto_resolve`) wants the material already gone while the accept is parked; Q26 wants the parked accept's DNA-evo to still extract it. `resolve_stable_replacement_subject` resolves the subject to the permanent *containing* the card, so once the material is tucked under the host the DNA-evo grabs the host, not the material. Closing this needs a dedicated **"leaving / limbo" holding slot**: a battle-area-departing material that is (a) no longer any permanent's top card (so the precondition passes) and (b) still resolvable + extractable by the replacement process. That zone primitive does not exist yet.
- **NEARLY DONE — G-DIGIXROS-UNPAYABLE-RETURN-TO-HAND (the recompute itself).** The declare-then-pay recompute + return-to-hand + 0-memory machinery IS implemented in `finalize_digixros_play_after_leave_windows` (`game_actions.rs`): after the leave windows resolve it prunes battle-area materials that left the field (`DigiXrosTransaction::retain_materials`, fixing both `final_cost()` and the material take), recomputes `(final_cost - total_reduction).max(0)`, and — when `memory - cost < memory_range.0` — aborts the play leaving the card in hand and paying 0 memory. It just never runs on the **parked** path today, because the host is committed eagerly on park (the G-DIGIXROS-REDIRECT-EXTRACTION blocker above). Once a material can be DNA-extracted from the in-flight transaction, this branch closes Q26/Q27 with no further work.
...
- **EX3-014 / AD1-004 / BT17-095 now all implemented (2026-06-03).** The judge-quiz Q25/Q26/Q27 scenarios are now **staged-and-pinned** (discover-then-pin) rather than empty `BLOCKED-CARD` stubs:
  ...
  - `tests/judge_quiz/c_declare_then_pay.rs::q26_dorbickmon_returns_to_hand_when_cost_unpayable_after_dna_evo` — **PARTIAL**, re-`#[ignore]`d on G-DIGIXROS-REDIRECT-EXTRACTION. On the new substrate the leave window fires, but the host commits eagerly on the parked accept, so the DNA-evo can't extract WarGreymon; Dorbickmon enters the field instead of returning to hand.
  - `tests/judge_quiz/c_declare_then_pay.rs::q27_dorbickmon_pays_zero_memory_when_returned_to_hand` — **PARTIAL**, same residual: the reduced cost is paid (10→7) on the eager commit instead of 0.
- **Blocks (judge-quiz):** Q25 — RESOLVED (PASS). Q26, Q27 — PARTIAL (BLOCKED-ENGINE on G-DIGIXROS-REDIRECT-EXTRACTION; the recompute/return-to-hand branch already exists, see above).
...
### §`[On Deletion]` resolves nested inside the deleting effect, bypassing the topmost-card remain-in-trash gate (G-ON-DELETION-RESOLVES-MID-EFFECT) — OPEN
