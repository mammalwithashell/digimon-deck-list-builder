...
- **Calibration spike (2026-05-29, change `fix-judge-quiz-engine-gaps` task 3.1) — decision: SPLIT to a follow-up change.** Read EX10-036 + `ex10_036_clause_a_after_source_trash_prompts_opp_field_delete`. The synchronous `drain_effect_queue()` in `fire_digivolution_card_trashed` is load-bearing for a reason ORTHOGONAL to the on-trash observer: when Clause A's cost trashes its first source, the synchronous drain lets **Clause B — a SEPARATE already-queued WhenDigivolving effect of the same card** — fire and pick up the just-trashed cards (~20 EX10-036 tests cover this). Q23's fix needs the inherited `OnDigivolutionCardTrashed` observer to DEFER past a *later separate effect* (Medusamon's return-2) and re-check remain-in-trash — and a remain-in-trash gate ALONE does not fix Q23 (all 3 Tumblemon are still in trash at synchronous-fire time, so the observer must genuinely defer). Cleanly separating "drain already-queued sibling effects (synchronous)" from "inherited on-trash observer (deferred + re-check)" is a deep change to the trash-observer dispatch/provenance model, risks EX10-036's tests, and **cannot be validated without Q23's cards** (Proganomon EX8-051 / Medusamon BT24-017 full chain). Deferred to its own follow-up change; stays OPEN.
