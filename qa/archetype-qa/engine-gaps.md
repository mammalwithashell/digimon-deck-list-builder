...
### §Queued-effect body inline-drains its own triggers (G-BODY-RAISED-TRIGGER-INLINE-DRAIN) — RESOLVED 2026-09-19 (c652676d1)

- **RESOLVED 2026-09-19 (c652676d1) — ENGINE FIX, flagged for human review.** Exam `EX4-074#effect#1` (`qa/dcgo-exams/EX4/EX4-074-effect1.yaml`, preserved sidecar `20260918T132154Z_d8f0816fb3ee46f2a9e9c8adb4cf33bc.state.jsonl`) diverged at step 28: ShineGreymon: Ruin Mode (EX4-074)'s `[End of Attack]` deletes itself first, and its own `[On Deletion]` (-5000 to all opposing Digimon) resolved BEFORE the body's opposing-Digimon pick (ours 3000/7000, DCGO 8000/12000). general_rule.pdf 15-8-3-2 (p.25): "Trigger-type effects can't activate during the processing for a rule or effect." The single-trigger path in `drain_effect_queue_inner` (`effect_queue.rs`) only held a deferred-drain scope around `OptionMain` bodies (P-170, 5108e58ee); every other queued body let the deletion batch's `exit_deferred_drain_and_flush` inline-drain at counter 0. Fix: the scope (enter + manual decrement, no flush) now wraps every single-trigger `run_queued_effect`. A parking body leaves the scope synchronously; its resume runs in `resolve_generic_selection`'s own scope, so no counter leak. Re-diff CLEAN 31/31, verdict CONFIRMED. cards_behavioral 8195/0; lib/combat/judge_quiz/option_flow/selection green; the 8 `archetypes` + 3 `effect_context` failures pre-exist identically on HEAD. Not changed: the security-bundle path and TriggerOrder-resolved bodies (already inside `resolve_generic_selection`'s scope).
...
### ~~Activated-Digivolve Alt-Path Has No Engine Execution Route~~  [G-ACTIVATED-DIGIVOLVE-EXECUTION] — RESOLVED 2026-06-16
- **Status:** RESOLVED 2026-06-16. Every affected card was re-modelled off the unreachable `kind: activated_digivolve` alt-path onto working machinery with **zero engine code**: BT24-016 Lamiamon (2026-05-22), then the residual BT22-013/026 → `when: main_from_hand` clauses (the `[Hand][Main]` Nokia-gated cost-6 [Agumon] jump, mirroring BT24-016 clause 1) and BT16-027 → a static `digivolve` add-source (DNA Omnimon gap-closure Tasks A1–A3). The action layer already masks a Hand `[Main]` action for any hand card with a `MainFromHand` effect whose `condition:` passes, so the re-modelled jumps are playable and behaviorally driveable — the `CompiledAltPathKind::ActivatedDigivolve` execution route is no longer needed by any card. The named DNA Omnimon combo E jump is now driven through the real effect (`tests/archetypes/dna_omnimon.rs::combo_e_nokia_cost6_lv6_jump`, formerly `#[ignore]`'d); per-card coverage in `tests/cards_behavioral/bt22/bt22_013.rs`. Full entry archived in `qa/resolved-gaps.md`.
...
### Outer-Tail Slot Theft on Effect-Driven Option USE  [G-DSL-OUTER-TAIL-USED-OPTION-THEFT] — FIXED for `use_option_from_revealed` 2026-07-04; siblings LATENT
- **Discovered in:** EX7-048 Gundramon end-to-end payoff test (pick the real EX7-070 Der Blitz from the reveal, use it free), 2026-07-04.
- **Scope:** Rust engine — a NON-PANICKING sibling of `G-DSL-OUTER-TAIL-NESTED-PARK` above. No invariant trips; the game silently resolves in the wrong order with wrong provenance.
- **Shape:** an outer DSL clause runs `use_option_from_revealed` mid-tail (itself running inside a selection callback via `run_tail_preserving_trigger_context`, e.g. a `select_reveal_buckets` terminal). The used Option's `[Main]` body parks MID-BODY (e.g. inside an `if` with a second `if` leg still to run — EX7-070's delete-then-place-self shape). The body's remaining steps land in the single global `Game::dsl_outer_tail` slot with the OPTION as their rightful provenance. Control unwinds to the OUTER scope, whose `run_tail_preserving_trigger_context` (selections.rs `drain_dsl_outer_tail_with_bindings` call) drains the slot as if it were its own: the Option's remaining body steps get wrapped onto the pending selection under the OUTER card's `source_card` and sequenced AFTER the outer clause's own tail.
- **Observed corruption (EX7-048 + EX7-070):** (1) ordering — Gundramon's mandatory "return the rest to the top or bottom of the deck" resolved in the MIDDLE of Der Blitz's `[Main]`; (2) provenance — Der Blitz's `place_self_under_permanent` ran with `source_card` = Gundramon, so its `pending_option` claim failed, the placement silently no-oped, and `advance_pending_option` mis-disposed Der Blitz to trash instead of seating it as a digivolution source.
- **Fix (2026-07-04, this change):** `compose_parked_used_option_body_tail` in `code/digimon-engine/src/dsl_cards/step/play_digivolve.rs` — immediately after `use_option_from` parks, the executor re-composes the `dsl_outer_tail` slot onto the parked selection under the OPTION's own provenance (fresh `EffectContext` with the option card as `source_card`), BEFORE the outer `run_steps` loop parks the outer clause's tail. Frame `outer_conts` then hold [option-body-tail, outer-tail] in the correct order.
- **Regression tests:** `code/digimon-engine/tests/dsl/option_source_use_cluster.rs::dsl_use_option_from_revealed_mid_body_park_resolves_before_outer_tail` (DSL-level, synthetic mid-park option) and `code/digimon-engine/tests/cards_behavioral/ex7/ex7_048.rs::ex7_048_on_play_pick_uses_tm_option_free_end_to_end` (real-card end-to-end).
- **LATENT siblings (no driver card yet, not fixed):** `use_option_from_sources` / `use_option_bound` executors (same file, same shape — a mid-body-parking Option used from a digivolution stack or hand/trash union would hit the identical theft), and the from-hand bucket terminal `run_reveal_bucket_use_option_terminal` (selections.rs ~2095). Apply the same `compose_parked_used_option_body_tail` composition when a driver card surfaces.
