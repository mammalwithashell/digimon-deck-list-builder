...
- Production YAML found under `code/digimon-engine/cards/**`: `BT1-090`, `BT14-001`, `BT16-082`, `BT21-013`, `BT21-072`, `BT8-097`, `EX8-074`, `P-035`.
- Example-only YAML also exists for `_examples/BT14-009` and `_examples/BT18-102`; those are not production set specs.
- High-frequency Red Hybrid cards without production YAML include `BT17-012`, `BT17-009`, `BT18-088`, `BT21-082`, `BT18-011`, `BT7-014`, `BT17-017`, `BT7-008`, `BT17-011`, `BT17-079`, `BT12-009`, `BT12-088`, `BT17-094`, `BT12-017`, `BT4-113`, `BT21-020`, `BT6-010`, `BT7-085`, `BT18-010`, and `EX1-066`.
...
| `BT17-009` Flamemon | Searcher / recursion source | Reveal top 3, add one Hybrid/Ten Warriors and one inherited-effect Tamer, bottom rest; inherited free Tamer play on deletion. |
...
- **Type:** `engine-gap`, `dsl-gap`
- **Tracker:** `docs/RUST_ENGINE_GAPS.md`; `qa/dsl-vocab-gaps.md`
- **Blocks Red Hybrid cards:** `BT17-009`, `BT17-017`, `BT21-020`, `BT21-082`, `BT17-094`, `AD1-002`, `AD1-020`, `BT12-012`, `BT12-017`, `BT7-008`
...
- **Type:** `dsl-gap`, possibly `engine-gap` for action-surface details
- **Tracker:** `qa/dsl-vocab-gaps.md`; `docs/RUST_ENGINE_GAPS.md`
- **Blocks Red Hybrid cards:** `BT17-009`, `BT18-010`, `BT7-008`, `BT16-082`, `P-035`, `LM-033`, `EX1-066`
...
- **Missing capability:** Existing reveal/select support handles simple one-card picks, but Red Hybrid needs reusable multi-slot picks from one reveal pool with separate predicates, duplicate prevention, and ordered/deck-bottom remainder disposition.
...
- **First test:** `BT17-009` reveals two Hybrid cards, one inherited-effect Tamer, and one blank. The pending selections allow one Hybrid/Ten Warriors and one Tamer to be chosen, then bottom the unchosen cards in the selected order.
- **Implementation hint:** `code/digimon-engine/src/dsl_cards/step/selections.rs`, `zone_moves.rs`, `code/digimon-dsl/src/step.rs`, and ordered permutation support for reveal remainders.
...
4. **Reveal multi-pick and ordered remainder:** `RH-05`.
...
2. Follow with `RH-04` and `RH-05` because union-zone free play and multi-category reveal selections recur across many non-Hybrid archetypes.
