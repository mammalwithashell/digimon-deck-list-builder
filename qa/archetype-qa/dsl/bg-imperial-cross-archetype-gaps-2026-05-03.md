...
- **Status update (2026-05-08):** The basic `dna_origin: true` predicate is now live for `WhenDigivolving` / `OnDnaDigivolve` drains created by DNA digivolution, and the origin bit is preserved through parked DSL continuations. BT20-060 proves a DNA-only rider after a target-selection step. Proof: `cargo test --manifest-path code/digimon-engine/Cargo.toml --test cards_behavioral -- bt20_060_dna_origin_trashes_security_and_recovers`.
- **Remaining capability:** Stable event payloads exposing DNA materials, resulting permanent, and resulting card properties to triggered effects and inherited/global observers are still open.
- **Why it matters:** Card authoring needs proven runtime event context so DNA-only riders do not fire on normal digivolution or fail to fire after DNA. The basic origin predicate is proven; material/result payloads remain future work.
...
- **Type:** hybrid `engine-gap` / `dsl-gap`
- **Status:** resolved for BT20-016 as of 2026-05-08.
- **Formerly blocked:** `BT20-016` and may affect future BG Imperial tech; not in the two assessed BG Imperial lists, but appears in Imperialdramon-adjacent authored YAML.
...
- **Implemented capability:** `may_attack_now` installs a pending attack selection from effect resolution that uses normal attack legality/mask machinery while preserving the effect-specific attacker and optional PASS.
- **Why it matters:** Omitting the attack is a fidelity loss; auto-attacking hides a player-visible choice.
- **Evidence:** `cargo test --manifest-path code/digimon-engine/Cargo.toml --test cards_behavioral -- bt20_016_after_when_digivolving_this_digimon_may_attack`.
