...
- **Partial closure 2026-05-19 (Phase 2 Track J Task S1.2):** The **count-capped / name-unique source multi-select + batch play** sub-primitive is resolved for battle-area carriers. A new `select_materials` DSL step picks up to N digivolution sources of a carrier permanent in ONE count-capped multi-pick with `uniqueness: name` ("1 of each different name"); it lowers to `EffectContext::select_count_capped_multi` + `CountCappedZone::Material` + `DistinctByMode` (REUSING the existing count-capped action mask — no `ACTION_SPACE_SIZE` change). `play_from_materials` now consumes the bound `CardList` as a batch, each picked source becoming a fresh permanent, composing with `suppress_on_play`. Proven by `cargo test --manifest-path code/digimon-engine/Cargo.toml --features dsl-yaml-loader --test dsl -- select_materials` (count-cap, name-uniqueness mask, batch play, suppress_on_play composition, DSL lowering). **Remaining open work for this gap:** (a) the same multi-pick from a **breeding-resident** carrier (King Drasil's own stack) — `select_materials` accepts a `BREEDING_TARGET` carrier binding, but the source-select action range (14 battle-area field slots) has no encoding for a breeding-area carrier's sources, so a breeding carrier currently resolves to zero candidates; (b) the Rush grant on the played Digimon.
...
- **Blocks:** `BT13-112`, `BT13-110`, `BT13-019`, `EX11-053`, `BT20-083`, `BT23-072` — all still blocked on the breeding-carrier source-select encoding (their carriers are King Drasil, breeding-resident) and/or the Rush grant; the reusable count-capped / name-unique substrate itself is no longer the blocker.
...
- **Evidence:** `BT20-083` example YAML uses `select_material` and `play_from_materials`. The `suppress_on_play` flag closure is proven by `cargo test --manifest-path code/digimon-engine/Cargo.toml --test cards_behavioral -- bt5_106`; the count-capped / name-unique multi-pick + batch play is proven by `cargo test --manifest-path code/digimon-engine/Cargo.toml --features dsl-yaml-loader --test dsl -- select_materials`.
- **First test:** Give King Drasil multiple Royal Knight sources with duplicate and distinct names, resolve `BT13-112`, and assert the player can choose at most one per name, selected cards enter battle, On Play effects are suppressed, King Drasil is trashed, and all played Digimon gain Rush. *(S1.2 proves the count-cap + name-uniqueness + batch-play + suppress slice with a synthetic battle-area carrier in `tests/dsl/select_materials.rs`; the full BT13-112 card remains card-authoring work pending the breeding-carrier source-select encoding.)*
- **Implementation hint:** A generic count-capped source selection with uniqueness predicates now exists (`select_materials`). It works for battle-area carriers today; extending it to breeding carriers needs a source-select action encoding for the `BREEDING_TARGET` field slot (or an `ACTION_SPACE_SIZE` increase).
