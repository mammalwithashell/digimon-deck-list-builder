...
| BT7-056#effect#0 | DCGO quirk | `general_rule.pdf` §15-15-10-1 (p.31) / §15-1-2 / §15-1-4 (p.22); `RevealLibrary.cs:291-329`, `SelectCardEffect.cs:813-815` | Same family (Dorumon's two-bucket reveal). **Re-triaged from scratch at close-out (2026-09-21)** in the full order of evidence (printed text → PDF read verbatim → DCGO C#) and re-measured `--all-diffs` against the preserved sidecar `20260921T042630Z_14e62ea8`: step 7 `p0.hand ours=[BT24-088 x3, BT25-092 x2] dcgo=[…, EX7-073]` is the LEAD and the ONLY row, nothing downstream; `--sim-only` passes all 7 end-state checks. Not a slot artefact (no `field.N`; P0 holds one permanent). Restored as the **original** driver of `G-EXAM-REVEAL-BUCKET-ADD-TIMING`, from which an editing slip had dropped it. No engine/DSL/card change. `NOTES-BT7-056.md`. |
