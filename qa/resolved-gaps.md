...
## The exam's replay driver auto-answered an engine-only prompt a scenario had already written a row for, so that row ate the NEXT decision (G-TOOLING-EXAM-AUTO-ANSWER-EATS-NEXT-PROMPT) — RESOLVED 2026-09-21

- **Severity (at close):** 🔴 measurement integrity, not gameplay. The engine was never wrong; the harness measured the wrong state and two clauses carried a `confirmed` verdict that the current engine could no longer reproduce. A false `confirmed` is worse than a `diverged`: it reads as verified when nothing verifies it.
- **Discovered in:** re-triage of the suspected-false `BT25-091#effect#2` verdict flagged in `qa/dcgo-exams/BT25/NOTES-BT25-091.md`. The note recorded a one-field, one-row diff — `p0.field[0].suspended: ours=true dcgo=false` at the `select: { yes: true }` row — and left OPEN whether it was a harness snapshot-boundary property (fix the differ) or a real ordering divergence (flip the verdict). It was **neither**.
- **Root cause:** `ReplayDriver::auto_answer_option_trash_order` (`code/digimon-engine/src/runners/replay.rs`) runs at the top of EVERY `step()` and answers our engine-only 9-1-5 trash-order prompt with entry 0 ("trash now"). That is correct for the DCGO corpus — DCGO makes no such decision, so no recording can carry an answer. It is wrong for an exam scenario, where the author writes that decision explicitly as a `sim_only:` row. Both fired, so the scenario's own row landed on whatever our engine parked NEXT and spent it, and every later row was shifted one decision forward. The lowering pass (`ScenarioAdapter::advance_through_selection`) never auto-answered, so lowering and replay disagreed about what each row meant — exactly the divergence that adapter's own doc comment says a second resolution path must not create.
- **What it actually ate, on the reported driver:** Monica Simmons' (BT25-091) `[Your Turn] … **by suspending this Tamer**, …` decline gate. With the gate consumed, the suspend cost auto-paid and the `select: { yes: true }` row written for the gate answered the target pick instead, so our snapshot at that row already had Monica suspended where DCGO's had her unsuspended. The `suspended` field was a symptom; the cause was a spent decision.
- **Resolution:** new `RecordingSource::answers_engine_only_prompts()` (default `false`; `ScenarioAdapter` overrides it to `true`), and `auto_answer_option_trash_order` returns immediately for such a source. DCGO-corpus replay (`dcgo-replay`, the engine MCP) is unchanged; an exam scenario now owns every one of our prompts, which is what the exam is for (rule 17 — no auto-selections).
- **Citations:** the decision that was eaten is a rules-mandated one. `general_rule.pdf` (Ver.3.6) **§15-7-1** (p.24) — optional processing conditions "include text such as 'by X, Y'"; **§15-7-4** (p.24) — "A player can choose whether or not to execute the content of optional processing conditions." DCGO agrees and asks it: `DCGO/Assets/Scripts/CardEffect/BT25/Black/BT25_091.cs:104` — `activateClass.SetUpActivateClass(CanActivateCondition, ActivateCoroutine, -1, true, …)`, the `isOptional: true` arm that produces the `OptionalSkill` yes/no row, whose body then runs `SuspendPermanentsClass` before `SelectPermanentEffect` (`BT25_091.cs:136-144`). The prompt the driver was auto-answering is licensed by **9-1-5** (p.19) + **18-1-2** (p.40) → **15-4-3-5-1** (p.23) — see `G-ENGINE-OPTION-TRASH-TURN-PLAYER-ORDER` in `docs/RUST_ENGINE_GAPS.md`.
- **Blast radius (measured, not assumed):** an instrumented sweep of all **369** exam scenarios found exactly **two** that reach the engine-only trash-order prompt — `qa/dcgo-exams/BT25/BT25-091-effect2.yaml` and `qa/dcgo-exams/BT3/BT3-096-effect0.yaml`. Both were stored `confirmed`; both re-diff **DIVERGED** against their preserved sidecars on the pre-fix harness (`BT25-091#effect#2`: `p0.field[0].suspended ours=true dcgo=false`; `BT3-096#effect#0`: `memory ours=2 dcgo=1` — Mimi's memory gain already applied because her cost gate was eaten the same way) and **CLEAN** on the fixed one. BT3-096 was the silent second instance the BT25-091 note warned might exist. A before/after sim-only sweep of all 369 scenarios is byte-identical, so nothing else moved.
- **Gate / coverage:** `code/tools/dcgo-harness/tests/exam_engine_only_prompt_not_auto_answered.rs` — asserts Monica is UNSUSPENDED at the row that answers her §15-7-4 gate and suspended only at the following target pick. FAILS before the fix (with the reported `suspended` symptom), passes after. Suites: `dcgo-harness` 362 + 24 + 1 + 3 = 390/0; `digimon-engine --lib runners::` 54/0; `dcgo-replay` 13/0; full `cards_behavioral` **8227 passed / 0 failed / 37 ignored** (279.8s, `RUST_MIN_STACK=268435456 … --test-threads=8`). Oracle re-measures, both against preserved sidecars (zero Unity time): `BT25-091#effect#2` CLEAN (compared 12 of 13 ours / 12 dcgo, 1 sim-only row) → `confirmed` re-recorded; `BT3-096#effect#0` CLEAN (compared 8 of 9 ours / 8 dcgo, 1 sim-only row) → `confirmed` re-recorded.
- **Lesson:** a harness that answers a prompt on the engine's behalf must be able to say WHOSE answer it is. The old code could not, so two independent answers to one question looked like one answer to two.

## A from-hand Plug-In Option link paid its cost at the declaration instead of after the host pick (G-ENGINE-OPTION-HAND-LINK-COST-TIMING) — RESOLVED 2026-09-21

- **Severity (at close):** 🟠 faithfulness bug on the §6-5-1-4 from-hand link declaration — the link cost left the memory gauge, and the card left the hand, one decision too early. End state converged, so it was invisible at rest; it is observable to anything that reads memory or the hand between the declaration and the host pick (a trigger or replacement raised on the link, a cost modifier reading the gauge, any prompt listing hand cards) and to the RL observation tensor at that decision point.
- **Discovered in:** Three Musketeers exam close-out (job three-musketeers-2), 2026-09-21 — the oracle diff for `BT25-093#effect#4` (Ignition Flare, `<Link> [TS] trait: Cost 3`), sidecar `20260921T042530Z_8f13b83c…`, reported exactly ONE divergence row at the host-pick prompt: `memory: ours=0 dcgo=3`, `p0.hand` missing BT25-093 on our side. `BT25-100#effect#5` (`ours=1 dcgo=3`) and `BT24-091#effect#4` showed the identical single row, which is what made it a property of the path rather than of any one card.
- **Card(s):** every Plug-In Option declared from the HAND — the eight `kind: link_requirement` cards in the pool (BT24-091, BT24-095, BT24-097, BT25-093, BT25-100, BT25-101, ST22-08, ST22-11). The Digimon-side (Shape-B) hand link was never affected: `activate_hand_link`'s second branch and `activate_field_link` already install the host selection first and pay inside `begin_digimon_link`.
- **Root cause:** `Game::activate_hand_link`'s Option branch (`src/game_actions/link.rs`) went straight to `play_option_core(.., OptionPlayMode::Link { cost }, OptionCostPolicy::Pay)`. That runs the whole Option lifecycle in one go — step 4 pays the link cost, step 5 removes the card from the source zone into `pending_option` — and the host question is asked only much later, from `dispose_option`'s `Link` arm. So the entire §10-1-3 procedure ran back to front.
- **Resolution:** the declaration now installs the host prompt and pays nothing. `install_option_hand_link_host_selection` parks a `ResumeFrame::OptionHandLinkHostSelection` data frame (clone-safe, rule 28) whose prompt is byte-identical to `install_link_host_selection`'s — same `SelectionKind::OwnField`, same attack-id encoding, same wording — so the wire meaning of the pick is unchanged and only its position in the procedure moved. The resolved pick runs `finish_option_hand_link`: it re-validates that the hand slot still holds the declared card (a stale slot aborts without paying, which §9-1-8 licenses — "the memory doesn't move when a card can't be used because its cost can't be paid"), pins the host on `Game::pending_option_link_host`, and re-enters `play_option_core` with the same `(source, mode, cost_policy)`. `dispose_option`'s `Link` arm consumes the pin instead of asking a second time for one choice; the pin is re-checked against the live candidate set and falls back to the ordinary prompt if the link-mode body moved the board.
- **Citations:** `general_rule.pdf` (Ver.3.6) **§10-1-3** (p.19) fixes the order — **10-1-3-1** "The player declares a link and reveals 1 card to link. 1 link requirement is chosen on the revealed card, then the player chooses 1 of their Digimon that meets the requirement." → **10-1-3-2** "The specified link cost is paid." → **10-1-3-3** "The card to link is plugged in sideways into the chosen Digimon, and the link procedure is resolved." Payment is step 2 of 3. **§9-1-8** (p.19) adds that revealing a card as part of a use procedure "isn't considered removal from an area", which is why the declared card is still in hand at the host prompt. DCGO matches the PDF: `DCGO/Assets/Scripts/Script/CardEffectFactory/KeyWordEffects/Link.cs:64-101` — `ActivateCoroutine` runs the `SelectPermanentEffect` "Select 1 Digimon to link." prompt first and only then hands the chosen permanent to `ILinkCard.LinkCard()`, which pays (`AddMemory(-1 * Cost)`, `DCGO/Assets/Scripts/Script/CardController.cs:3758-3762`) and then attaches (`AddLinkCard`, `CardController.cs:3768`).
- **Gate / coverage:** `tests/cards_behavioral/bt25/bt25_093.rs` — `bt25_093_hand_link_pays_after_the_host_is_chosen_not_at_declaration` (FAILS on the parent commit: memory 7 vs 10 at the host prompt). Two ST22-08 cases encoded the old pay-at-declaration contract and now assert the §10-1-3 order: `st22_08_link_mode_charges_link_cost_2`, `st22_08_link_action_stays_affordable_when_the_use_action_is_not`. Suites: full `cards_behavioral` **8223 passed / 0 failed / 37 ignored**; `--lib` 338/0; `option_color_requirements` 6/0, `live_game_action_validation` 7/0, `policies_greedy` 5/0, `policies_headless` 1/0, `debug_runner_dsl` 7/0, `alt_path_reachability` 1/0. Oracle: `BT25-093#effect#4` re-diffed against its preserved sidecar — **CLEAN** (compared 9 of 10 ours / 10 dcgo), verdict re-recorded `confirmed`.
- **Commit:** `9af21698c`.
- **Follow-up — DONE (2026-09-21, three-musketeers-2 close-out).** Both sibling drivers have been re-diffed against their own preserved sidecars and flipped `diverged` → `confirmed` with no further code change: `BT25-100#effect#5` (sidecar `20260921T042552Z_0bad16abd…`) and `BT24-091#effect#4` (Tidal Stream, sidecar `20260921T042039Z_c3e0a78e6c254e31b2ae6721e408f494.state.jsonl`, **CLEAN — compared 13 of 14 ours / 14 dcgo steps, 1 sim-only row + 1 DCGO intermediate row not comparable**; clause-text sha unchanged at `9d4716573dac…`, so the flip is a genuine re-measure, not drift). BT24-091 is now 6/6 confirmed. No `diverged` clause remains on the from-hand Plug-In Option link path. `G-TOOLING-EXAM-OPTION-HAND-LINK` (the older "no shared wire action for a hand plug-in" blocker) was already closed by `7e30343d4`.

## A condition-false turn-scheduled `<Delay>` trashed its carrier anyway (residual of G-ENGINE-DELAY-BODY-BEFORE-TRASH) — RESOLVED 2026-09-20

- **Severity (at close):** 🟠 faithfulness bug on the shared scheduled-`<Delay>` scan — a Delay Option whose printed gate happened to be false at its window was trashed for free, taking every later window with it.
- **Discovered in:** the `G-ENGINE-DELAY-BODY-BEFORE-TRASH` fix (`70694334d`, 2026-09-19) surfaced it and logged it as a residual in `6c4be5885`; carried into the Three Musketeers close-out open-findings table. Closed by the three-musketeers-2 close-out (2026-09-20).
- **Card(s):** every turn-scheduled `<Delay>` whose clause carries an `active_when` — the `start_of_your_turn` / `end_of_your_next_turn` family (LM-027/029/030/031/032/034, BT15-096, BT21-093, BT21-097, BT22-099, BT24-100, P-206, LM-055). **LM-032 Purple Scramble** is the worked case: "[Start of Your Turn] **If your opponent has a Digimon**, ＜Delay＞".
- **Root cause:** `Game::resolve_delayed_options_matching` (`src/game_phases.rs`) drove every matured carrier through `enqueue_triggered(DelayEffect)` + `drain_effect_queue` and then deleted it with `ReplacementCause::Cost`. After `70694334d` the body pays that trash itself, so the post-body delete normally re-finds nothing — but a clause-level condition that is currently FALSE is dropped by the queue's own condition filter (`non_firing_queued_effect_indices_for` / `run_queued_effect_inner`), so no body ran, nothing paid, and the vestigial delete fired on a Delay that never activated. The §16-16-2 DECLINE path already had the right shape (carrier stays, window rescheduled); the condition-false path had no path at all.
- **Resolution:** the scan now probes the carrier BEFORE enqueueing, via a new `Game::scheduled_delay_is_activatable(handle, triggers)`: it looks up the carrier's top card's effects, keeps the `DelayEffect` clauses whose `delay_trigger` matches this scan, and evaluates their clause conditions. When **every** matching clause's condition is currently false the scan leaves the carrier in the battle area, calls `reschedule_delayed_option(key)` and skips to the sibling Options. `reschedule_delayed_option` is the body of the old `take_declined_delay_reschedule` extracted so both "the window closed without activating" paths share one reschedule (`compute_delay_trash_turn` → the controller's next window; the scan matches on `trash_on_turn == turn`, so a carrier left at its spent turn number would never be offered again). The probe returns `true` — unchanged legacy behaviour — in every ambiguous case: carrier gone or zombie, effects not in the registry, a clause with no condition, an unconditional sibling clause, or no matching `DelayEffect` at all. It can therefore only SUPPRESS a trash the rules do not license, never introduce one. Marker-only Delays (empty `process`, ST23-15 / ST24-15) carry no condition and are untouched — `st23_15_marker_delay_does_not_trash_it_at_end_of_next_turn` still pins their survival.
- **Citations:** `general_rule.pdf` (Ver.3.6) **§16-16-1** (p.35) — "＜Delay＞ is a keyword effect. While a card with this effect is in the battle area, **by trashing that card**, the effect specified in ＜Delay＞ will activate" (the trash is the activation, so no activation means no trash, and the card stays in the battle area where §16-16-1 keeps the Delay available); **§16-16-2** — "The processing from ＜Delay＞ is optional"; **§16-16-3** — the same-turn lockout, which is likewise a gate on the window rather than on the card's presence. DCGO: `DCGO/Assets/Scripts/CardEffect/LM/Purple/LM_032.cs:137-143` — `CanUseCondition` = `IsOwnerTurn && IsExistOnBattleArea(card) && card.Owner.Enemy.GetBattleAreaDigimons().Count >= 1 && CanDeclareOptionDelayEffect(card)`; a false `CanUseCondition` never reaches `ActivateCoroutine`, whose `DeletePeremanentAndProcessAccordingToResult` is the only thing that trashes the card (`LM_032.cs:146-158`).
- **Gate / coverage:** `tests/cards_behavioral/lm/lm_032.rs` — `lm_032_condition_false_leaves_the_delay_carrier_in_the_battle_area` (carrier stays out of the trash) and `lm_032_condition_false_reschedules_the_delay_to_the_next_window` (the survivor is re-offered at the next [Start of Your Turn] once the opponent commits a Digimon). Both FAIL on the parent commit. Suites: full `cards_behavioral` **8214 passed / 0 failed / 37 ignored**; `dsl` 926/0; `dsl_eval_arm_coverage` 8/0 (`timing_dispatch`'s single trash-observer failure reproduces on unmodified main). Oracle: all three LM-032 exam clauses re-diff **CLEAN** against their preserved DCGO sidecars — `LM-032#effect#0` 13/13, `LM-032#effect#1` (the `<Delay>` clause, which exercises the §16-16-2 decline + reschedule path at T7 and the accept at T9) 27/27, `LM-032#effect#2` 20/20.
- **Commit:** `5f091f5b2`.
- **Follow-up (logged, not fixed):** the scan's post-body `delete_permanent_with_cause` is now vestigial for every DSL `<Delay>` with a non-empty process, since `70694334d` made the body pay its own carrier trash. One arm is still unexamined: if a replacement window PREVENTS the body's trash (`DelayCostStatus::Unpaid` → the body aborts per §15-7-2), the scan re-finds the carrier and opens a second delete with a fresh replacement window, and on a cancel it only adds the key to `cancelled_keys` without rescheduling. No printed card in the pool was shown to reach this, and no oracle line covers it, so it stays an observation rather than a fix.

## A dual-mode Plug-In Option offered its Link play mode with no legal host (F-ENGINE-PLUGIN-MODE-SELECT-WITHOUT-HOST) — RESOLVED 2026-09-20

- **Severity (at close):** 🔴 illegal action in the RL action space — the "Plug in via Link Requirements" branch was offered, and executed, with zero legal hosts: it paid the link cost and put the card in the trash linked to nothing.
- **Discovered in:** Three Musketeers exam stage, 2026-09-18, while authoring `qa/dcgo-exams/BT25/BT25-091-effect2.yaml` (Iron Slash BT25-100 played with only the Tamer Monica Simmons on P0's board). Logged in `qa/dcgo-exams/BT25/NOTES-BT25-091.md`. Closed by the three-musketeers-2 close-out (2026-09-20).
- **Card(s):** every dual-mode Plug-In Option — the eight `kind: link_requirement` cards in the pool (BT24-091, BT24-095, BT24-097, BT25-093, BT25-100, BT25-101, ST22-08, ST22-11). All eight also carry a `[Main]` body, so the Link mode was the only one that could be filtered out; no pure-Link Option exists yet.
- **Root cause:** `Game::option_legal_play_modes` (`src/game_actions/mod.rs`) filtered `classify_option_modes`' output on AFFORDABILITY only. Its own doc comment said so explicitly ("host availability for a Link play is resolved later by `dispose_option`"), while the shared helper `link_host_candidates` right above it already advertised itself as backing "the dual-mode legality check in `option_legal_play_modes`" — the check was never wired. With two modes surviving, `play_option_core` installed the mode-select; the Link branch then paid the cost, found no candidate in `dispose_option`, and trashed the card.
- **Resolution:** `option_legal_play_modes` now drops every `OptionPlayMode::Link` when `link_host_candidates(player, card.handle(), &effects)` is empty. The mask (`action/mask.rs`, via the same function) and the mode-select offer therefore agree, and a single surviving mode plays directly with no prompt. `dispose_option`'s "no candidate → trash" arm is kept as the resolution-time safety net for a host that disappears mid-resolution; it is no longer reachable from a player-declared Link play.
- **Citations:** `general_rule.pdf` (Ver.3.6) **§10-1-3-1** (p.19) — "The player declares a link and reveals 1 card to link. 1 link requirement is chosen on the revealed card, **then the player chooses 1 of their Digimon that meets the requirement**"; **§6-5-1-4** (p.12) names "Linking a Card in the Hand or Battle Area" as the main-phase action that procedure implements. With no Digimon meeting the requirement the procedure cannot be carried out, so the action cannot be declared. DCGO: `DCGO/Assets/Scripts/Script/CardEffectFactory/KeyWordEffects/Link.cs:24` — `LinkEffect` returns `null` outright when `!CardEffectCommons.HasMatchConditionPermanent(CanSelectPermanentCondition)`, re-checked in `CanUseCondition` at `Link.cs:53`; `BT25_100.cs:20-25` is the host predicate (`IsDigimon && HasTSTraits`). The engine already held this contract on the Digimon side: `Game::hand_digimon_link_available` (`src/game_actions/link.rs`) requires `!hosts.is_empty()`.
- **Gate / coverage:** `tests/cards_behavioral/bt25/bt25_100.rs` — `bt25_100_no_mode_select_when_no_link_host_exists` and `bt25_100_no_mode_select_when_only_non_ts_digimon_on_board` (both FAIL on the parent commit, pass after), plus the positive control `bt25_100_mode_select_is_offered_when_a_legal_host_exists`. `st22_08_link_mode_with_no_host_trashes` encoded the old contract and is rewritten as `st22_08_link_mode_not_offered_without_an_eligible_host`; two BT25-093 cases now resolve synchronously and assert `Trashed`. Suites: full `cards_behavioral` **8212 passed / 0 failed / 37 ignored**; `--lib` 329/0; `option_color_requirements` 6/0, `live_game_action_validation` 7/0, `policies_greedy` 5/0, `policies_headless` 1/0, `debug_runner_dsl` 7/0. All 21 BT24-091 / BT25-091 / BT25-093 / BT25-100 exam scenarios re-run `--sim-only`: 21 lowered, 0 failed.
- **Commit:** `f24b986d0`.
- **Follow-up:** the Option **hand** plug-in remains `unreachable` on the oracle (BT25-100#effect#5, BT25-093#effect#4): our Option plug-in lives on the PLAY bit behind the mode-select, DCGO's is a separate `ActivateCardAction` the harness emits only from a `HAND_EFFECT` bit, so no scripted line can mean "hand plug-in" on both engines. That is the offering/masking half's companion and is tracked in `qa/dcgo-exams/BT25/NOTES-BT25-100.md`.
...
- **Discovered in:** three-musketeers-1 gap-closure stage for BT25-092 (2026-09-17), while choosing the `[Main]` suspend-cost idiom. Closed by the three-musketeers-2 close-out (2026-09-20).
- **Card(s):** every `when: main_on_field` clause whose first body step is `activation_cost: {…}` — an exhaustive sweep of `code/digimon-engine/cards/**/*.yaml` finds exactly two, **BT25-089 Kazuki & Itsuki** (`suspend_self`) and **EX11-071 Cool Boy** (`return_self_to_deck_bottom`). The other 51 `activation_cost` clauses sit on queue-dispatched timings (`on_digivolve`, `on_any_link`, `start_of_your_turn`, …), where `effect_queue::run_queued_effect_inner` already consulted `activation_cost_fn` — which is why the drivers' own per-card tests passed: they drove the clause through `enqueue_triggered(MainOnField)` + `drain_effect_queue`, never through the decoder.
- **Root cause (both halves):**
  1. **Resolution.** `Game::activate_field_main` (`src/game_actions/play.rs`) — the FIELD_EFFECT `+2` sub-slot decoder target — checked `condition`, recorded the OPT activation, and then called `effect.process` directly. It never invoked `effect.activation_cost_fn`, documented in `src/effect.rs` as "consumed during triggered-ability resolution". Probed empirically at discovery time: `Game::activate_field_main(0, kazuki)` on BT25-089 → `fired=true, suspended_after_fire=false, pending=true`.
  2. **Mask.** `main_effect_select::field_main_match` had no activation-cost gate, so a suspended Tamer's `[Main]` bit stayed legal. The closure alone could not fix this: `ActivationCostFn` takes `&mut EffectContext`, so nothing could ask "could this be paid?" without paying it.
- **Resolution (engine, 2026-09-20):**
  - **`ActivationCostKind`** (`src/effect.rs`) — a plain `Copy` enum (`SuspendSelf` / `ReturnSelfToDeckBottom` / `TrashSelf`) carried as a DATA twin of `activation_cost_fn` on `Effect` (`activation_cost_kind: Option<ActivationCostKind>`). Data, not a second closure, so `Effect` stays clone-safe and declarative (rule 28). Its `is_payable(&Game, Option<PermanentHandle>)` probe mirrors each `EffectContext::*_as_cost` mutator's own refusal conditions exactly — `SuspendSelf` ⇒ permanent exists && `!is_suspended` && `!CannotSuspend` (the `suspend_self_as_cost` gate, DCGO `CanActivatePermanentSuspendCostEffect` → `!permanent.IsSuspended && permanent.CanSuspend`); `ReturnSelfToDeckBottom` / `TrashSelf` ⇒ permanent exists (their mutators refuse only a departed source).
  - **`EffectBuilder::activation_cost_of_kind(kind)`** sets the closure AND the kind together, so the mask's "can this be paid?" gate and the resolution path's "pay it" step can never drift. `dsl_cards/lower_triggered.rs` now lowers `CompiledActivationCostKind` through it; the raw `EffectBuilder::activation_cost` closure escape hatch remains for hand-written effects and simply carries no mask gate.
  - **`Game::activate_field_main`** runs `activation_cost_fn` between the recorded activation and the body `process`; on `false` it returns without running the body. Deliberately mirrors the queue's failure semantics (`effect_queue.rs`): body suppressed, **OPT slot stays consumed**, so a cost-impossible activation cannot be retried the same turn (Working Rule 17 — no silent retries).
  - **`main_effect_select::field_main_match`** suppresses the `[Main]` bit when `activation_cost_kind.is_payable(...)` is false — after the OPT, `condition` and `outer_optional_guard` gates. `explain_action` shares this function, so the label and the mask cannot disagree.
- **Citations:** `general_rule.pdf` (Ver.3.6) **§15-7-1** — "by X, Y" is an **optional processing condition**; **§15-7-2** — "If the content of the optional processing conditions isn't executed, the processing after the conditions can't be executed" (p.24). (§15-7-4/§15-7-5 let a player *choose* to attempt an impossible condition, but the result is a pure no-op with no game action, which is precisely what the mask must not offer the policy — DCGO agrees and does not offer it.) DCGO: `DCGO/Assets/Scripts/CardEffect/BT25/Green/BT25_089.cs:36` gates the `EffectTiming.OnDeclaration` ActivateClass's `CanUseCondition` on `CardEffectCommons.CanActivateSuspendCostEffect(card)` → `DCGO/Assets/Scripts/Script/CardEffectCommons/CanUseEffects/CanSuspend.cs:17-25`, and pays inside `ActivateCoroutine` via `SuspendPeremanentAndProcessAccordingToResult(..., SuccessProcess, null)` (BT25_089.cs:66-70) — no suspend, no link. `DCGO/Assets/Scripts/CardEffect/EX11/White/EX11_071.cs:108-116` wraps its body in `DeckBouncePeremanentAndProcessAccordingToResult(..., successProcess, failureProcess: null)` — no bounce, no play; its `CanUseCondition` needs no extra gate because a Tamer on the field can always be returned.
- **Gate / coverage:** `tests/cards_behavioral/bt25/bt25_089.rs` — `bt25_089_main_action_path_pays_the_suspend_cost` (the Tamer IS suspended after the decoder path), `bt25_089_main_action_masked_off_when_already_suspended` (mask bit 1.0 unsuspended → 0.0 suspended), `bt25_089_main_action_path_refuses_when_cost_unpayable` (no fire, no selection installed, hand untouched); `tests/cards_behavioral/ex11/ex11_071.rs` — `ex11_071_main_action_path_returns_self_to_deck_bottom` (deck +1, Tamer at deck index 0, gone from the battle area). All four FAIL on the parent commit and pass after. Suites: full `cards_behavioral` **8206 passed / 0 failed / 37 ignored**, `--lib` 329/0, `--test dsl` 926/0, `--test selection` 140/0, `--test option_flow` 79/0.
- **Commit:** `b96ebd4df`.
- **Follow-up (none blocking):** the other `[Main]` dispatchers (`activate_hand_main`, `activate_trash_main`, `activate_breeding_main_training`) likewise do not consult `activation_cost_fn`, but the same YAML sweep shows **zero** cards author `activation_cost` on `main_from_hand` / `main_from_trash` / `<Training>` today, so there is nothing to fix there yet. If a card ever does, mirror this pair of changes. BT25-092's `[Main]` keeps its `condition: { source_is_unsuspended }` + `suspend: { target: source }` authoring — that idiom is still correct and tested; `activation_cost` is now simply also available on `main_on_field`.

## Digivolve into a card from {hand|trash} onto a CHOSEN Digimon, with a {hand|digivolution-card} trash cost (G-DSL-DIGIVOLVE-FROM-UNION-WITH-SOURCE-TRASH-COST) — RESOLVED 2026-09-17

- **Severity (at close):** 🟡 DSL vocabulary gap — two predicate leaves; every step verb already existed.
- **Discovered in:** BT25 Three Musketeers tamers slice (2026-06); closed by the three-musketeers-1 campaign gap-closure stage (2026-09-17).
- **Card(s):** BT25-092 Asuna Shiroki clause 2 ("[Main] By suspending this Tamer and trashing 1 Option card from your hand or your Digimon's digivolution cards, 1 of your Digimon may digivolve into a Digimon card with [Three Musketeers] in its text or the [TS] trait in the hand or trash with the cost reduced by 1"). Generalizes to every "1 of your Digimon may digivolve into a … card in the hand or trash" clause whose target is a CHOSEN permanent rather than the effect's source (the `can_digivolve_from_source` shape — BT25-005 / P-196..198 — only covers "it/this Digimon may digivolve").
- **What was actually missing (re-audit):** the entry's two sub-gaps were both already lowerable — (1) a `select_union_zone{hand,trash}` binding resolves through `resolve_card_source_ref` to its true `CardSourceRef::Hand/Trash`, and `effect_initiated_digivolve { source: <binding> }` commits a trash-resident card (BT16-040 precedent); (2) `select_union_zone { zones: [hand, material] }` with `material_of` omitted scans every own Digimon's digivolution cards in ONE prompt (BT25-085 convention) and `trash_union_bound` trashes from the true origin. What no vocabulary expressed were DCGO's two gates in `BT25_092.cs` / `CardEffectCommons.DigivolveIntoHandOrTrashCard`: `ValidTarget(card, root, permanent)` = `CanPlayCardTargetFrame(selectedPermanent.PermanentFrame, …)` (the result pick must be routable onto the CHOSEN target) and `CanDigivolveDigimon(permanent)` (the target pick offers only Digimon with such a result in hand OR trash). Without them the target prompt would offer Digimon with no legal result and the result prompt cards the commit rejects — a mask/no-op mismatch the RL policy would loop on.
- **Resolution (DSL + engine evaluator, 2026-09-17):**
  - Card-subject leaf **`can_digivolve_onto: <binding>`** (`PredicateSpec.can_digivolve_onto: Option<String>` → `CompiledPredicate.can_digivolve_onto`); evaluator `can_card_digivolve_onto(rctx, card, target)` (shared with `can_digivolve_from_source`) = `Game::all_digivolve_routes_for_card(candidate, target).any(!app_fusion)` — printed circles, DSL alt-digivolve paths, `CanOnlyDigivolveInto` gate, App Fusion excluded: exactly what `effect_initiated_digivolve_from_source_inner` accepts. False when the binding is absent or no longer a permanent. Validator: undeclared binding → error (`validate_predicate_binding_scope`).
  - Permanent-subject leaf **`has_digivolve_candidate: { of (default you), zone: [hand|trash], filter? }`** (`DigivolveCandidatePredicate` → `CompiledDigivolveCandidate { of, zones, filter }`); evaluator `permanent_has_digivolve_candidate` scans the listed zones, applies `filter` as a card subject, then the same route probe onto the subject permanent (breeding permanents probe via the `BREEDING_TARGET` sentinel). Validator: empty zone list / any zone other than hand|trash → error.
  - `[Main]` suspend cost authored with the established `condition: { source_is_unsuspended }` + `suspend: { target: source }` idiom (BT11-061 / P-224), NOT `activation_cost`, which at the time was not paid on the `[Main]` action path (`G-ENGINE-MAIN-ON-FIELD-ACTIVATION-COST-UNPAID`, RESOLVED 2026-09-20 — see the entry above; the idiom here stays as authored and tested).
- **Citations:** `general_rule.pdf` (Ver.3.6) §8-1-3-1 (1 digivolution requirement is chosen on the revealed card, then a card that MEETS it — the `can_digivolve_onto` gate), §8-1-3-2 (cost paid), §8-1-3-3 (place on top, draw 1), §8-1-2-7 (a card that can no longer be digivolved is returned to its original location — the engine's `restore_card_source_ref` on payment failure); DCGO `BT25_092.cs:92-97` (`CanDigivolveDigimon`), `:106-110` (`ValidTarget`), `:181-251` (prompt order), `CardEffectCommons.cs:783-1080` (`DigivolveIntoHandOrTrashCard`: `CanSelectCardCondition` = `IsDigimon && cardCondition && !CanNotEvolve && CanPlayCardTargetFrame`, `canNoSelect: isOptional`).
- **Gate / coverage:** `tests/dsl/digivolve_target_predicates.rs` (6: compile of both leaves, undeclared-binding rejection, unsupported-zone rejection, target pick restricted to routable permanents, result pick restricted to routable hand+trash cards, no-candidate → no prompt) + `tests/cards_behavioral/bt25/bt25_092.rs` (12, incl. cost from hand / from any own Digimon's sources / both origins in one prompt / result from hand at cost 3−1 / result from trash / declines / activation refusals). `--test dsl` 926/0, `--lib` 329/0.
- **YAML shape:** `select_own_permanent { bind_as: target, optional: true, filter: { all_of: [kind: digimon, has_digivolve_candidate: { of: you, zone: [hand, trash], filter: … }] } }` → `select_union_zone { zones: [hand, trash], optional: true, filter: { all_of: [kind: digimon, can_digivolve_onto: target, …] }, bind_as: evo }` → `effect_initiated_digivolve { target: target, source: evo, cost: { reduce: 1 } }`.

## `OnAddDigivolutionCards` trigger timing — cards placed into a permanent's digivolution cards by an effect (G-ENGINE-ON-ADD-DIGIVOLUTION-CARDS / G-DSL-ON-CARD-PLACED-UNDER-TRIGGER) — RESOLVED 2026-09-17

- **Severity (at close):** was 🔴 BLOCKED — net-new trigger timing + dispatch hook; not composable from existing DSL vocabulary.
- **Discovered in:** BT25 orphan-staples-1 slice (2026-06-06); re-confirmed by the three-musketeers-1 campaign (2026-09-12/13).
- **Card(s):** BT25-005 Pagumon (inherited "[Your Turn] [Once Per Turn] When [Three Musketeers] trait cards are placed in this Digimon's digivolution cards, it may digivolve … cost reduced by 2"), EX7-005 Kapurimon ("When effects place Option cards with the [Three Musketeers] trait in this Digimon's digivolution cards, gain 1 memory"), BT7-056 Dorumon ("When one of your effects places a digivolution card under this Digimon, gain 1 memory"), plus the same shape on BT7-005, RB1-001, BT8-005, BT9-066, ST13-05/14, EX11-074. The DSL-side twin `G-DSL-ON-CARD-PLACED-UNDER-TRIGGER` (BT10-093 Yuu Amano clause 1 "[All Turns][Once Per Turn] When a purple card is placed under this Tamer, <Draw 1> and gain 1 memory" — `event_host_permanent_is_source: true` + `event_added_card_any: { color_has: [purple] }`; EX10-056 Bagramon's placed-under observer half) closes with it — those clauses are now authorable and stay OMITTED in their YAML until a card stage picks them up.
- **What was missing:** the `Timing` enum had the source-REMOVED observer (`on_digivolution_card_trashed`) but no source-ADDED sibling, and no engine dispatch fired when an effect pushed cards under a permanent (`place_as_bottom_source` / `place_self_under_permanent` / material placement / Save / Material Save / Training / Mind Link).
- **Resolution (engine + DSL, 2026-09-17):**
  - `EffectTiming::OnAddDigivolutionCards` (`enums.rs`) ← DSL `Timing::OnAddDigivolutionCards` (`when: on_add_digivolution_cards`) via `CompiledTiming::OnAddDigivolutionCards` (`timing_map.rs`, `lower_triggered.rs`, `Effect::on_add_digivolution_cards`).
  - `TriggerSource::SourcesAddedToStack { host, host_card, cards: Vec<CardHandle>, cause: EffectAttribution }` (`selection.rs`; `TriggerSource` is now `Clone` but not `Copy`). Board-wide fan-out over every battle-area AND breeding permanent of both players (`effect_queue.rs`), so the placed card's own inherited observer fires (rule 15-5-3).
  - `TriggerContext` gains `event_cause_effect: Option<EffectAttribution>` (the placing effect, captured at the mutation site) and `added_source_cards()` (the batch, via `moved_card_sets`); `event_host_permanent` / `event_host_card` carry the host. `event_card` is deliberately unset — the event is a batch.
  - **Batch window** (`Game::pending_added_sources: Vec<PendingAddedSources>`, `note_effect_added_sources` / `flush_pending_added_sources`, host re-resolved by stable top card at flush): fires ONCE per host per placing-effect body, flushed at the outermost `drain_effect_queue` next to the `OnDiscardHand` window. Rule 15-5-2; DCGO `AddDigivolutionCardsBottom(list, cardEffect)` once per list — `<Material Save N>`'s per-card loop coalesces.
  - Noting sites (effect-driven only): `Game::place_as_source_observed` (battle + breeding branches; the Security branch notes inside `complete_effect_security_removal` `BottomSource`/`TopSource`, carrying the effect card through `pending_security_source_cause_card`), `EffectContext::{place_card_under_permanent_bottom, place_cards_under_tamer_bottom_in_order, move_all_matching_sources_under_tamer, move_selected_sources_under_tamer, attach_tamer_to_digimon, training_place_deck_top_under_self_face_down}`, `Game::place_permanent_as_bottom_sources_by_effect`, `Game::place_specific_link_card_as_bottom_source(.., cause)`, `Game::seat_card_source_under_permanent(.., cause)` / `move_field_option_under_permanent(.., cause)`. NOT noted (DCGO `cardEffect: null`): normal digivolution, DNA / Blast DNA, DigiXros + Assembly materials, App Fusion, restore/undo paths.
  - Predicates: `event_host_permanent_is_source` (existing), `event_caused_by_own_effect` (generalized: reads `discard_cause_controller` else `event_cause_effect.controller`), NEW `event_added_card_any: { <card predicate> }` (DCGO `cardCondition`, ANY added card matches; evaluated with `PredicateSubject::Card` per batch member).
- **Citations:** `general_rule.pdf` (Ver.3.6) 15-5-2 "a triggering from 1 trigger condition … only trigger once, even if it occurred multiple times at the same time" and 15-5-3 (p.23-24, whose worked example is this exact inherited effect); DCGO `Permanent.cs:1078-1136` / `:1147-1240`, `CardEffectCommons/CanUseEffects/OnAddDigivolutionCards.cs` (non-null `CardEffect` gate, `permanentCondition`, `cardEffectCondition`, `cardCondition` ANY), `BT7_056.cs:101-105`, `EX7_005.cs:34-40`, `BT25_005.cs:30-52`, null-cardEffect paths `CardController.cs:1736`, `SelectDigiXrosClass.cs:910`, `SelectAssemblyClass.cs:302`, `SelectAppFusionEffect.cs:234`; keyword placements `Save.cs:61`, `MaterialSave.cs:114`, `Training.cs:29`.
- **Gate / coverage:** `code/digimon-engine/tests/on_add_digivolution_cards.rs` (11 tests, green): single-card fire with host + batch + attribution; multi-card batch fires ONCE; own-only vs any-effect gate vs the opponent's placing effect; host gate vs a sibling; rule 15-5-3 self-placement; normal digivolution does NOT fire; DNA digivolution does NOT fire; host deleted before flush fires nothing; DSL Dorumon-shape (memory +1 on own effect, silent on opponent's) and Kapurimon-shape (`event_added_card_any` positive / non-Option / wrong-trait negatives). Driver YAML (BT25-005 / EX7-005 / BT7-056) intentionally left to the card-authoring stage. **Driver YAML shipped 2026-09-17** (three-musketeers-1 / stage on-add-digivolution-cards drivers): EX7-005 / BT7-056 / BT25-005 IMPLEMENTED with 13 / 16 / 18 per-card tests (`tests/cards_behavioral/{ex7/ex7_005,bt7/bt7_056,bt25/bt25_005}.rs`); see the `docs/RUST_ENGINE_GAPS.md` entry for the authored shapes.
- **Related:** `OnDigivolutionCardTrashed` (source-removed sibling), `OnDiscardHand` (the batch-window precedent), `G-DSL-PLACE-AS-TOP-SOURCE`.
...
## G-OPT-REFUND-ON-DECLINE — RESOLVED 2026-06-10

- **Severity:** 🟡 DSL + engine semantics gap (once-per-turn consumed on full decline; optionality inexpressible at DCGO fidelity).
- **Discovered in:** AD1-024 Imperialdramon: Fighter Mode ([All Turns][Once Per Turn] play/digivolve observer).
- **Card(s):** `AD1-024` (pattern generalises to every DCGO `ActivateClass` with `SetIsSkippable` + `RemoveUse`).
- **Effect text:** "[All Turns] [Once Per Turn] When Digimon are played or digivolve, you may suspend 1 of your opponent's Digimon and unsuspend this Digimon. Then, if played or digivolved by effects, you may return 1 of your opponent's suspended Digimon to the bottom of the deck."
- **Resolution (four substrate pieces):**
  1. **Clause `outer_prompt: true`** — forces the explicit outer accept/decline confirm for an `optional: true` clause even when its first body step is declinable (DCGO's always-shown initial Yes/No). Declining the confirm drops the queued effect BEFORE `run_queued_effect`'s process tail records the OPT — so the declined-outright path refunds for free (the engine's outer-prompt decline already had `RemoveUse` semantics; the lowering just never forced the prompt).
  2. **Step `refund_opt: {}`** — DCGO `RemoveUse` for the accepted-then-declined-everything path. Lowers to `CompiledStep::RefundOpt`; the lowering forces a refund-bearing OPT clause into its own `shared_opt_group` (`0x80 | clause_index`) so the OPT key is statically known, captures it in `StepRuntime::opt_key` (survives parked tails), and the step runner calls `Game::unrecord_source_permanent_activation` → `Permanent::unrecord_activation`. Authored as a final `if:` over `binding_absent:` for every pick the body could make.
...
- **Severity:** 🟡 DSL verb + engine primitive gap (top-position digivolution-source insertion).
- **Discovered in:** BT13-088 Belphemon: Sleep Mode (judge-quiz first wave, 2026-05-29); closed when EX9-074 Kimeramon's last cosmetic divergence was cleared.
- **Card(s):** `EX9-074` Kimeramon ("place 1 level 4 or lower [DM] trait Digimon card from your trash as this Digimon's **top** digivolution card"), `BT13-088` Belphemon: Sleep Mode ("placing 1 [Belphemon: Rage Mode] from your trash on **top** of this Digimon's digivolution cards"). Partially unblocks EX10-059 DarknessBagramon's sentence-2 cost (placement half only — still OMITTED on the 3-pick cost flow + "with cards under it" filter).
- **Resolution:** New DSL verb `place_as_top_source: { source, target, face_down? }` — sibling of `place_as_bottom_source` with the same args shape. Lowers to `EffectContext::place_as_top_source` → `Game::place_as_top_source_observed` → `Permanent::push_as_top_source`, inserting the card DIRECTLY BENEATH the active top card (this engine's `card_sources` is bottom-first / top-last, so the top-source slot is index `len − 1`) — exactly DCGO `Permanent.AddDigivolutionCardsTop` (`cardSources.Insert(1, …)` in DCGO's top-first ordering); the permanent's top card / identity is unchanged. Shares the bottom verb's entire lifecycle via the position-parameterized `Game::place_as_source_observed` body: security sources materialize and route through `fire_effect_security_removal` with the new `SecurityRemovalDestination::TopSource` variant (handled in the effect-queue security-removal resolver next to `BottomSource`), and Material-source extraction soft-removes an emptied carrier. Unlike the bottom arm there is intentionally NO whole-permanent branch (DCGO's `AddDigivolutionCardsTop` explicitly cannot tuck a field permanent; that path is `IPlacePermanentToDigivolutionCards`).
- **Coverage:** `--test dsl -- place_as_top_source` (4: position with 2 pre-existing sources — lands at `len − 2`, NOT bottom, ordering/top card untouched; sourceless stack → sole source; `face_down: true`; face-up default) + `--test cards_behavioral -- ex9_074` (position pinned in `ex9_074_on_play_accepting_placement_moves_card_to_source_stack`; structural pin asserts `PlaceAsTopSource` present and `PlaceAsBottomSource` absent) + `--test cards_behavioral -- bt13_088` (`bt13_088_on_play_places_rage_at_top_source_position`).

## G-DSL-AURA-GRANT-TRAITS — RESOLVED 2026-09-12

- **Severity:** 🟡 DSL vocabulary gap (engine primitive existed).
- **Discovered in:** EX7-010 Deputymon, three-musketeers-1 campaign (slice ex7-line, 2026-09-12).
- **Card(s):** `EX7-010` Deputymon ("[Your Turn] This Digimon gains the [Three Musketeers] trait"). Generalizes to every printed "gains the [X] trait" / "is also treated as having the [X] trait" continuous grant (DCGO `ChangeTraitsClass`).
- **What was missing:** the engine already had `ModifierType::ChangeTraits` with `ModifierPayload::Traits { add, replace }`, consumed by `Permanent::synth_identity` (so `trait_has` filters / `has_trait_for_rules` / alt-path trait gates see it), but no DSL field lowered to it — `kind: aura` could only carry `modifier_name` (a `Name` payload), and installing `ChangeTraits` with a `Name` payload trips the registry's payload-mismatch guard.
- **Resolution:** new `AuraBody.grant_traits: Vec<String>` (spec) → `CompiledDeclarativeClause::Aura.grant_traits` (compiled, `#[serde(default)]` for the bincode pack) → `lower_aura` installs `add_declarative_modifier_with_payload(ChangeTraits, Traits { add, replace: false })` on the self-aura and filtered-target paths every declarative tick (so `active_when: { your_turn: true }` gating drops the overlay on the opponent's turn). Validator accepts it as an aura payload. Not wired on the `while_condition` self-aura path (falls through to the tick path).
- **Coverage:** `--test cards_behavioral -- ex7_010` (`gains_three_musketeers_trait_on_own_turn_only` — printed traits kept, overlay gone on the opponent's turn; `trait_grant_is_face_up_only`).
- **YAML shape:** `- kind: aura` / `target: {}` / `active_when: { your_turn: true }` / `grant_traits: ["Three Musketeers"]`.

## G-DSL-RETURN-UNION-BOUND-TO-DECK — RESOLVED 2026-09-12

- **Severity:** 🟡 hybrid — DSL verb + one small engine primitive (hand → deck).
- **Discovered in:** EX7-043 Tankmon, three-musketeers-1 campaign (slice ex7-line, 2026-09-12).
- **Card(s):** `EX7-043` Tankmon ("By returning 3 cards with the [Three Musketeers] trait from your hand or trash to the top of the deck, <De-Digivolve 1> 1 of your opponent's Digimon" — official Q&A: any hand/trash combination). Generalizes to every "return N cards from your hand or trash to the top/bottom of the deck" cost.
- **What was missing:** `select_union_zone` bindings could be played (`play_union_bound_free`), used (`use_option_bound`) or trashed (`trash_union_bound`), but not returned to the deck; the engine had `return_trash_cards_to_deck_top/bottom` for the trash origin but no hand → deck move at all.
- **Resolution:** new step `return_union_bound_to_deck: { binding, position: top | bottom }` (`StepSpec::ReturnUnionBoundToDeck`, compiled `CompiledStep::ReturnUnionBoundToDeck`, validator checks the binding is declared and rejects `random`/`choice`), lowered in `zone_moves.rs` by origin: hand → NEW `EffectContext::return_hand_card_to_deck(owner, card, to_bottom)` (a return, not a trash — no `OnDiscardHand`; Digi-Eggs route to the digitama deck via `move_card_to_deck`), trash → `return_trash_cards_to_deck_top/bottom`, material → `return_card_source_to_deck`. Each moved card is recorded via `record_returned_to_deck`. Registered in `qa/impact/lowering_verb_map.json`.
- **Ordering note:** each call moves ONE card, so a "return N" cost is N sequential union picks each returned at once — the pick order IS the deck order (last card returned to `top` ends on top). Every ordering DCGO's separate "specify the order" prompt can reach is reachable with no auto-selection; the choice surface is equivalent, not narrower.
- **Coverage:** `--test cards_behavioral -- ex7_043` (`returns_three_to_deck_top_in_pick_order_then_de_digivolves_opponent` asserts the deck-top order = pick order across a hand+trash mix; decline / no-opponent / WD variants).
