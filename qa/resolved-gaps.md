...
- **Commit:** `9af21698c`.
- **Follow-up:** `BT25-100#effect#5` and `BT24-091#effect#4` are the same single row and are fixed by this change; each needs a re-diff against its own preserved sidecar to flip its verdict from `diverged` to `confirmed`. `G-TOOLING-EXAM-OPTION-HAND-LINK` (the older "no shared wire action for a hand plug-in" blocker) was already closed by `7e30343d4`.
