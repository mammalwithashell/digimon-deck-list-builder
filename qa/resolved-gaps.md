...
- **Resolution (DSL + engine evaluator, 2026-09-17):**
  ...
  - `[Main]` suspend cost authored with the established `condition: { source_is_unsuspended }` + `suspend: { target: source }` idiom (BT11-061 / P-224), NOT `activation_cost` — see the new `G-ENGINE-MAIN-ON-FIELD-ACTIVATION-COST-UNPAID` finding in `docs/RUST_ENGINE_GAPS.md`.
