"""Simple game player using urllib to avoid requests connection pool issues."""
...
def run_game(idx, p1_name, p2_name):
    ...
    gid = data['game_id']
    print(f'\n=== GAME {idx}: {p1_name} vs {p2_name} | ID: {gid} ===', flush=True)
    ...
    for step in range(1, 301):
        ...
        try:
            status_code, result = api_post(f'/games/{gid}/actions', {'action': aid})
        except Exception as e:
            issues.append(f'T{turn}: Action exception [{aid}] {desc}: {e}')
            # Try fallback
            fallback = False
        ...
        if status_code != 200:
            issues.append(f'T{turn}: HTTP {status_code} on [{aid}] {desc}')
            log.append(f'  ERR [{aid}] {desc} HTTP {status_code}')
            for oaid, odesc in acts:
                if oaid != aid:
                    try:
                        sc2, r2 = api_post(f'/games/{gid}/actions', {'action': oaid})
        ...
        if any(kw in dl for kw in ['attack', 'block', 'counter', 'security', 'play ', 'digivolve', 'hatch', 'move']):
            log.append(f'  [{aid}] {desc[:60]} -> T{ns.get("turnCount")} Mem={ns.get("memoryGauge")}')

        if turn >= 15:
            log.append(f'  Reached turn {turn}.')
            break

    final = api_get(f'/games/{gid}/state')
    p1f = final['player1']
    p2f = final['player2']
    log.append(f'FINAL: T{final["turnCount"]} Over={final["isGameOver"]} W={final.get("winner")}')
    log.append(f'  P1({p1_name}): h={p1f["handCount"]} s={p1f["securityCount"]} d={p1f["deckCount"]} BA={len(p1f["battleArea"])} tr={len(p1f["trashIds"])}')
    log.append(f'  P2({p2_name}): h={p2f["handCount"]} s={p2f["securityCount"]} d={p2f["deckCount"]} BA={len(p2f["battleArea"])} tr={len(p2f["trashIds"])}')

    # BA detail
    for pi, pf, pn in [(1, p1f, p1_name), (2, p2f, p2_name)]:
        if pf['battleArea']:
            dets = []
            for d in pf['battleArea'][:5]:
                dets.append(f'{d.get("topCardId")}({d.get("topCardName","?")} DP={d.get("dp")} kw={d.get("keywords",[])})')
    ...
    return {'gid': gid, 'p1': p1_name, 'p2': p2_name, 'log': log, 'issues': issues, 'final': final, 'steps': step}
