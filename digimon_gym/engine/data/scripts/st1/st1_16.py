...
class ST1_16(CardScript):
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        effect_main = ICardEffect()
        effect_main.set_effect_name("ST1-16 Main Effect")
        effect_main.set_effect_description("[Main] Delete 1 of your opponent's Digimon.")

        def activate_main():
            print("Activated Gaia Force Main Effect")

        effect_main.set_on_process_callback(activate_main)

        effect_sec = ICardEffect()
        effect_sec.set_effect_name("ST1-16 Security Effect")
        effect_sec.set_effect_description("[Security] Activate this card's [Main] effect.")
        effect_sec.is_security_effect = True

        def activate_sec():
            print("Activated Gaia Force Security Effect")
