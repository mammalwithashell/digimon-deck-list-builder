...
class BT14_032(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        effect0 = ICardEffect()
        effect0.set_effect_name("BT14-032 Security to hand, then optional Sukamon to security")
        ...
        def process0(ctx: Dict[str, Any]):
            ...
            # Add the top card of your security stack to your hand.
            if player.security_cards:
                security_top = player.security_cards.pop(0)
                player.hand_cards.append(security_top)

            # Then, you may place 1 card with [Sukamon] in its name from your hand on top of your security stack.
            sukamon_idx = None
            for i, hand_card in enumerate(player.hand_cards):
                name = getattr(hand_card, 'card_name', None) or getattr(hand_card, 'name', None) or ''
                if 'sukamon' in str(name).lower():
                    sukamon_idx = i
                    break

            if sukamon_idx is not None:
                chosen = player.hand_cards.pop(sukamon_idx)
                player.security_cards.insert(0, chosen)

        effect0.set_on_process_callback(process0)
        effects.append(effect0)

        # Inherited: [On Deletion] 1 of your opponent's Digimon gets -3000 DP for the turn.
        effect1 = ICardEffect()
        effect1.set_effect_name("BT14-032 Inherited DP -3000")
