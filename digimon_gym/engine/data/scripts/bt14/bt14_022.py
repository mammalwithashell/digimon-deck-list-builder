...
class BT14_022(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        effect0 = ICardEffect()
        effect0.set_effect_name("BT14-022 Trash digivolution card and return 1 Digimon to hand")
        ...
        def process0(ctx: Dict[str, Any]):
            """Action: Trash 1 opponent digivolution card, then bounce opponent level 5 or lower with no digivolution cards."""
            player = ctx.get('player')
            game = ctx.get('game')
            if not (player and game):
                return

            # Step 1: Trash any 1 digivolution card of 1 opponent Digimon.
