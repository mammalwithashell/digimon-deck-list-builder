...
class BT14_043(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def process0(ctx: Dict[str, Any]):
            ...
            def own_filter(p):
                return p is not None and not p.is_suspended()

            def opp_filter(p):
                return True

            def after_pay_cost(_paid_perm):
                def do_suspend_opp(target_perm):
                    target_perm.suspend()
            ...
            # Cost payment is optional for activation due to card text "By ..." on optional On Play effect.
            # If player declines or has no valid unsuspended Digimon, no opponent suspension occurs.
