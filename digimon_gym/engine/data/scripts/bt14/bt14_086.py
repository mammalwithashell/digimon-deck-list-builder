...
class BT14_086(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def condition1(context: Dict[str, Any]) -> bool:
            ...
            if not opponent:
                return False
            return len(game.get_battle_area_cards(opponent)) > 0
        ...
        def condition3(context: Dict[str, Any]) -> bool:
            ...
            if not permanent:
                return False
            name = (getattr(permanent, 'name', None) or getattr(permanent, 'card_name', None) or "")
            if 'Numemon' in name or 'Monzaemon' in name:
                return True
            traits = getattr(permanent, 'traits', None) or getattr(permanent, 'types', None) or []
            return 'DigiPolice' in traits
        ...
        def process5(ctx: Dict[str, Any]):
            ...
            def play_filter(c):
                return getattr(c, 'card_name_eng', None) == 'Satsuki Tamahime' or getattr(c, 'name', None) == 'Satsuki Tamahime'
