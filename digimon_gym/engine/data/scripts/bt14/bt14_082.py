    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def process0(ctx: Dict[str, Any]):
            ...
            def target_filter(p):
                ...
                if not card_obj:
                    return False
                if not getattr(card_obj, 'is_digimon', lambda: False)():
                    return False
                has_trait = getattr(card_obj, 'has_trait', None)
                if callable(has_trait):
                    return has_trait('Vaccine')
                return False

            def on_apply(target_perm):
                target_perm.change_dp(2000)

            game.effect_select_permanent(player, on_apply, filter_fn=target_filter, is_optional=False)
        ...
        def process1(ctx: Dict[str, Any]):
            """Action: Suspend this Tamer, then gain 1 memory."""
            player = ctx.get('player')
            if not player:
                return
            perm = card.permanent_of_this_card() if card else None
