...
class BT14_058(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def _can_pay_satsuki_cost(ctx: Dict[str, Any]) -> bool:
            ...
            for c in hand:
                if getattr(c, 'card_id', None) == 'BT14-084' or getattr(c, 'card_name_eng', None) == 'Satsuki Tamahime':
                    return True
            return False

        def _pay_cost_and_grant_rush(ctx: Dict[str, Any]):
            player = ctx.get('player')
            ...
            game.move_card_to_digivolution_cards_bottom(satsuki, perm)
        ...
        # [On Play]
        effect0 = ICardEffect()
        effect0.set_effect_name("BT14-058 Place 1 Satsuki Tamahime and grant Rush")
        ...
        effect0.set_on_process_callback(_pay_cost_and_grant_rush)
        effects.append(effect0)

        # [When Digivolving]
        effect1 = ICardEffect()
        effect1.set_effect_name("BT14-058 Place 1 Satsuki Tamahime and grant Rush")
        ...
        effect1.set_on_process_callback(_pay_cost_and_grant_rush)
