...
class BT14_023(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        effect0 = ICardEffect()
        effect0.set_effect_name("BT14-023 Trash 2 opponent digivolution cards")
        ...
        def process0(ctx: Dict[str, Any]):
            ...
            def target_filter(p):
                return getattr(p, 'is_digimon', False) and not getattr(p, 'has_no_digivolution_cards', False)
        ...
        def process1(ctx: Dict[str, Any]):
            ...
            def target_filter(p):
                return getattr(p, 'is_digimon', False) and len(getattr(p, 'digivolution_cards', []) or []) <= this_count

            def on_grant(target_perm):
                if hasattr(target_perm, 'grant_keyword_until_end_of_opponents_turn'):
                    target_perm.grant_keyword_until_end_of_opponents_turn('_is_cannot_attack')
                else:
