...
class BT14_040(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def condition2(context: Dict[str, Any]) -> bool:
            if card and card.permanent_of_this_card() is None:
                return False
            played_card = context.get('played_card') or context.get('card_played') or context.get('trigger_card')
            if played_card is None:
                return False
            return getattr(played_card, 'card_kind', None) == 1 or getattr(played_card, 'is_tamer', False)
