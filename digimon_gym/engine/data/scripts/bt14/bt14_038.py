...
class BT14_038(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def process1(ctx: Dict[str, Any]):
            ...
            def play_filter(c):
                ...
                if getattr(c, 'level', None) != 6:
                    return False
                return any('Etemon' in _n for _n in (getattr(c, 'card_names', []) or []))
        ...
        def process2(ctx: Dict[str, Any]):
            """Action: Add this card to bottom of security"""
            player = ctx.get('player')
            perm = ctx.get('permanent')
            game = ctx.get('game')
            if not (player and perm and game):
                return
            game.move_card(perm, player.security, position='bottom')
        ...
        def condition3(context: Dict[str, Any]) -> bool:
            ...
            if not player:
                return False
            for c in getattr(player, 'trash', []) or []:
                if any('Etemon' in n for n in (getattr(c, 'card_names', []) or [])):
                    return True
            return False
        ...
        def process3(ctx: Dict[str, Any]):
            """Action: Add 1 Etemon from trash to bottom of security"""
            player = ctx.get('player')
            game = ctx.get('game')
            if not (player and game):
                return
            ...
            def pick_filter(c):
                return any('Etemon' in n for n in (getattr(c, 'card_names', []) or []))

            game.effect_move_card_from_zone_to_zone(
                player=player,
                from_zone='trash',
                to_zone='security',
                card_filter=pick_filter,
                amount=1,
                position='bottom',
                is_optional=True,
            )
