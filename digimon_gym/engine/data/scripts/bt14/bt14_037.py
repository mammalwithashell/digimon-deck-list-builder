...
class BT14_037(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def condition1(context: Dict[str, Any]) -> bool:
            ...
            if player is None:
                return False
            return len(player.security_stack) <= 5
        ...
        def process1(ctx: Dict[str, Any]):
            ...
            sec_count = len(player.security_stack)
            dp_minus = sec_count * 1000
            if dp_minus <= 0:
                return
            ...
            candidate = None
            for digimon in getattr(opponents, 'battle_area', []):
                if digimon is not None:
                    candidate = digimon
                    break

            if candidate is not None:
                candidate.add_dp_modifier(-dp_minus, until_turn_end=True)
