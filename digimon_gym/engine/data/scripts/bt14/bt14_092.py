...
class BT14_092(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def process1(ctx: Dict[str, Any]):
            ...
            if player and card_source is not None:
                if hasattr(player, 'security_cards') and card_source in player.security_cards:
                    player.security_cards.remove(card_source)
                if hasattr(player, 'hand_cards') and card_source not in player.hand_cards:
