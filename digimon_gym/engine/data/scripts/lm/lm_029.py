...
class LM_029(CardScript):
    """LM-029 Yellow Scramble | Option (Yellow, Cost 2)
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def _is_yellow_digimon(c) -> bool:
            if not getattr(c, 'is_digimon', False):
                return False
            colors = getattr(c, 'card_colors', []) or []
            color_names = [col.name for col in colors]
            return 'Yellow' in color_names

        def _is_yellow_digimon_dp_2000(c) -> bool:
            if not _is_yellow_digimon(c):
                return False
            dp = getattr(c, 'base_dp', None) or getattr(c, 'dp', None) or 0
            return dp <= 2000

        # --- Effect 0: [Main] Yellow Digimon digivolves with cost -3, then place in battle area ---
        effect0 = ICardEffect()
        effect0.set_timing(EffectTiming.OptionSkill)
        effect0.set_effect_name("LM-029 Yellow Digimon digivolve cost -3, then place in battle area")
        effect0.set_effect_description(
            "[Main] 1 of your yellow Digimon may digivolve into a yellow Digimon "
        ...
        effect1 = ICardEffect()
        effect1.set_effect_name("LM-029 Delay")
        ...
        effect2.set_effect_name("LM-029 Delay: Return yellow Digimon to deck top, then play")
        ...
        # --- Effect 3: [Security] Play yellow Digimon DP <= 2000 from trash, add this to hand ---
        effect3 = ICardEffect()
        effect3.set_timing(EffectTiming.SecuritySkill)
        effect3.set_effect_name("LM-029 Security: Play yellow Digimon from trash")
        effect3.set_effect_description(
            "[Security] You may play 1 yellow Digimon card with 2000 DP or less "
