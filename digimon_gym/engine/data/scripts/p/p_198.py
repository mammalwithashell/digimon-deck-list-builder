    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def process1(ctx: Dict[str, Any]):
            ...
            def digi_filter(c):
                ...
                if not any('Fallen Angel' in _t or 'TS' in _t for _t in traits):
                    return False
                return True
        ...
        def process2(ctx: Dict[str, Any]):
            ...
            if player and game and player.hand_cards:
                def on_trash(selected_card):
                    if selected_card:
                        player.hand_cards.remove(selected_card)
                        player.trash_cards.append(selected_card)
                game.effect_select_hand_card(player, lambda c: True, on_trash, is_optional=False)
