    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def process0(ctx: Dict[str, Any]):
            ...
            if not (player and game):
                return
            def hand_filter(c):
                if not (any('Evil' in _t or 'Fallen Angel' in _t or 'Seven Great Demon Lords' in _t for _t in (getattr(c, 'card_traits', []) or []))):
                    return False
                return True
            ...
            def reveal_filter_1(c):
                if not (any('Seven Great Demon Lords' in _t for _t in (getattr(c, 'card_traits', []) or []))):
                    return False
                return True
