...
class BT24_045(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        effect0 = ICardEffect()
        effect0.set_effect_name("BT24-045 Alternate digivolution requirement")
        effect0.set_effect_description("Alternate digivolution requirement")
        # Alternate digivolution: Lv.3 with [Demon] trait for cost 2
        ...
        effect1 = ICardEffect()
        effect1.set_effect_name("BT24-045 If you have 5 or less cards, draw 1")
        ...
        effect2 = ICardEffect()
        effect2.set_effect_name("BT24-045 Trash From Hand, Suspend, Gain Keyword Cannot Unsuspend")
        effect2.set_effect_description("Trash From Hand, Suspend, Gain Keyword Cannot Unsuspend")
        ...
        def process2(ctx: Dict[str, Any]):
            """Action: Trash From Hand, Suspend, Gain Keyword Cannot Unsuspend"""
            player = ctx.get('player')
            perm = ctx.get('permanent')
            game = ctx.get('game')
        ...
        effect4 = ICardEffect()
        effect4.set_effect_name("BT24-045 When your hand is trashed from, digivolve")
        ...
        effect4.set_hash_string("BT24_045_YT_ESS")
        ...
        def process4(ctx: Dict[str, Any]):
            ...
            def digi_filter(c):
                if not getattr(c, 'is_digimon', False):
                    return False
                if not (any('Titamon' in _n for _n in getattr(c, 'card_names', [])) or any('Titan' in _t for _t in (getattr(c, 'card_traits', []) or []))):
                    return False
                return True
