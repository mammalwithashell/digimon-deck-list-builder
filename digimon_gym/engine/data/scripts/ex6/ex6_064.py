...
class EX6_064(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        def process0(ctx: Dict[str, Any]):
            ...
            def reveal_filter(c):
                if not getattr(c, 'is_digimon', False):
                    return False
                if not (any('Beastkin' in _t or 'Holy Beast' in _t or 'Cherub' in _t for _t in (getattr(c, 'card_traits', []) or []))):
                    return False
                return True
        ...
        effect2 = ICardEffect()
        effect2.set_effect_name("EX6-064 Security: Play this card")
