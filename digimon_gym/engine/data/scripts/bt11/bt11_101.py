...
class BT11_101(CardScript):
    ...
    def get_card_effects(self, card: 'CardSource') -> List['ICardEffect']:
        ...
        # Cost -1
        effect0 = ICardEffect()
        effect0.set_effect_name("BT11-101 Cost -1")
        ...
        def process1(ctx: Dict[str, Any]):
            """Action: DP -5000, Change Security Attack"""
            player = ctx.get('player')
            perm = ctx.get('permanent')
            game = ctx.get('game')
        ...
        effect2 = ICardEffect()
        effect2.set_effect_name("BT11-101 Security: Play this card")
