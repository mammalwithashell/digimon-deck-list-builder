...
export function DeckCardEntry({ entry }: DeckCardEntryProps) {
  ...
  return (
    ...
    >
      <Card cardId={entry.cardId} cardName={entry.cardData?.name} size="sm" />
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          onClick={() => removeCardFromDeck(entry.cardId)}
        ...
        <button
          onClick={() => {
            if (entry.cardData) addCardToDeck(entry.cardId, entry.cardData);
          }}
