...
27. **DCGO recorder maintenance (2026-05-26; hook map re-verified 2026-08-21).** The DCGO mod intercepts gameplay through `GameRecorder.Instance.*` at **~28 call sites across 11 files**. Do not trust a summarised subset — an out-of-date version of this list caused two separate false "this path is entirely unhooked" diagnoses, each of which sent an investigation down a wrong branch. **Re-derive the map before relying on it:**
    ...
    Current map, by recorder method:

    | Method | Call sites |
    |---|---|
    | `LogSelectionRow` | **14** — `UserSelectionManager.cs` (×2), `SelectCardEffect.cs` (×2), `SelectHandEffect.cs` (×2), `SelectPermanentEffect.cs` (×2), `SelectAttackEffect.cs` (×2), `SelectCountEffect.cs`, `SelectDigiXrosClass.cs`, `MultipleSkills.cs`, `OptionalSkill.cs` |
    | `LogAction` | 5 — `TurnStateMachine.cs` (×4), `CardController.cs` |
    ...
    The `LogSelectionRow` family is the one most often forgotten: it covers **Assembly/DigiXros material selection**, and it fires on the AI/harness path (each `Activate()` routes both seats through `AutoSelect()` under `HarnessAuto.DrivesLocalSeat`). When rebasing the DCGO submodule onto a newer upstream commit, verify every hook is still in place at its chokepoint — if upstream refactored the chokepoint, the hook needs to move.
