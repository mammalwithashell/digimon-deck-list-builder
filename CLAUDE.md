...
When questioning *what a card or keyword should do* — for instance, "is Fragment optional?", "what does Save target?", "does Progress block X?" — consult sources in this order. **DCGO is battle-tested and community-curated; trust it over `RULES_CONTEXT.md` (which is LLM-generated) and over the card-text JSON (which is API-ingested and not always accurate).** Note the split: this ordered list governs *rules and behavior*. For **printed card *data*** — traits (including Rule-granted ones), digivolution costs/conditions, printed effect/inherited/security text, and official Q&A rulings — the **official Bandai DB outranks even DCGO** (see "Printed card data" below).
...
3. **`data/card_overrides.json`** then **`data/cards.json`** — the printed card text (`effect_text`, `inherited_text`, `security_text`). `cards.json` is ingested from the digimoncard.io API and is **not always accurate** (it drops multi-colour digivolve costs and the `(Rule) Trait: Has [X] Type/attribute.` grants *entirely*); `card_overrides.json` holds our hand-maintained corrections and is trusted over raw `cards.json` (overrides survive re-ingestion). When this printed data is wrong, the **official Bandai DB** (see "Printed card data" below / `data/card_official.json`) is the upstream authority — fold its corrections into `card_overrides.json`. Use the text as the no-approximations *target*, but when it conflicts with DCGO's behavior, DCGO wins for now.
...
**Printed card data — the official Bandai DB is authoritative.** For the *printed/structured data* of a card (traits — **including `(Rule) Trait: Has [X] Type/attribute.` grants that don't appear in the trait line** — digivolution costs/conditions, printed effect/inherited/security text, official Q&A rulings) the publisher's own database, **`world.digimoncard.com`**, is the source of truth and **outranks both `cards.json` and DCGO** for that data. The digimoncard.io API behind `cards.json` is lossy and silently drops these (notably Rule-granted traits → see the Ice-Snow line, the Mineral/Rock LIBERATOR cards, the Iliad/TS cards, and `[Free]`-attribute Royal Knights). A local, offline mirror is committed: **`data/card_official.json`** (machine-readable index) + **`data/card_bundles/<ID>.md`** (per-card human/agent bundle), produced by `code/tools/build_card_bundles.py`; reconcile dropped traits (Rule grants AND the API-dropped `form` field, e.g. the Appmon mechanic trait) into `card_overrides.json` with `code/tools/audit_digivolve/reconcile_traits.py`. Prefer this mirror over re-scraping. **DCGO remains the authority for *behavior* (how a card resolves) and `general_rule.pdf` for *rules*; the official DB wins only for *what the card prints*.**

When DCGO and the official PDF disagree on a rules question, the PDF governs; when DCGO and the card-text JSON disagree on what a card does, DCGO governs; when the question is what a card *prints* (traits, costs, text), the official Bandai DB governs over both DCGO and the card-text JSON. The retired guidance ("do not skip ahead to DCGO", "RULES_CONTEXT.md is canonical") is intentionally reversed here.

**Look at the actual card.** For *printed text* ("what does it say") the highest-fidelity sources are the **card image itself** — the literal card face — and the **official Bandai DB bundle** (`data/card_bundles/<ID>.md`), its structured/searchable companion; both outrank the API-ingested `cards.json` (and `card_overrides.json`, which only patches sparse fields). The image is the visual tiebreak for tiny print (e.g. digivolve circles); the bundle is the authoritative structured text + traits + costs. A complete local image mirror exists and the `Read` tool renders `.webp` natively. When reasoning about, implementing, fixing, or QA'ing a card — or a whole archetype — use the **`/digimon-card-lookup`** skill (`.claude/skills/digimon-card-lookup/`): it resolves a card ID, a card *name* (all printings — e.g. "Wormmon" is 8 distinct cards), or an *archetype* to image paths + text via `resolve_cards.py`, then you `Read` the images. This governs *printed text only*; DCGO C# / `general_rule.pdf` remain authoritative for *behavior*. A `UserPromptSubmit` hook (`.claude/hooks/digimon_card_image_hint.py`) reminds you with image paths whenever a prompt mentions card IDs.
...
# Full cards_behavioral (~5.9k tests): use THIS invocation — see rule 33 for why
...
27. **DCGO recorder maintenance (2026-05-26; hook map re-verified 2026-08-21).** The DCGO mod intercepts gameplay through `GameRecorder.Instance.*` at **~29 call sites across 12 files**. Do not trust a summarised subset — an out-of-date version of this list caused two separate false "this path is entirely unhooked" diagnoses, each of which sent an investigation down a wrong branch. **Re-derive the map before relying on it:**

    ```bash
    BASE_DCGO="$(dirname "$(git rev-parse --path-format=absolute --git-common-dir)")/DCGO"
    grep -rn "GameRecorder\.Instance" "$BASE_DCGO/Assets/Scripts/Script/" --include=*.cs | grep -v "/Recording/"
    ```
    ...
    | `LogEffectActivation` | 1 — `ICardEffect.cs` (`ICardEffectExtensionClass.Activate_Optional_Effect_Execute`, immediately AFTER its `Activate_Effect_Execute` coroutine returns — NOT at the pre-existing `Debug.Log` earlier in the same method, which fires before the optional yes/no decision and would misrecord a declined effect as executed). `Activate_Effect_Execute` has exactly one caller in the whole codebase, so this single hook catches every activation — both the generic queue-driven path (`AutoProcessing.ActivateEffectProcess`, 4 call sites) and the ~35 call sites where a card script directly activates a linked/cutin/chain sub-effect. **Known bypass**: 25 `SetIsBackgroundProcess(true)` cards route through `AutoProcessing.ActivateBackgroundEffectsOfCards`, which calls `.Activate(hashtable)` directly and skips this hook entirely — their clauses are structurally unmeasured, not evidence of non-firing. See `docs/DCGO_RECORDING_SCHEMA.md`'s `effect_activation` row. |
    | `LogSelectionInt` / `LogSelectionBool` | **0 — defined but DEAD.** Superseded by `LogSelectionRow` in `5b1284ded`. Do not treat their existence as evidence that a path is hooked. |

    **The `LogSelectionRow` family (added `5b1284ded`, 2026-08-16) is the one that gets forgotten**, and it covers **Assembly/DigiXros material selection**. It fires on the AI/harness path: each `Activate()` routes both seats through `AutoSelect()` under `HarnessAuto.DrivesLocalSeat`.

    **`UserSelectionManager.SetIntForPlayer` / `SetBoolForPlayer` are NOT "every selection response"** — an earlier version of this rule said so and it was wrong. They are the **fallback channel only**, emitting `generic_int` / `generic_bool`; every typed prompt (`SelectCardEffect`, `SelectHandEffect`, `SelectPermanentEffect`, `SelectAttackEffect`, `SelectCountEffect`, `SelectDigiXrosClass`, …) routes through its own dedicated per-class hook. See `docs/DCGO_RECORDING_SCHEMA.md`'s `selection` row payload table for the full prompt-kind list.

    When rebasing the DCGO submodule onto a newer upstream commit, verify every hook is still in place at its chokepoint — if upstream refactored the chokepoint, the hook needs to move.

    **`LogAction` records ATTEMPTED actions, not completed ones.** It hooks `QueueMainPhaseAction`, which fires when an action is queued — *before* DCGO decides legality — so a rejected attempt is recorded like any other. `LogActionResolution` (`action_detail`, correlated by `(step, actor)`) is what proves an action actually resolved; its **absence on a play-range action means DCGO never completed that play**. A replay that treats every recorded action as one our engine must accept will therefore manufacture false `illegal_action` divergences against our engine. After ANY change to `code/digimon-engine/src/action/space.rs` (constants, ranges, encoder formulas), regenerate `ActionSpace.cs` in the **base-repo** DCGO (rule 29) via `cargo run -p action-space-export | python code/tools/action-space-export/emit_csharp.py --out "$(dirname "$(git rev-parse --path-format=absolute --git-common-dir)")/DCGO/Assets/Scripts/Script/Recording/ActionSpace.cs"` and commit the diff (in the base repo, where the submodule is checked out). CI (`.github/workflows/action-space-codegen-drift.yml`) catches drift, but local development should regen explicitly when touching action-space code. The JSONL recording schema is described in `docs/DCGO_RECORDING_SCHEMA.md`; the build setup in `docs/DCGO_BUILD.md`.
28. **Card scripting is DSL-first (2026-05-29).** The default path for a new card is a YAML spec in `code/digimon-engine/cards/<set>/`, lowered to `Effect`/`CardEffect` by the `digimon-dsl` crate. The intent is a compounding-coverage flywheel: when a card needs something the DSL lacks, **widen the substrate instead of routing around it** — add DSL vocabulary (lower it in `code/digimon-dsl/`, log the gap to `qa/dsl-vocab-gaps.md`) or, if the engine itself lacks the primitive, add it and log to `docs/RUST_ENGINE_GAPS.md`. That investment amortizes across every later card, so implementation complexity should decline as coverage grows. Hand-writing a `CardEffect` in `code/digimon-engine/src/cards/raw_rust/` is a **last resort** for cards the DSL fundamentally can't express — reaching for it because it's faster in the moment starves the vocabulary and breaks the flywheel. **Clone-safety constraint (make-engine-cloneable, 2026-06-23):** a `raw_rust` effect MUST be clone-safe — either **atomic** (no mid-effect player selection) or it must **drive selections through the resumable VM** (build a `crate::resume::ResumeStack` / `ResumeFrame` and set `Game::pending_selection_resume`), NEVER install a bespoke closure-based `pending_selection` (a `callback: Box::new(move |…| …)`). The whole pool's selection surface is on the data VM so `Game: Clone` powers MCTS/AlphaZero search; a closure-based park clones to a panic-stub (`PendingSelection::clone`, `selection.rs`) that fails loudly when the cloned selection resolves. If a raw_rust card needs a selection, reuse a flipped DSL installer's path or construct a `ResumeFrame` directly (see the `RevealBucketStep` example — fully data-driven). Verdicts: DSL cards tracked in `qa/qa-reports/validated_cards_dsl.json`; hand-written in `qa/qa-reports/validated_cards.json`. Skills: `/batch-implement-cards-rust-dsl` and `/implement-rust-dsl-archetype` (YAML path), `/assess-archetype-rust` (pre-flight gap audit → `docs/RUST_ENGINE_GAPS.md` + a fix-plan under `.claude/plans/`), `/batch-implement-cards-rust` (hand-written escape-hatch path).
...
33. **Running the big engine test binaries reliably (2026-07-02; distilled from field incidents).** The full `cards_behavioral` target (~5.9k tests, dev build) has recurring NON-code failure modes that mimic regressions/hangs — know them before debugging:
    - **Reliable + fast invocation (re-measured 2026-08-24):** `RUST_MIN_STACK=268435456 cargo test --manifest-path code/digimon-engine/Cargo.toml --test cards_behavioral -- --test-threads=8` — **~5 min** (was 56.5 min before the perf pair below; the "~9 min" this line used to claim was never reproduced here). Two changes did it, and they only work together:
        1. `[profile.dev] opt-level = 2` (root `Cargo.toml`) — the tests are simulation-heavy and were dominated by unoptimized engine code.
        2. **mimalloc** as the `cards_behavioral` binary's `#[global_allocator]` (`[dev-dependencies]`, test-only). The Windows system heap's lock was the scaling wall: on a 484-test subset at opt-level 2 it ran 4 threads SLOWER than 1 (136-166s vs 60s). With mimalloc: 1t 42.9s / 4t 20.3s / 8t 19.5s / 16t 19.0s.
      Adopt neither alone: opt-level without the allocator makes 4-thread runs worse, and the allocator without opt-level only reaches ~13 min.
      Threads flatten hard after 4 — 8 is fine, 16 buys ~7%. Raising it is not the lever it used to look like.
      The 256 MB stack is still set here, but is probably now unnecessary: at opt-level 2 the subset passes with no `RUST_MIN_STACK` at all (smaller frames). Removing it was NOT re-verified across the full suite — do that before dropping it.
      Underlying cause still open: `card_store::shared_card_store` memoizes per distinct test card set into a process-global map that is never evicted (~2 MB retained per test; 523 MB -> 1471 MB across 484 tests at ONE thread). mimalloc absorbs the symptom; bounding that cache would remove it.
    - **Flaky mid-run abort:** a parallel run dying with NO `test result:` line, at a *varying* test, exit 1 (not 101) is the environmental stack-overflow abort — NOT a regression. Confirm a real failure only via a clean `test result: FAILED` line (exit 101) or a `panicked at` message.
    - **OOM masquerading as a hang:** `RUST_MIN_STACK=536870912` (512 MB) × default ~16 threads commits 8 GB+ of stacks; the run dies mid-suite with no summary and a background invocation waits forever. Don't raise the stack past 256 MB with many threads; suspect OOM before an infinite loop.
    - **Hung binaries poison everything downstream:** a stale `cards_behavioral-<hash>.exe` from a prior (often background) run (a) holds the exe name → the next build fails with `LNK1104`, and (b) **holds the CARGO_TARGET_DIR build lock → every subsequent cargo command in that target dir appears to hang**, making healthy tests look broken. Before any full run — and whenever cargo mysteriously stalls — `Get-Process | ? { $_.ProcessName -match 'cards_behavioral' }` / stray `cargo` processes, and kill them.
    - **CI coverage is partial; attribute against main first:** only lib + `cards_behavioral` + `dsl` + `archetypes` + `judge_quiz`/`selection` run in CI (`engine-clone-safety.yml`, `dsl-guards.yml`); other integration binaries (`cost_hooks`, `deck_tools`, …) rot on main. When a full run (`--no-fail-fast` to see everything) fails in a suite your change didn't touch, run that suite on unmodified main before debugging your change; fix stale-on-main tests in their own commit.
...
- **DCGO recording pipeline**: `docs/DCGO_BUILD.md` (build the mod) + `docs/DCGO_RECORDING_SCHEMA.md` (JSONL format) + `docs/DCGO_HARNESS.md` (unattended job harness: queue N games, DCGO drains them without a human, then triage the corpus; seeded games are reproducible end to end) — modded DCGO client that records games as 2192-action-space JSONL, consumed by `code/tools/dcgo-replay/` as an additional Rust-engine faithfulness oracle
- **DCGO card-clause exam**: `docs/DCGO_EXAM.md` + the `/dcgo-exam` skill — scripted scenarios (`qa/dcgo-exams/<SET>/<CARD-ID>.yaml`) that make DCGO answer **"what does this card do HERE"** per printed clause, instead of the corpus's "what did DCGO do". Both seats scripted over a stacked deck; the sim side is a third `RecordingSource` adapter. Verdicts are per-`(card, clause)` in `qa/qa-reports/exam-verdicts/` with five classes — `confirmed` / `diverged` / `unreachable` / `unavailable` / `unmeasured` — and the **full denominator is always printed** (a card reads "8 clauses: 5 confirmed, 1 diverged, 2 unmeasured", never "passed"). `diverged` is a finding to triage, not proof we are wrong: `general_rule.pdf` outranks DCGO. `exam --sim-only` gates PRs (no Unity); the oracle pass is deliberate and local — see the doc for the four reasons DCGO cannot gate PRs on GitHub-hosted runners, and the known gaps. **`/archetype-campaign`** is the dispatch unit built on top of the exam: point it at one archetype and it claims the card pool, drives the missing cards through `/batch-implement-cards-rust-dsl`, drives the outstanding clauses through this same oracle loop, triages divergences, and releases its claims with a ledger entry so a second node doesn't repeat the work. `qa/qa-reports/exam-index.md` (sorted by `unmeasured` descending) is what to dispatch next
