...
- **DCGO card-clause exam**: `docs/DCGO_EXAM.md` + the `/dcgo-exam` skill — scripted scenarios (`qa/dcgo-exams/<SET>/<CARD-ID>.yaml`) that make DCGO answer **"what does this card do HERE"** per printed clause, instead of the corpus's "what did DCGO do". Both seats scripted over a stacked deck; the sim side is a third `RecordingSource` adapter. Verdicts are per-`(card, clause)` in `qa/qa-reports/dcgo_exam_verdicts.json` with five classes — `confirmed` / `diverged` / `unreachable` / `unavailable` / `unmeasured` — and the **full denominator is always printed** (a card reads "8 clauses: 5 confirmed, 1 diverged, 2 unmeasured", never "passed"). `diverged` is a finding to triage, not proof we are wrong: `general_rule.pdf` outranks DCGO. `exam --sim-only` gates PRs (no Unity); the oracle pass is deliberate and local — see the doc for the four reasons DCGO cannot gate PRs on GitHub-hosted runners, and the known gaps
