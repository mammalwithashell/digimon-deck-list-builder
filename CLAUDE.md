...
33. **Running the big engine test binaries reliably (2026-07-02; distilled from field incidents).** The full `cards_behavioral` target (~5.9k tests, dev build) has recurring NON-code failure modes that mimic regressions/hangs — know them before debugging:
    - **Reliable + fast invocation (verified):** `RUST_MIN_STACK=268435456 cargo test --manifest-path code/digimon-engine/Cargo.toml --test cards_behavioral -- --test-threads=8` (~9 min, ~2 GB RSS). The 256 MB stack defeats the flaky per-test-thread stack overflow; the 8-thread cap avoids both the OOM trap and slow 16-thread contention.
