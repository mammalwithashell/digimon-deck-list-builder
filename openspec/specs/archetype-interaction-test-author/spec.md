...
Defines the `/archetype-interaction-test-author` capstone skill that researches a Digimon TCG archetype as a system (combos, digivolution lines, playstyle, win conditions), emits a durable archetype-model document, then plans and authors multi-card DebugRunner interaction tests plus the static archetype tests. The skill runs after the archetype's cards are implemented and per-card tests are green; it does not re-implement cards, and confirmed test failures route to the shared gap trackers rather than editing engine code.
