# Gameplay Reward Config

## Purpose
Separates the universal game-mechanic reward shape (terminal outcome, security shaping, quick-win, stall, digivolve, breeding, digivolve-driven attack) from archetype-specific overlays. The gameplay shape lives in its own `gameplay.yaml` file and serves as the inheritance root for all profiles in `profiles.yaml`. Adds new components (`quick_win_bonus`, `stall_penalty`, `breeding_digivolve`, `digivolve_driven_attack`), telemetry surfaces, and a two-file loader. Replaces the byte-identical-legacy guarantee with explicit re-baselining on resume.
