# activated-digivolve-execution Specification

## Purpose

Define how a hand card declaring a `kind: activated_digivolve` alternate path is
offered to the action space and executed: a digivolve action against each
field permanent the alt-path can target, resolving at the alt-path's declared
cost while ignoring printed digivolution requirements. This unblocks Medusamon
tier-3 cards that rely on activated-digivolve alternate paths.
