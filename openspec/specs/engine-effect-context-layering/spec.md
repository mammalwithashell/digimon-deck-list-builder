# engine-effect-context-layering Specification

## Purpose

Defines the engine's effect-execution tier contract: a 3-tier structure (Tier-1 core `Game`, Tier-2 operations `game_actions`, Tier-3 scripting facade `EffectContext`) organized by mechanic, with rules machinery confined to Tier 2, no layering inversions, a shared source-trashing primitive, and a read-only observation output port. These are enforceable structural invariants (the placement lint at `code/tools/check_facade_placement.py` checks a subset); the contract keeps the engine's god-files decomposed and new mechanics landing at a predictable `<tier>/<mechanic>` address.
