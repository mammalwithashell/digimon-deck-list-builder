...
- [x] 6.2 Retire `docs/RUST_PYTHON_PARITY.md`; update `docs/ARCHITECTURE.md` / `docs/DEPLOYMENT.md`. **— done; RUST_PYTHON_PARITY.md gets a RETIRED banner (kept as historical record). CLAUDE.md updated: Tech Stack/System Overview/Project Layout entries removed, rule 22 rewritten to "removed", the stale parity-test command dropped. Dockerfile no longer COPYs `engine_py_legacy` (hosted-API image is now Python-legacy-free).**
- [x] 6.3 **Delete `code/engine_py_legacy/`** (the final gate). **— DONE. `git rm -r code/engine_py_legacy` (3783 files) + removed leftover untracked `__pycache__`. `test_rust_backend_parity.py` was NOT migrated — it is a dual-backend test (Python `HeadlessGame` vs Rust) pinned to the retired `standard_compact_v1` profile; its Python half is gone and its Rust-shape coverage is redundant with `code/tests/test_rust_bindings_surface.py` (55 tests) + the ffi_parity Rust tests, so it dies with the dir. pyproject: dropped `--ignore=code/engine_py_legacy`, the `norecursedirs` line, the `engine_py_legacy*` packages-find exclude, and the now-unused `scenario`/`behavioral` markers. The 3 remaining `engine_py_legacy` references in `code/tests` are deletion-safe by design (negative-control imports that now ImportError naturally; an `importorskip` parity gate that now skips).**
