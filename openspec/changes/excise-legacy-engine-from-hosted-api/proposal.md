> **STATUS: DEFERRED — DO NOT IMPLEMENT.** This proposal exists to capture scope, risk, and a phasing sketch for severing the Python legacy engine from the hosted API. It is **not** scheduled. No task in `tasks.md` should be started without an explicit decision to prioritize this work. The actionable sibling change is `make-training-build-legacy-free` (training build only); this one (the server) is the larger, parked counterpart.
