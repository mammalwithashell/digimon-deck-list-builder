> **STATUS: COMPLETE (2026-06-15).** The hosted API runs **all** gameplay on the Rust engine, `code/engine_py_legacy/` has been **deleted**, and `code/server/` imports zero `engine_py_legacy` (guardrail-enforced; the whole Python legacy engine is gone). Phases 1–3 + 5 were delivered by `shrink-legacy-engine-surface`; phase 4 (live PvP) was a full cutover to the `RustInteractiveGame` adapter; the Rust `GameEvent` set is fully wired (4.4); the directory + config + docs were retired (6.x). The "differential-test against the Python path" gate was dropped — the Python engine was bit-rotted and could not serve as an oracle (design findings, 2026-06-14); the Rust faithfulness suite + new interactive-wire tests were the gate instead. **One follow-on noted, out of scope:** engine `GameEvent`s use PascalCase while the frontend animation components match snake_case (some with no Rust equivalent), so engine events do not drive frontend animations without a separate frontend-adaptation task (pre-existing; affects desktop too).
