...
- [~] 7.1 Author the missing cards: Lilithmon EX6-057 done (grant wave); **Q15 wave done 2026-06-11: BT19-073 LordKnightmon (X Antibody), BT17-016 Gallantmon, BT12-016 WarGrowlmon, EX3-057 Growlmon (all IMPLEMENTED) + EX8-073 Gallantmon (X Antibody) re-authored with a real `effect_immunity` aura (new DSL aura slot)**. Still TODO for Q29: Yuu Amano BT10-093 + EX10-031/-039/-044/-056/-059 (DarkKnightmon/ChuuChuumon/Damemon/Bagramon/DarknessBagramon; three need `G-DSL-HAND-MAIN-SELF-PLAY-REDUCED`). Paildramon BT16-025 already implemented.
- [~] 7.2 **Q15 DONE 2026-06-11** (`e::q15_sequential_de_digivolve_halted_by_x_antibody_immunity` PASSES — N separate `<De-Digivolve 1>` instances, per-card immunity re-check, Gallantmon X topmost, exactly 1 trashed); Q16 + Q25 done in prior waves. Q29 pending its cards.
- [~] 7.3 Sequential de-digivolve respecting newly-acquired immunity: pinned via BT19-073's per-instance loop + EX8-073's live `effect_immunity` aura — no NEW engine gap (the per-card immunity re-check already held once the immunity was a real aura). `<Partition>` cause-filter + DigiXros departure resolved in prior waves. Remaining: DigiXros placement-order legality (Q29).
- [ ] 7.4 Confirm cluster E tests green; archive closed gaps (Q15/Q16/Q25 green; Q29 pending)
