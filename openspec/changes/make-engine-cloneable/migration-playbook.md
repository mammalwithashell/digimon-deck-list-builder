...
## Current state (updated 2026-06-23 — the playbook's DSL card-effect selection-STEP installers (Batches 0–4) are flipped; the BROADER engine selection surface is NOT — see the CORRECTION below)

> **CORRECTION (2026-06-23, `cutover-selection-site-audit`).** This playbook's scope was the DSL card-effect **selection-step** installers (the ~16 below). An earlier "100% of the production selection surface" framing was **OVERSTATED**: the audit found the playbook undercounted even the DSL installers (~9 more — `select_effect_choice`, `use_option_from_hand`, `choose_from_reveal`, `dna_pair`, `order_remainder`, `remainder_permutation_with_tail`, two tamer-cost helpers, + `combat.rs`/`link_cards.rs`/`zone_moves.rs` step files) AND that the entire **non-DSL** selection surface is still closure-based: the `EffectContext::select_*` primitive API, combat/keyword interrupts (`keyword_effects.rs`+`combat/mod.rs`), digivolve cost-choice/reducers, BO3 play-order, Overclock, TriggerOrder/optional-trigger, replacement-accept, Delay, `effect_context/action/*`. `Game: Clone` is faithful **only at the flipped DSL select kinds**; a clone at a combat/digivolve/play-order/`select_effect_choice` node panics on resolve. The cutover (task 6.1) is blocked by these ~12 subsystems + ~9 installers. Substrate + most-common-path DSL selections done; broad surface remains.
>
> **UPDATE (2026-06-24) — the audit's ~9 extra DSL installers are now flipped (full-suite-gated each).** `select_effect_choice`, `choose_from_reveal`, `remainder_permutation_with_tail` + single-dest `order_remainder`, `use_option_from_hand`, `dna_pair`, and both tamer-trash-cost helpers (incl. multi-pick `trash_n`, via Design 3 — a `FieldPermanent{post:Trash}` frame whose tail re-enters the count-decrementing step, no new frame) are on the VM; the step-file sites `zone_moves.rs` (MayAddTopSecurityToHand → `SecurityPostAction`), `combat.rs:152` (redirect → new `AttackTarget` kind), and `combat.rs:93/102/124` (may_attack/force-attack → new `BeginAttack` kind) are flipped too. **DSL surface now complete EXCEPT `link_cards.rs`** (recursive 5-stage pick-loop, designed in `.claude/plans/make-engine-cloneable-remaining-installers.md` §#8). The **non-DSL** surface (b) is unchanged and remains the larger pre-cutover effort. Lessons banked: mutation-per-pick → re-express the next pick as a CompiledStep tail (not `after_selection_resume_hooks`, which fires too early); a sub-machine post-action (begin_attack_open) still threads via `outer_conts`/`drain_or_rewrap`; a single-caller primitive → park in the step handler.
>
> **UPDATE (2026-06-27) — MULTI-destination `order_remainder` flipped.** `ResumeSelectKind::EffectChoice` extended unit → `EffectChoice { post: Option<EffectChoicePostAction> }` + new `EffectChoicePostAction::OrderRemainder { positions, player }` (plain data). `post: Some(OrderRemainder)` maps the chosen top/bottom index → deck position and chains into the already-flipped `install_remainder_permutation_with_tail` (parks its own `PermutationStep`) — frame-installs-frame, same shape as `DnaPairLeft`; `inner_tail` becomes the permutation's tail, `outer_conts` thread via `run_outer_conts`, decline `None`. Gate: `cards_behavioral` 5830/5830, `dsl` 782/782, `archetypes` 211/211, lib `resume` 17/17, new `bt25_063_order_remainder_dest_choice_clones_faithfully`. **`link_cards.rs` is now the LAST DSL-surface site.**
>
> **UPDATE (2026-06-28) — `link_cards.rs` flipped; the DSL selection surface is COMPLETE.** `try_run_relink` (relink, EX11-027) → new `FieldPermanentPostAction::AbsorbStandingAsLink { source }` (reuses the FieldPermanent arm, no new frame). The recursive LOOP → new `ResumeFrame::LinkPickStep(LinkPickState)` + 4-variant `LinkStage` (`ZoneChoice`/`CardSelectHandTrash`/`CardSelectSources`/`HostSelect`); each stage's `install_*` parks the frame via `park_link_frame`, and `run_link_pick_step` decodes per stage + advances by re-entering the same `install_*` helpers, with the post-attach `install_pick(K+1)` inline (the design crux dissolved — clone-safety covers clones at a pick prompt, NOT a nested OnLink park; no continuation channel). Because the park sets `pending_selection_resume` on every link select, the 63 pre-existing link tests now exercise the resume driver and stand as the parity gate. Gate: `cards_behavioral` 5830/5830, `dsl` 782/782, `archetypes` 211/211, `option_flow` 133/133, lib `resume` 17/17, + 4 new clone tests (relink + all 3 loop stages). **A clone paused at ANY DSL selection kind is now faithful; only the non-DSL surface (b) remains for the cutover.**
...
**Result: every multi-pick selection TRAMPOLINE (the playbook's set) is now on the resumable VM.** `Game: Clone` is faithful at the flipped DSL card-effect selection-step kinds — but NOT at the broader surface (`select_effect_choice` + other DSL installers + combat/keyword/game-action/replacement selections). See the CORRECTION in "Current state".
...
## Batch 2 — Moderate RunTail flips (DONE 2026-06-22, commit 960d7dac2)

> **Learning:** flipping `own_permanent` exposed the SECOND general substrate gap — the resume-side continuation channel (see "Current state"). Five callback-continuation wrappers compose onto the bypassed closure; all are now resume-aware. With that + the Batch-1 nested-composition substrate, the remaining RunTail flips are genuinely mechanical.
...
- **B0-2 — resume-side continuation channel (SOLVED in Batch 2).** A DIFFERENT closure-bypass family than B0: ~5 callback-continuation wrappers (`wrap_pending_play_cost_continuation`, the digivolve/option interactive-reducer continuations, `arm_digixros_resume_after_parked_leave`, `run_after_selections_drain`) compose a continuation onto the selection's closure `callback`/`on_decline`. A flipped (resume-driven) select bypasses all of them. Now handled: each wrapper defers its continuation onto `Game.after_selection_resume_hooks` (`resume::ResumeContinuationHooks`) when the parked select is resume-driven; `resolve_generic_selection` drains them right after `run_resume`. If you add a NEW callback-continuation wrapper, make it resume-aware too (same `if pending_selection_resume.is_some() { defer } ` guard) — else a flipped select silently drops its continuation. (No tripwire here: a missed wrapper fails as a *missing continuation*, caught by `cards_behavioral` parity, not a panic.)
