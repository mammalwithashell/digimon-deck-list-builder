...
- [ ] 1.1 Add `then: Vec<StepSpec>` to the field/zone select arg structs: `SelectFieldArgs` (select_*_permanent), `SelectZoneArgs` (select_hand/select_trash/select_security). Mirror the existing seven-struct lowering exactly (tail runs with the selection binding in scope). **See the implementation note above** re: composing the explicit `then` with the implicit dispatcher tail.
