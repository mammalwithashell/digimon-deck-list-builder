...
- [x] 10.2 **DONE** — full `game.rs` mechanic split. `game/mod.rs` 4657 → 2748 (remainder is mostly the `struct Game` field block); 48 more methods moved into 8 submodules (setup, memory, lifecycle, staging, handles, triggers, opt, suspend) joining queries/until_condition — 10 Tier-1 mechanic modules total. Core accessors + struct def + try_replace/resolve_selection stay in mod.rs. Gate: 3548/7 (baseline), PyO3 clean, lint 0. Commit `675c5349`.
