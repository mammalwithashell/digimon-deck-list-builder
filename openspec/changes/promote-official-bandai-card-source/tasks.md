...
- [~] 7.5 (broad sweep, in progress) `audit_alt_evo.py` + a 22-agent verification workflow over the partial-mirror candidates → 19 fix / 2 false-positive (BT19-012, BT21-021 already correct) / 1 engine-gap (EX9-012, "[X] in text" name match). APPLIED 7 clean trait-alt fixes (BT22-017, BT22-041, BT23-035, BT23-057, BT24-062, BT25-033, BT25-069) — pack compiles, their behavioral tests (55) + dsl suite (782) green. DEFERRED 12 for careful per-card handling: name_contains "[X] in name" (AD1-004, BT22-013, BT22-026, EX9-019), DNA/burst (BT22-015, BT22-025, BT22-052), cost change/conflict (BT21-030 3→5, BT24-101 TS cost 3-vs-official-5 conflicts with existing tests), combined colour+trait gate (EX11-022), cost-0 component (BT23-047).
- [ ] 7.6 (broad sweep, pending the full-pool scrape) Re-run `audit_alt_evo.py` against the complete official mirror to catch alt-evo authoring bugs in cards not yet in the partial mirror; verify-and-fix the new candidates the same way.
