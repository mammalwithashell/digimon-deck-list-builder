...
- [ ] 7.3 (follow-up, surfaced by the cost-choice suite) P-215 Icemon's printed alt "[Digivolve] Lv.3 w/[Ice-Snow]/[Mineral]/[Rock] trait: Cost 2" is authored COLOUR-gated (`from: { color_is: blue/black }`) in `code/digimon-engine/cards/p/P-215.yaml` instead of trait-gated — so the cheaper evo is BOTH hidden as-printed AND over-permissive (any blue/black Lv.3 base gets cost 2, even without the trait). Re-author the alt as `trait_has`-gated and add it to the cost-choice suite.
