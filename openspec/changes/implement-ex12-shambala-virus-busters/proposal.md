...
This set is also the first authored **without the DCGO behavioral oracle** (no community C# exists for EX12) and without official-DB bundles (`world.digimoncard.com` is mid-restructure and unscrapeable), so the change must codify a scan-grounded authoring discipline: printed card scans + official rulings via the wiki + `general_rule.pdf`, with conservative BLOCKED verdicts over guesses.
...
- **54 card implementations** as YAML DSL specs + per-card DebugRunner behavioral tests, in review-gated waves (implementer → Opus scan-grounded reviewer), including the DUAL EX12-018 Siriusmon (ST23-09/BT25-043 dual-YAML shape) and the Lv.7s.
...
- **Dependencies/risk**: no DCGO oracle for EX12 (first set authored scan-first); official DB down (no bundles); usage-credit availability gates the agent-driven phases.
