...
- [~] 0.4 Pre-register robustness baselines — LAUNCHED 2026-07-01 on RunPod A40 pod `baselines-04` (`dye0p7m8juhqnl`, image `training-v0.46`, no volume — **harvest before terminate**). Package (fixed, pre-registered): 3 targets (`starter_pool_single_v1` MLP + registry champions `v022-generalist-v1`/`v020-generalist-v1` LSTM) × {anchored panel n=300 (greedy + other champions, self-excluded registries, `starter_pool_single_v1` deck-pool snapshot, layout a20462fb) + exploiter at fixed budget **1,000,000 steps**}. Driver: `/workspace/run_baselines.sh` (two staggered lanes; sentinels `ANCHORED_DONE`/`EXPLOITER_DONE`/`ALL_DONE` in `/workspace/baselines/`). Harvest: scp `/workspace/baselines/` → commit numbers to `baselines.md` in this change dir, then `runpodctl pod delete dye0p7m8juhqnl` MANUALLY (no auto-teardown watchers — see league-incident memory).
