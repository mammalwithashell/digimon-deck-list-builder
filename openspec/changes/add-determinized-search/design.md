...
- **DSL consolidations first, then Tier 0, then this.** `make-engine-cloneable` is itself sequenced after the in-flight DSL changes (`collapse-dsl-step-idioms`, `unify-dsl-scalar-and-comparators`, `fix-dsl-substrate-rot-and-bugs`; see that change's task 0.1). This lane inherits that ordering transitively.
