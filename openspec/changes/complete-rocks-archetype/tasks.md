...
- [ ] 4.1 Write a failing DSL/behavioral test for a `move_from_breeding` step with an optional accept/decline prompt and a breeding-permanent `filter`
- [ ] 4.2 Add the `move_from_breeding` step (`StepSpec` → `CompiledStep` → lowering) in `code/digimon-dsl/src/` over the existing `EffectContext::move_from_breeding_by_effect`
- [ ] 4.3 Implement the optional level-filtered prompt wrapper so the choice surfaces through `pending_selection` (CLAUDE.md §17); pair it with the already-wired `SelectOwnBreedingPermanentArgs::filter`
- [ ] 4.4 Confirm tests pass; add regression coverage under `tests/dsl/` / `tests/selection/`

## 5. B3 — union-zone cost selector (EX11-065)

- [ ] 5.1 Write a failing test for a cost selector spanning hand ∪ own digivolution-card sources with a single trait filter, trashing the chosen card
- [ ] 5.2 Decide whether the union selector composes existing `SelectionKind`s or needs a new kind; if new, append an additive pending-selection action sub-range (never renumber) and update `docs/ACTION_SPEC.md`
- [ ] 5.3 Implement the union-zone cost selector in `code/digimon-engine/src/effect_context/` parameterised by zone set (hand∪source for EX11-065; also expressible as hand∪trash for Royal Knights `G-UNION-HAND-TRASH-SOURCE-COST`)
- [ ] 5.4 Wire DSL parse → compile → lowering for the union-zone cost verb
- [ ] 5.5 Confirm tests pass (including the no-eligible-card negative case); add regression coverage

## 6. B4 — face-up security lifecycle (BT20-055)

- [ ] 6.1 Write failing tests for (a) a no-choice `flip_security_face_up` step and (b) a "when your Digimon checks a face-up security card" observer timing
- [ ] 6.2 Implement the `flip_security_face_up` primitive in the engine security module + its DSL step
- [ ] 6.3 Implement the face-up-security-check observer timing: dispatch wiring + DSL `when:` token
- [ ] 6.4 Confirm tests pass; move the closed face-up-security entries from `docs/RUST_ENGINE_GAPS.md` to `qa/resolved-gaps.md`

## 7. B5 — Delay-on-attack trigger support (BT23-096)

- [ ] 7.1 Write failing tests for a `<Delay>` clause armed by an attack event, including the attacker-trait condition
- [ ] 7.2 Extend `lower_delay.rs` to map attack timings (`OnAllyAttack` et al.) to an `OnEvent` `DelayTrigger` instead of degrading to `EndOfYourNextTurn`
- [ ] 7.3 Extend combat dispatch (`combat.rs` / `effect_queue.rs`) to fan attack events out to event-gated delayed options
- [ ] 7.4 Add a delay-context attacker predicate so the activation condition can read the attacking Digimon's traits for a plain attack
- [ ] 7.5 Confirm tests pass; run the full `combat` and `option_flow` suites as the hot-path regression gate
...
- [ ] 8.2 P-130 — author the `[On Play]` move-from-breeding clause with the optional level-3+ prompt; remove the stale `#[ignore]`s
- [ ] 8.3 EX11-065 — author Clause 0 `[Start of Your Main Phase]` with the union-zone cost selector; remove the stale `#[ignore]`s
- [ ] 8.4 EX11-038 — author the `[When Moving]/[On Play]` union-zone Draw clause (same B3 selector as EX11-065)
- [ ] 8.5 BT20-055 — author the face-up-security-flip rider and the omitted `[Your Turn]` self-security-placement clause; add the previously-missing behavioral tests
- [ ] 8.6 BT23-096 — author the `[Your Turn]` CS-attack `<Delay>` clause; remove the stale `#[ignore]`
...
- [ ] 10.1 `G-EVENT-TARGET-LEVEL-LTE` — add `event_target_level_lte/eq/gte` predicate family to `PredicateSpec` (`code/digimon-dsl/src/predicate.rs`), wire through compile/lowering/evaluator; then author both BT8-094 clauses
- [ ] 10.2 `G-ON-OPTION-TRASHED-DSL` — add `Timing::OnOptionTrashed` to the DSL `Timing` enum (`code/digimon-dsl/src/clause.rs`) + lowering to the engine's existing `EffectTiming::OnOptionTrashed`; then author BT23-059 Clause B
- [ ] 10.3 `G-HIGHEST-PLAY-COST-SELECTOR` — add the `HighestPlayCost` sibling to `FieldSelector`/`CompiledFieldSelector` + evaluator (mirrors the resolved `LowestPlayCost`); then author EX11-044 Clause A
- [ ] 10.4 `event_host_permanent_is_source` — add the DSL predicate gating `OnDigivolutionCardTrashed` to the observer-is-host case; then author EX11-044 Clause B
- [ ] 10.5 `G-DSL-GRANT-TRIGGERED-EFFECT-TO-BINDING` — add a DSL verb that grants a printed triggered ability ("[Start of Your Main Phase] This Digimon attacks") to a selected permanent; then author EX10-034 Clause A
- [ ] 10.6 EX8-050 play-from-reveal-free — confirm/close the gap blocking the "you may play 1 from the revealed cards free" sub-step; then author that sub-step

## 9. Phase D — reconcile and verify

- [ ] 9.1 Finalize `qa/qa-reports/validated_cards_dsl.json`: every Rocks card has a verified verdict matching `cargo test` reality
- [ ] 9.2 Move all gaps closed by B1–B5 from `qa/dsl-vocab-gaps.md` / `qa/archetype-qa/engine-gaps.md` to `qa/resolved-gaps.md` with resolution notes + test commands
- [ ] 9.3 Update `qa/archetype-qa/dsl/rocks.md` with the final per-card verdict table
- [ ] 9.4 Confirm no Rocks behavioral test carries an `#[ignore]` citing a closed gap
- [ ] 9.5 Run the full `cargo test --manifest-path code/digimon-engine/Cargo.toml` suite — must be green with no regressions
